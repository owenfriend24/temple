...
def _verified_reverse1(mapper, onesample):
    ...
    if not len(rsample) == 1:
        warning(
            "Reverse mapping single sample yielded multiple -- can lead to unintended behavior!"
        )
    return rsample[0]
...
class Mapper(Learner):
    ...
    def _forward_dataset(self, dataset):
        ...
        if __debug__:
            debug(
                "MAP_",
                "Forward-map %s-shaped samples in dataset with '%s'."
        ...
        if __debug__:
            debug(
                "MAP_",
                "Make shallow copy of to-be-forward-mapped dataset "
                "and assigned forward-mapped samples ({sf}a_filters: "
                "%s, %s, %s)." % (self._sa_filter, self._fa_filter, self._a_filter),
            )
        mds = dataset.copy(
            deep=False, sa=self._sa_filter, fa=self._fa_filter, a=self._a_filter
        )
    ...
    def _reverse_dataset(self, dataset):
        ...
        if __debug__:
            debug(
                "MAP_",
                "Reverse-map %s-shaped samples in dataset with '%s'."
                % (dataset.samples.shape, self),
            )
        ...
        if __debug__:
            debug(
                "MAP_",
                "Make shallow copy of to-be-reverse-mapped dataset "
    ...
    def forward(self, data):
        ...
        if is_datasetlike(data):
            if __debug__:
                debug(
                    "MAP",
                    "Forward-map %s-shaped dataset through '%s'." % (data.shape, self),
                )
    ...
    def forward1(self, data):
        ...
        if __debug__:
            debug(
                "MAP",
                "Forward-map single %s-shaped sample through '%s'."
                % (data.shape[1:], self),
            )
    ...
    def reverse(self, data):
        ...
        if is_datasetlike(data):
            if __debug__:
                debug(
                    "MAP",
                    "Reverse-map %s-shaped dataset through '%s'." % (data.shape, self),
                )
    ...
    def reverse1(self, data):
        ...
        if __debug__:
            debug(
                "MAP",
                "Reverse-map single %s-shaped sample through '%s'."
                % (data.shape[1:], self),
            )
        ...
        if __debug__:
            debug(
                "MAP",
                "Mapped single %s-shaped sample to %s."
    ...
    def forward1(self, data):
        ...
        for m in self:
            if __debug__:
                debug(
                    "MAP",
                    "Forwarding single input (%s) though '%s'." % (mp.shape, str(m)),
                )
            mp = m.forward1(mp)
        return mp
        ...
        for m in reversed(self):
            ...
            try:
                if __debug__:
                    debug(
                        "MAP",
                        "Reversing %s-shaped input though '%s'." % (mp.shape, str(m)),
                    )
                mp = m.reverse(mp)
            ...
            try:
                ...
                if __debug__:
                    debug(
                        ...
                        "switch to reverse() for the remainder of the chain." % str(m),
                    )
...
class CombinedMapper(Mapper):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(CombinedMapper, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["mappers", "combine_axis", "a"])
        )
