...
def _suppress_scipy_warnings():
    ...
    if not __debug__ or (__debug__ and "PY" not in debug.active):
        filter_lines = []
        if "0.6.0" <= scipy_ver < "0.7.0" and numpy_ver > "1.1.0":
            if __debug__:
                debug(
                    "EXT",
            ...
            filter_lines += [
                ("NumpyTest will be removed in the next release.*", DeprecationWarning),
                ("PyArray_FromDims: use PyArray_SimpleNew.", DeprecationWarning),
                (
...
def __assign_nibabel_version():
    try:
        ...
        if "We had not expected long double type <type 'numpy.float128'>" in e_str:
            warning(
                "Must be running under valgrind?  Available nibabel experiences "
...
def __check_pywt(features=None):
    ...
    try:
        for node in wp.get_level(2):
            wp2[node.path] = node.data
    except:
        raise ImportError("Failed to reconstruct WP by specifying data in the layer")

    if "wp reconstruct fixed" in features:
        rec = wp2.reconstruct()
        if np.linalg.norm(rec[: len(data)] - data) > 1e-3:
            raise ImportError("Failed to reconstruct WP correctly")
    return True
...
def __check_libsvm_verbosity_control():
    ...
    try:
        ...
        raise ImportError(
            "Provided version of libsvm has no way to control " "its level of verbosity"
        )
...
def __check_shogun(bottom_version, custom_versions=None):
    ...
    if (ver in custom_versions) or (ver >= bottom_version):
        ...
        raise ImportError(
            "Version %s is smaller than needed %s" % (ver, bottom_version)
        )
...
def __assign_skl_version():
    try:
        ...
        if skl.__doc__ is None or skl.__doc__.strip() == "":
            raise ImportError(
                "Verify your installation of scikits.learn. Its "
...
def __check_scipy_weave():
    ...
    try:
        ...
        raise ImportError(
            "Weave cannot be used due to failure to import because of %s" % e
        )
    ...
    try:
        ...
        fmsg = "Failed to build simple weave sample." " Exception was %s" % str(e)
...
def __check_atlas_family(family):
    ...
    if not os.path.exists(filename):
        raise ImportError(
            "Cannot find file %s for atlas family %s" % (filename, family)
        )
    pass
...
def __check_stablerdist():
    ...
    try:
        ...
        if "rdist_gen" in dir(distributions) and (
            "_cdf" in list(distributions.rdist_gen.__dict__.keys())
        ):
...
def __check_rv_continuous_reduce_func():
    ...
    try:
        ss.t.fit(np.arange(6), floc=0.0, fscale=1.0)
    except IndexError as e:
        raise RuntimeError("rv_continuous.fit can't candle 2 fixed params: %s" % str(e))
    ...
    if backend:
        ...
        if mpl_backend != backend.lower():
            ...
            warnings.filterwarnings(
                "ignore", "This call to matplotlib.use() has no effect.*", UserWarning
            )
        elif __debug__:
            debug(
                "EXT_",
...
def __check(name, a="__version__"):
    ...
    try:
        ...
        if __debug__:
            debug("EXT", "Failed to acquire a version of %(name)s: %(e)s" % locals())
...
def __check_rpy():
    ...
    if not cfg.getboolean("rpy", "interactive", default=True) and (
        rpy.get_rpy_input() is rpy.rpy_io.rpy_input
    ):
        if __debug__:
...
def _R_library(libname):
    ...
    try:
        if not tuple(
            ro.r(
                "suppressMessages(suppressWarnings(require(%r, quiet=TRUE)))" % libname
            )
        )[0]:
            raise ImportError("It seems that R cannot load library %r" % libname)
    except Exception as e:
        raise ImportError("Failed to load R library %r due to %s" % (libname, e))
...
_KNOWN = {
    "libsvm": "import mvpa2.clfs.libsvmc._svm as __; x=__.seq_to_svm_node",
    ...
    "lxml": "__check('lxml.etree', '__version__');" "from lxml import objectify as __",
...
def exists(dep, force=False, raise_=False, issueWarning=None, exception=RuntimeError):
    """
    ...
    if isinstance(raise_, str):
        if raise_.lower() == "always":
            ...
            raise ValueError(
                "Unknown value of raise_=%s. " "Must be bool or 'always'" % raise_
            )
    else:  # must be bool conditioned on the global settings
        raise_ = raise_ and cfg.getboolean("externals", "raise exception", True)

    # prevent unnecessary testing
    if (
        cfg.has_option("externals", cfgid)
    ...
    if dep not in _KNOWN:
        ...
        if __debug__:
            ...
            debug(
                "EXT",
                "Presence of %s%s is%s verified%s"
    ...
    if not result:
        if raise_:
            raise exception("Required external '%s' was not found" % dep)
        if issueWarning is not None and cfg.getboolean(
            "externals", "issue warning", True
        ):
...
def check_all_dependencies(force=False, verbosity=1):
    ...
    if __debug__:
        debug(
            "EXT",
            "The following optional externals are present: %s"
            % [
                k[5:]
                for k in cfg.options("externals")
                if k.startswith("have") and cfg.getboolean("externals", k)
            ],
        )
