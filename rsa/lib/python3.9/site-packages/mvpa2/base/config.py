...
class ConfigManager(SafeConfigParser):
    ...
    def __init__(self, filenames=None):
        ...
        for sec, vars in list(ConfigManager._DEFAULTS.items()):
    ...
    def reload(self):
        ...
        if not os.path.exists(user_configfile) and os.path.exists(user_configfile_old):
            ...
            warn(
                "You seems to have a configuration file %s for previous "
                "version of PyMVPA but lacking configuration for PyMVPA2. "
                "Consider copying it into %s" % (user_configfile_old, user_configfile)
            )
