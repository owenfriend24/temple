...
class Parameter(IndexedCollectable):
        ...
        **kwargs
        ...
        if allowedtype is not None:
            warnings.warn(
                ...
                "Adjust your code, provided value '%s' was ignored" % str(allowedtype),
        ...
        IndexedCollectable.__init__(
            self, index=index, name=name, doc=doc  # value=value,
        )
        self._isset = False
        if value is None:
    ...
    def __reduce__(self):
        ...
        res = (
            self.__class__,
            (self.__default, self.constraints, self._ro) + icr[1],
            state,
        )
    ...
    def __repr__(self):
        ...
        plist = [
            "%s=%s" % (p, self.__getattribute__(p)) for p in self._additional_props
        ]
        if len(plist):
            s += ", " + ", ".join(plist)
        if self._ro:
    ...
    def _set(self, val, init=False):
        ...
        if self._ro and not init:
            raise RuntimeError(
                "Attempt to set read-only parameter %s to %s" % (self.name, val)
            )
        if (isarray and np.any(different_value)) or ((not isarray) and different_value):
            if __debug__:
            ...
            debug(
                "COL", "Parameter: not setting %s since value is the same" % (str(self))
            )
