...
#
### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ### ##
r"""Query atlases for anatomical labels of voxel coordinates, or their statistics
...
def parsed_coordinates_iterator(
    ...
    for line in inputStream.readlines():
        ...
        if not match:
            if __debug__:
                debug("ATL", "Line '%s' did not match '%s'" % (line, parseString))
        else:
            r = match.groupdict()
            if "v" in r:
                v = dtype(r["v"])
...
def get_summary(args, summary, output):
    ...
    sort_keys = [(k, len(v["values"]), v["maxcoord"][1]) for k, v in list(summary.items())]
    ...
    sort_keys.sort(
        cmp=lambda x, y: cmp(x[sort_index], y[sort_index]), reverse=sort_reverse
    )
    ...
    for index in keys:
        ...
        if N < args.countThreshold or Npercent < args.countPercentThreshold:
            continue
        count_reported += N
        msg = "%%%ds:" % maxkeylength
        output.write(msg % index)
        output.write("%4d/%4.1f%% items" % (N, Npercent))
        ...
        if args.createSummary > 3 and "distances" in summary_:
            ...
            Nr, mean, std, minv, maxv, ssummary = statistics(summary_["distances"])
            Nr = len(summary_["distances"])
            # print "N=", N, " Nr=", Nr
            output.write(
                " Referenced: %d/%d%% Distances: %s"
                % (Nr, int(Nr * 100.0 / N), ssummary)
            )
        output.write("\n")
...
def setup_parser(parser):
    ...
    parser.add_argument(
        ...
        dest="atlasName",
        default="talairach",
        choices=list(KNOWN_ATLASES.keys()),
        help="Atlas to use. Choices: %s" % ", ".join(list(KNOWN_ATLASES.keys())),
    )
    ...
    parser.add_argument(
        ...
        help="How to sort summary entries. " " a-p sorts anterior-posterior order",
    )
    ...
    parser.add_argument(
        ...
        dest="MNI2TalTransformation",
        default="matthewbrett",
        help="Choose between available transformations from mni " "2 talairach space",
    )
    ...
    parser.add_argument(
        ...
        "--show-target-voxel",
        action="store_true",
        dest="showTargetVoxel",
        help="Show target coordinates",
    )

    parser.add_argument(
        "--rc",
        "--show-referenced-coord",
        action="store_true",
        dest="showReferencedCoordinates",
    ...
    parser.add_argument(
        "-F",
        "--forbid-direct-mapping",
        action="store_true",
        dest="forbidDirectMapping",
        default=False,
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="halfVoxelCorrection",
        default=False,
        help="Adjust coord by 0.5mm after transformation to " "Tal space.",
    )
    ...
    parser.add_argument(
        ...
        help="Which level to reference in the case of reference" " atlas",
    )
...
def run(args):
    ...
    # input is stdin
        ...
        coordsIterator = parsed_coordinates_iterator(args.inputLineFormat, ctype=ctype)
    ...
    if (
        not args.forbidDirectMapping
        and niftiInput is not None
        and not args.transformationFile
    ):
        ...
        verbose(
            1,
            "Will attempt direct mapping from input voxels into atlas "
        ...
        if atlas.space != args.inputSpace:
            ...
            niftiInput.header.get_qform() == atlas._image.header.get_qform()
        ).all():
            if args.atlasImageFile is None:
                ...
                warning(
                    ...
                    % (
                        infile,
                        niftiInput.header.get_qform(),
                        args.atlasImageFile,
                        atlas._image.header.get_qform(),
                    ),
                    1,
                )
        else:
    ...
    if not (
        args.inputSpace == atlas.space
        or (args.inputSpace in ["MNI", "Talairach"] and atlas.space == "Talairach")
    ):
        raise XMLAtlasException(
            "Unknown space '%s' which is not the same as atlas "
    ...
    if query_voxel:
        ...
        if args.transformationFile:
            ...
            verbose(
                2,
                "coordT got linear transformation from file %s"
        ...
        if not args.coordRelativeToOrigin:
            if args.inputSpace == "Talairach":
                ...
                warning(
                    "Assuming elderly sizes for MNI volumes with"
        ...
        if not (voxelOriginOriginal is None and voxelSizeOriginal is None):
            verbose(
                2,
                "Assigning origin adjusting transformation with"
                + " origin=%s and voxelSize=%s"
                % (repr(voxelOriginOriginal), repr(voxelSizeOriginal)),
            )

            coordT = SpaceTransformation(
                origin=voxelOriginOriginal,
        ...
        if args.inputSpace == "MNI" and atlas.space == "Talairach":
            verbose(2, "Assigning transformation %s" % args.MNI2TalTransformation)
            # What transformation to use
            coordT = {
                "matthewbrett": MNI2Tal_MatthewBrett,
    ...
    if args.levels is None:
        ...
        raise ValueError(
            "Don't know how to handle list of levels %s."
    ...
    if args.dumpmapFile:
        if niftiInput is None:
            raise RuntimeError(
                "You asked to dump indexes into the volume, "
                "but input wasn't a volume"
            )
            sys.exit(1)
        ni_dump = nb.load(infile)
        ni_dump_data = np.zeros(
            ni_dump.header.get_data_shape()[:3] + (len(args.levels),)
        )
    ...
    for c in coordsIterator:
        ...
        if __debug__:
            debug(
                "ATL",
                "Obtained coord_orig=%s with value %s" % (repr(coord_orig), value),
            )
        ...
        if lt is not None and value < lt:
            verbose(
                5,
                "Value %s is less than lower threshold %s, thus voxel "
        ...
        if ut is not None and value > ut:
            verbose(
                5,
                "Value %s is greater than upper threshold %s, thus voxel "
        ...
        if args.createSummary:
            ...
            if "voxel_referenced" in voxel:
                if voxel["voxel_referenced"] and voxel["distance"] >= 1e-3:
                    verbose(
                        5,
                        "Appending distance %e for voxel at %s"
