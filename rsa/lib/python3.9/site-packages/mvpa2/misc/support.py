...
def transform_with_boxcar(data, startpoints, boxlength, offset=0, fx=np.mean):
    ...
    for sp in startpoints:
        if (sp + offset + boxlength - 1 > len(data) - 1) or (sp + offset < 0):
            raise ValueError(
    ...
    selector = [list(range(i + offset, i + offset + boxlength)) for i in startpoints]
...
def __xrandom_unique_combinations(L, n, k=None):
    ...
    if k is not None:
        ...
        for i in npr.permutation(len(L) - n + 1):
            for cc in xrandom_unique_combinations(npr.permutation(L[i + 1 :]), n - 1):
                yield [L[i]] + cc
...
def xrandom_unique_combinations(L, n, k=None):
    ...
    if (ncomb < 1e6 or k > math.sqrt(ncomb)) and sys.version_info[:2] >= (2, 6):
...
def version_to_tuple(v):
    ...
    regex = re.compile("(?P<numeric>[0-9]*)" "(?P<alpha>[~+-]*[A-Za-z]*)(?P<suffix>.*)")
    for x in v:
        try:
            vres += [int(x)]
        except ValueError:
...
class SmartVersion(Version):
    ...
    def __cmp__(self, other):
        if isinstance(other, (str, tuple, list)):
            ...
            raise ValueError("Do not know how to treat version %s" % str(other))
            ...
            for x, y, mult in ((si, oi, 1), (oi, si, -1)):
                if x is None:
                    ...
                    if isinstance(y, str):
                        ...
                        raise RuntimeError("Should not have got here with %s" % y)
...
class MapOverlap(object):
    ...
    def __call__(self, maps):
        ...
        self.spread_map = np.logical_and(
            ovstats > 0.0, ovstats < self.__overlap_threshold
        )
...
class Event(dict):
    ...
    def as_descrete_time(self, dt, storeoffset=False, offsetattr="offset"):
        ...
        if "duration" in out:
            ...
            out["duration"] = int(
                np.ceil((onset + out["duration"]) / dt) - out["onset"]
            )
    ...
    if limit is None:
        ...
        for i, uvs in enumerate(product(*(collection[x].unique for x in limit))):
            ...
            for l, uv in zip(limit, uvs):
                np.logical_and(uv_filter, collection[l].value == uv, out=uv_filter)
