...
class AttributeMap(object):
    ...
    def __init__(self, map=None, mapnumeric=False, collisions_resolution=None):
        """
        Parameters
        ----------
        map : dict
    ...
    def __repr__(self):
        ...
        if self.collisions_resolution:
            args.append("collisions_resolution=%r" % (self.collisions_resolution,))
    ...
    def to_numeric(self, attr):
        ...
        if self._nmap is None:
            ...
            if (ua != mkeys).any():
                # maps to not match
                raise KeyError(
                    "Existing attribute map not suitable for "
                    "to be mapped attribute (i.e. unknown values. "
                    "Attribute has '%s', but map has '%s'." % (str(ua), str(mkeys))
                )

        num = np.empty(attr.shape, dtype=int)
        for k, v in list(self._nmap.items()):
            num[attr == k] = v
        return num
    ...
    def _get_lmap(self):
        ...
        if cr == "lucky":
            lmap = dict([(v, k) for k, v in list(self._nmap.items())])
    ...
    def to_literal(self, attr, recurse=False):
        ...
        if self._lmap is None and self._nmap is None:
            raise RuntimeError(
                "AttributeMap has no mapping information. " "Ever called to_numeric()?"
            )
