...
class McFlirtParams(ColumnData):
    ...
    def __init__(self, source):
        ...
        """
        ColumnData.__init__(
            self, source, header=McFlirtParams.header_def, sep=None, dtype=float
        )

    def tofile(self, filename):
        """Write motion parameters to file."""
        ColumnData.tofile(
            self, filename, header=False, header_order=McFlirtParams.header_def, sep=" "
        )
    ...
    def toarray(self):
        ...
        return np.array([self[i] for i in McFlirtParams.header_def], dtype="float").T
...
class FslGLMDesign(object):
    ...
    def _load_file(self, fname):
        ...
        if not self.mat.shape == (ntimepoints, nwaves):
            raise IOError(
                "Design matrix file '%s' did not contain expected "
