...
def mean_tpr(predicted, target):
    ...
    if len(predicted) != len(target) or not len(target):
        raise ValueError("Both predicted and target should be of the same non-0 length")
    ...
    for t in targets:
        ...
        assert len(TPmask) <= len(
            Pmask
        ), "do not see how this could have been violated unless indexing is screwedup..."
        P = np.sum(Pmask)
        TP = np.sum(TPmask)
        if not P:
            ...
            raise ValueError(
                ...
                "verify if your shuffling etc is correct)" % str(t)
            )
...
def auc_error(predicted, target):
    ...
    tp = np.concatenate(([0], np.cumsum(t) / t.sum(dtype=float), [1]))

    # calculate the false positives
    fp = np.concatenate(([0], np.cumsum(~t) / (~t).sum(dtype=float), [1]))
...
if externals.exists("scipy"):
    ...
    def correlation(predicted, target):
        ...
        """
        l = len(predicted)
        r = np.corrcoef(np.reshape(predicted, l), np.reshape(target, l))[0, 1]
        if np.isnan(r):
            r = 0.0
        return r
