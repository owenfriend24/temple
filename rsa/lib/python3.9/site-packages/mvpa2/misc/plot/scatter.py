...
def plot_scatter(
    ...
    if len(dataXd) != 2:
        raise ValueError(
            "First axis of dataXd can only have two dimensions, "
    ...
    if dataXd.ndim < 5:
        ...
        raise ValueError(
            "Do not know how to handle data with %d dimensions" % (dataXd.ndim - 1)
        )
    ...
    verbose(
        1,
        "total: %d union: %d%s intersection: %d x_only: %d y_only: %d%s"
        % (
            len(nzsum),
            np.sum(union),
    ...
    if not (
        bool(ax_scatter) or bool(ax_hist_x) or bool(ax_hist_y)
    ):  # no custom axes specified
        # our default setup
    ...
    if bp_location is not None:
        ...
        if ax_bp_x_parent:
            ...
            ax_bp_x = pl_axes(
                [
                    hist_x_pos.x0,
                    hist_x_pos.y0 + hist_x_pos.height * 0.9,
                    hist_x_pos.width,
                    hist_x_pos.height * 0.1,
                ],
                facecolor="y",
            )
        ...
        if ax_bp_y_parent:
            ...
            ax_bp_y = pl_axes(
                [
                    hist_y_pos.x0 + hist_y_pos.width * 0.9,
                    hist_y_pos.y0,
                    hist_y_pos.width * 0.1,
                    hist_y_pos.height,
                ],
                facecolor="y",
            )
    ...
    if cm and len(dataXd.shape) > dimcolor + 1:
        ...
        edgecolors = cm(
            ((cm.N - 1) * ndindices_nz[:, dimcolor] / dimcolor_len).astype(int)
        )
        if mask is not None:
        ...
        if len(x_plot):
            ax_scatter.scatter(x_plot, y_plot, edgecolors=edgecolors_plot, **sc_kwargs)
        ...
        if np.size(zdim_entries):
            zdim_counts, _ = np.histogram(
                zdim_entries, bins=np.arange(0, np.max(zdim_entries) + 1)
            )
            ...
            if hint_opacity:
                ...
                # create new axes
                axslice = pl_axes(
                    [left, bottom + height * 0.72, width / 4.0, height / 5.0],
                    facecolor="y",
                )
                axslice.axis("off")
                sslice = np.zeros(
                ...
                sslice[
                    dataXd_mint[0, ..., zdim_max] == 0
                ] = -1  # reset those not in the picture to be "under" range
    ...
    if uniq:
        if np.sum(xnoty):
            ax_scatter.scatter(
                fill_nonfinites(data[0, np.where(xnoty)[0]]),
        ...
        if np.sum(ynotx):
            ax_scatter.scatter(
                fill_nonfinites(data[0, np.where(ynotx)[0]]),
    ...
    if isinstance(limits, str):
        ...
        if limits == "auto":
            ...
            limits = {True: "same", False: "per-axis"}[
    ...
    if thresholds is not None:
        ...
        if len(thresholds) > 1:
            ax_scatter.hlines(
                ...
                ax_scatter.get_ylim()[1] * 0.9,
                **stylekwargs
            )

    if same_range:
        # now determine nice limits by hand:
        binwidthx = binwidthy = binwidth = np.max(datainter) / 51.0  # 0.25
        ...
        binwidthx = (maxx - minx) / 51.0
        binwidthy = (maxy - miny) / 51.0
    ...
    x_hist, y_hist = (
        (x, y) if (mask is None or not np.sum(mask)) else (x_masked, y_masked)
    )
    ...
    if np.any(binsy) and ax_hist_y is not None:
        ...
        histy = ax_hist_y.hist(
            y_hist, bins=binsy, orientation="horizontal", facecolor="g"
        )
    ...
    if style == "upper_triang":
        ...
        for d1 in range(0, n):
            ...
            for d2 in range(0, n):
                ...
                # only upper triangle
                hint_opacity = (
                    kwargs.pop("hint_opacity", 0.9) if (d1 == 0 and d2 == 1) else 0
                )
                plot_scatter(
                    [d[d2], d[d1]],
                    ax_scatter=axes[d1, d2],
                    ax_hist_x=axes[d2, d2]
                    if (d2 == d1 + 1 or (d1 == 1 and d2 == 0))
                    else None,
                    ax_hist_y=None,
                    bp_location="hist",
    ...
    if style == "pair1":
        for i in range(1, len(datas)):
            ...
            if not "labels" in kwargs:
                kwargs["labels"] = unique_path_parts(
                    files[0] + labels_add[0], files[i] + labels_add[i]
                )
            figs.append(
                plot_scatter(
                    data4d,
        ...
        if not "labels" in kwargs:
            kwargs["labels"] = unique_path_parts(
                *[f + l for f, l in zip(files, labels_add)]
            )
