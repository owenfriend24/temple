...
def plot_err_line(
    ...
    **kwargs
    ...
    return plot_err_line_missing(
        ...
        linestyle=linestyle,
        fmt=fmt,
        perc_sigchg=perc_sigchg,
        baseline=baseline,
        **kwargs
    )


def plot_err_line_missing(
    data,
    x=None,
    errtype="ste",
    curves=None,
    ...
    **kwargs
    ...
    if x is None:
        ...
        if not len(md) == len(x):
            raise ValueError(
                "The length of `x` (%i) has to match the 2nd "
...
def plot_feature_hist(
    dataset, xlim=None, noticks=True, targets_attr="targets", chunks_attr=None, **kwargs
):
    """This function is deprecated and will be removed. Replacement mvpa2.viz.hist()"""
    import warnings

    warnings.warn(
        "plot_feature_hist() is deprecated and will be removed", DeprecationWarning
    )
    ...
    return hist(
        ...
        xlim=xlim,
        noticks=noticks,
        ygroup_attr=targets_attr,
        xgroup_attr=chunks_attr,
        **kwargs
    )
...
def plot_samples_distance(dataset, sortbyattr=None):
    ...
    if sortbyattr is not None:
        ...
        for attr in dataset.sa[sortbyattr].unique:
            slicer += get_samples_by_attr(dataset, sortbyattr, attr).tolist()
...
def plot_decision_boundary_2d(
    ...
    if False:
        ...
        dataset.samples += (
            dataset.sa.chunks[:, None] * 0.1
        )  # slight shifts for chunks ;)
    ...
    for l in utargets:
        ...
        targets_colors[l] = c = cmap((l - vmin) / float(vmax - vmin))
    ...
    (x, y) = np.mgrid[
        xmin : xmax : np.complex(0, maps_res), ymin : ymax : np.complex(0, maps_res)
    ]
    ...
    if maps == "estimates":
        ...
        if len(trained_targets) == 2:
            ...
            vals = (trained_targets[:-1] + trained_targets[1:]) / 2.0
...
def inverse_cmap(cmap_name):
    ...
    new_data = dict(
        [
            (k, [(vi[0], v[-(i + 1)][1], v[-(i + 1)][2]) for i, vi in enumerate(v)])
            for k, v in list(cmap_data.items())
        ]
    )
    return mpl.colors.LinearSegmentedColormap(
        "%s_rev" % cmap_name, new_data, _cm.LUTSIZE
    )
...
def timeseries_boxplot(
    ...
    **kwargs
    ...
    if not (min is None or max is None):
        pl.fill_between(x, max, min, color="0.8", alpha=0.5, lw=0, zorder=2, **kwargs)
    if not (p25 is None or p75 is None):
        pl.fill_between(x, p75, p25, color="0.3", alpha=0.5, lw=0, zorder=3, **kwargs)
    if not (std is None or n is None):
        pl.fill_between(
            x, mean - err, mean + err, color="0.1", alpha=0.5, lw=0, zorder=4, **kwargs
        )
    pl.plot(x, median, color="0.0", zorder=5, **kwargs)
    if segment_sizes is not None:
        for i, run in enumerate(segment_sizes[:-1]):
            pl.axvline(
                np.sum(segment_sizes[: i + 1]), color="0.2", linestyle="--", **kwargs
            )
