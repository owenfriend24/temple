...
if externals.exists("nibabel"):
    ...
    class SpatialImage(object):
        ...
        def __init__(self, filename):
            raise ValueError(
                "plot_lightbox was provided a filename %s.  "
...
def plot_lightbox(
    ...
    def handle_arg(arg):
        ...
        if len(argshape) == 4:
            if argshape[-1] > 1:
                warning(
                    "For now plot_lightbox can handle only 3d, 4d data was provided."
    ...
    if isinstance(vlim_type, str):
        if vlim_type == "symneg_z":
            ...
            raise ValueError(
                "Unknown specification of vlim=%s" % vlim + " Known is: symneg"
            )
    ...
    class Plotter(object):
        ...
        def do_plot(self):
            ...
            if np.isscalar(vlim):
                vlim = (vlim, None)
            if vlim[0] is None:
                vlim = (np.min(func), vlim[1])
            if vlim[1] is None:
                vlim = (vlim[0], np.max(func))
            if __debug__ and "PLLB" in debug.active:
                debug(
                    "PLLB",
                    "Maximum %g at %s, vlim is %s"
            ...
            func_masked = func[func_mask]
            if vlim[0] < np.min(func_masked) != np.max(func_masked):
                vlim = list(vlim)
            ...
            for v, vl in ((add_hist, "hist"), (add_info, "info")):
                if v and not isinstance(v, bool):
            ...
            if self.fig is None:
                self.fig = pl.figure(
                    facecolor="white", figsize=(4 * ncolumns, 4 * nrows)
                )
            fig = self.fig
            fig.clf()
            #
            # how to threshold images
            thresholder = lambda x: np.logical_and(x >= vlim[0], x <= vlim[1]) ^ invert
            ...
            for islice in slices[::-1]:  # range(nslices)[::-1]:
                ax = fig.add_subplot(
                    nrows, ncolumns, locs.index(islice) + 1, frame_on=False
                )
                ...
                if bg is not None:
                    ...
                    slice_bg_ = np.ma.masked_array(
                        slice_bg, mask=np.logical_not(bg_mask[:, :, islice])
                    )
                    # slice_bg<=0)
                    ...
                    if __debug__:
                        debug(
                            "PLLB",
                            "Plotting %i background elements in slice %i"
                ...
                slice_sl_ = np.ma.masked_array(
                    slice_sl,
                    mask=np.logical_or(
                        out_thresh, np.logical_not(func_mask[:, :, islice])
                    ),
                )
                ...
                if __debug__:
                    debug(
                        "PLLB",
                        "Plotting %i foreground elements in slice %i"
                ...
                im = ax.imshow(np.ones(slice_sl_.shape).T, cmap=bg_cmap, **kwargs)
                im.set_clim((0, 1))
            ...
            if add_info and len(func_thr):
                self.info_ax = ax = fig.add_subplot(
                    nrows, ncolumns, locs.index("info") + 1, frame_on=False
                )
 ...
 Thresholded: %(sthr)s:
  ...
  """
            ...
            # Add histogram
            if add_hist:
                self.hist_ax = fig.add_subplot(
                    nrows, ncolumns, locs.index("hist") + 1, frame_on=True
                )

                minv, maxv = np.min(func_masked), np.max(func_masked)
                if minv < 0 < maxv:  # then make it centered on 0
                    maxx = max(-minv, maxv)
                    range_ = (-maxx, maxx)
                else:
                    range_ = (minv, maxv)
                ...
                halign = (
                    externals.versions["matplotlib"] >= "1.0.0" and "mid" or "center"
                )
                H2 = pl.hist(
                    func_masked, bins=H[1], align=halign, facecolor="#FFFFFF", hold=True
                )
