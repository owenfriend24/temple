...
class VolSurf(object):
    ...
    def __reduce__(self):
        return (
            self.__class__,
            (self._volgeom, self._white, self._pial, self._intermediate),
        )
    ...
    def surf_unproject_weights_nodewise(self, weights):
        ...
        if len([s for s in weights.shape if s > 1]) != 1:
            raise ValueError(
                "Weights should be a vector, but found " "shape %s" % (weights.shape,)
            )
...
class VolSurfMapping(VolSurf):
    ...
    def __init__(
        ...
        'pial' and 'white' should have the same topology.
        """
        super(VolSurfMapping, self).__init__(
            vg=vg, white=white, pial=pial, intermediate=intermediate
        )
        self.nsteps = nsteps
...
class VolSurfMaximalMapping(VolSurfMapping):
    def __init__(
        ...
        super(VolSurfMaximalMapping, self).__init__(
            vg=vg,
            white=white,
            pial=pial,
...
class VolSurfMinimalMapping(VolSurfMapping):
    def __init__(
        ...
        super(VolSurfMinimalMapping, self).__init__(
            vg=vg,
            white=white,
            pial=pial,
            intermediate=intermediate,
    ...
    def get_node2voxels_mapping(self):
        ...
        if __debug__ and "SVS" in debug.active:
            nnodes = len(n2vs_max)
            nvoxels_max = sum(map(len, iter(list(v2ns_max.values()))))
            nvoxels_max_per_node = float(nvoxels_max) / nnodes
            debug(
                "SVS",
                "Maximal node-to-voxel mapping: %d nodes, "
        ...
        # initialize mapping
        n2vs_min = dict(
            (n, None if vs is None else dict()) for n, vs in list(n2vs_max.items())
        )
        v2n_min = dict()

        # helper function to compute distance to intermediate surface
        dist_func = lambda __p: abs(__p[1] - 0.5)

        for v, ns in list(v2ns_max.items()):
        ...
        if __debug__ and "SVS" in debug.active:
            ...
            debug(
                "SVS",
                "Pruned %d/%d voxels (%.1f%%), %.2f voxels/node"
                % (
                    nvoxels_delta,
                    nvoxels_max,
...
class VolSurfMinimalLowresMapping(VolSurfMinimalMapping):
    def __init__(
        ...
        super(VolSurfMinimalLowresMapping, self).__init__(
            vg=vg,
            white=white,
            pial=pial,
            intermediate=intermediate,
    ...
    def get_node2voxels_mapping(self):
        n2v = super(VolSurfMinimalLowresMapping, self).get_node2voxels_mapping()

        # set low and high res intermediate surfaces
        lowres = surf.from_any(self._intermediate)
        highres = (self.pial_surface * 0.5) + (self.white_surface * 0.5)
        ...
        if __debug__ and "SVS" in debug.active:
            ...
            debug(
                "SVS",
                "Reassigned %d nodes by moving %.2f +/- %.2f to low-res",
                (n, mu, s),
            )
...
class VolumeBasedSurface(surf.Surface):
    ...
    def __str__(self):
        return "%s(volgeom=%s)" % (self.__class__.__name__, self._vg)
    ...
    def circlearound_n2d(self, src, radius, metric="euclidean"):
        ...
        if shortmetric == "e":
            ...
            src_coord = np.asanyarray(src_coord, dtype=float)
            ...
            msk_ijk = np.zeros(self._vg.shape[:3], int)
