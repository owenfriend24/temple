...
class CorrCoef(FeaturewiseMeasure):
    ...
    def __init__(self, pvalue=False, attr="targets", corr_backend=None, **kwargs):
        """Initialize
    ...
    def _call(self, dataset):
        ...
        if np.issubdtype(attrdata.dtype, "c") or np.issubdtype(attrdata.dtype, "U"):
            raise ValueError(
                "Correlation coefficent measure is not meaningful "
...
def pearson_correlation(x, y=None):
    ...
    # normalize
    n = 1.0 / (x.shape[0] - 1)  # normalize

    # standard deviation
    xs = (n * np.sum(xd * xd, axis=0)) ** -0.5
    ys = (n * np.sum(yd * yd, axis=0)) ** -0.5
