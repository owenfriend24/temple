...
class Measure(Learner):
    ...
    def __init__(self, null_dist=None, **kwargs):
        ...
        if __debug__:
            debug(
                "SA",
                "Assigning null_dist %s whenever original given was %s"
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(Measure, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["null_dist"])
        )
    ...
    def _precall(self, ds):
        ...
        if self.__null_dist is not None:
            if __debug__:
                debug(
                    "STAT", "Estimating NULL distribution using %s" % self.__null_dist
                )
    ...
    def _postcall(self, dataset, result):
        ...
        if self.__null_dist is None:
            ...
            if self.ca.is_enabled("null_t"):
                ...
                null_prob, null_right_tail = self.__null_dist.p(
                    result, return_tails=True
                )
...
class RepeatedMeasure(Measure):
    ...
    repetition_results = ConditionalAttribute(
        enabled=False, doc="""Store individual result datasets for each repetition"""
    )
    stats = ConditionalAttribute(
        enabled=False,
        doc="""Summary statistics about the node performance across all repetitions
       """,
    )
    datasets = ConditionalAttribute(
        enabled=False,
        doc="""Store generated datasets for all repetitions. Can be memory expensive
       """,
    )
    ...
    def __init__(
        self, node, generator=None, callback=None, concat_as="samples", **kwargs
    ):
        """
        Parameters
        ----------
    ...
    def __repr__(self, prefixes=None, exclude=None):
        ...
        return super(RepeatedMeasure, self).__repr__(
            prefixes=prefixes
            + _repr_attrs(
                self, [x for x in ["node", "generator", "callback"] if not x in exclude]
            )
    ...
    def _call(self, ds):
        ...
        if self.ca.is_enabled("stats") and (
            not "stats" in node.ca or not node.ca.is_enabled("stats")
        ):
            warning(
                "'stats' conditional attribute was enabled, but "
...
class CrossValidation(RepeatedMeasure):
    ...
    training_stats = ConditionalAttribute(
        enabled=False,
        doc="""Summary statistics about the training status of the learner
    ...
    def __init__(
        ...
        **kwargs
        ...
        if splitter is None:
            ...
            splitter = Splitter(
                generator.get_space() if generator else "partitions", attr_values=(1, 2)
            )
        ...
        RepeatedMeasure.__init__(self, tm, generator=generator, space=space, **kwargs)
    ...
    def _repetition_postcall(self, ds, node, result):
        ...
        if ca.is_enabled("training_stats"):
            ...
            if isinstance(training_stats, dict):
                ...
                for k, v in list(training_stats.items()):
                    if (
                        not len(ca["training_stats"].value)
                        or k not in ca["training_stats"].value
                    ):
...
class TransferMeasure(Measure):
    ...
    stats = ConditionalAttribute(
        enabled=False,
        doc="""Optional summary statistics about the transfer performance""",
    )
    training_stats = ConditionalAttribute(
        enabled=False,
        doc="""Summary statistics about the training status of the learner""",
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(TransferMeasure, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["measure", "splitter"])
        )
    ...
    def _call(self, ds):
        ...
        if space:
            ...
            train_chunks = ",".join(
                [str(i) for i in dstrain.get_attr(splitter.get_space())[0].unique]
            )
        ...
        if space:
            ...
            test_chunks = ",".join(
                [str(i) for i in dstest.get_attr(splitter.get_space())[0].unique]
            )
        res = measure(dstest)
        if space:
        ...
        if ca.is_enabled("stats"):
            if not hasattr(measure, "__summary_class__"):
                warning(
                    "%s has no __summary_class__ attribute -- "
                ...
                ca.stats = stats
        if ca.is_enabled("training_stats"):
            if "training_stats" in measure.ca and measure.ca.is_enabled(
                "training_stats"
            ):
                ca.training_stats = measure.ca.training_stats
            else:
                warning(
                    "'training_stats' conditional attribute was enabled, "
...
class FeaturewiseMeasure(Measure):
    ...
    def _postcall(self, dataset, result):
        ...
        if not (len(result.shape) == 1 or isinstance(result, AttrDataset)):
            raise RuntimeError(
                "FeaturewiseMeasures have to return "
...
class StaticMeasure(Measure):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(StaticMeasure, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["measure", "bias"])
        )
...
class Sensitivity(FeaturewiseMeasure):
    ...
    clf = property(fget=lambda self: self.__clf, fset=_set_classifier)
...
class CombinedFeaturewiseMeasure(FeaturewiseMeasure):
    ...
    sensitivities = ConditionalAttribute(
        enabled=False, doc="Sensitivities produced by each analyzer"
    )
    ...
    def __init__(
        self,
        analyzers=None,  # XXX should become actually 'measures'
    ...
    def _call(self, dataset):
        ...
        for ind, analyzer in enumerate(self.__analyzers):
            if __debug__:
                debug("SA", "Computing sensitivity for SA#%d:%s" % (ind, analyzer))
        ...
        if isinstance(sensitivities[0], AttrDataset):
            ...
            sensitivities = Dataset(
                sensitivities, sa={sa_attr: np.arange(len(sensitivities))}
            )
    ...
    analyzers = property(
        fget=lambda x: x.__analyzers, fset=_set_analyzers, doc="Used analyzers"
    )
...
class BoostedClassifierSensitivityAnalyzer(Sensitivity):
    ...
    def __init__(
        self, clf, analyzer=None, combined_analyzer=None, sa_attr="lrn_index", **kwargs
    ):
        ...
        if combined_analyzer is None:
            ...
            combined_analyzer = CombinedFeaturewiseMeasure(sa_attr=sa_attr, **kwargs)
    ...
    def _call(self, dataset):
        ...
        for clf in self.clf.clfs:
            if self.__analyzer is None:
                ...
                if analyzer is None:
                    raise ValueError(
                        "Wasn't able to figure basic analyzer for clf %r" % (clf,)
                    )
                if __debug__:
                    debug("SA", "Selected analyzer %r for clf %r" % (analyzer, clf))
            else:
...
class ProxyClassifierSensitivityAnalyzer(Sensitivity):
    ...
    def _call(self, dataset):
        ...
        if analyzer is None:
            ...
            if analyzer is None:
                raise ValueError(
                    "Wasn't able to figure basic analyzer for clf %s" % repr(clfclf)
                )
            if __debug__:
                debug("SA", "Selected analyzer %s for clf %s" % (analyzer, clfclf))
...
class BinaryClassifierSensitivityAnalyzer(ProxyClassifierSensitivityAnalyzer):
    ...
    def _call(self, dataset):
        ...
        if targets_attr in sens.sa:
            ...
            am = AttributeMap(
                dict(
                    [(l, -1) for l in clf.neglabels] + [(l, +1) for l in clf.poslabels]
                )
            )
            ...
            # asobjarray(
            sens.sa[targets_attr] = am.to_literal(
                sens.sa[targets_attr].value, recurse=True
            )
        return sens
...
class MappedClassifierSensitivityAnalyzer(ProxyClassifierSensitivityAnalyzer):
    ...
    def _call(self, dataset):
        ...
        if __debug__:
            debug("SA", "Mapped incoming dataset %s to %s" % (dataset_mapped, dataset))
        sens = super(MappedClassifierSensitivityAnalyzer, self)._call(dataset_mapped)
