...
if externals.exists("scipy"):
    ...
    _coo_shape_argument = {True: "shape", False: "dims"}[
        externals.versions["scipy"] >= "0.7.0"
    ]
...
def _inds_list_to_coo(inds, shape=None):
    ...
    inds_i = r_helper(*[[i] * len(ind) for i, ind in enumerate(inds)])
...
def _inds_array_to_coo(inds, shape=None):
    ...
    cps_inds = inds.ravel()
    row_inds = np.repeat(np.arange(n_sums)[None, :], n_cols_per_sum, axis=0).T.ravel()
...
class SimpleStatBaseSearchlight(BaseSearchlight):
        ...
        **kwargs
        ...
        if indexsum is None:
            ...
            if indexsum == "sparse" and not externals.exists("scipy"):
                warning(
                    "Scipy.sparse isn't available so taking 'fancy' as "
                    "'indexsum' method."
                )
                indexsum = "fancy"
        ...
        if not self.nproc in (None, 1):
            raise NotImplementedError(
                "For now only nproc=1 (or None for "
    ...
    def _compute_pb_stats(self, labels_numeric, X, shape):
        #
    ...
    def _compute_pl_stats(self, sis, pl):
        ...
        *args:
        ...
        for il, l in enumerate(self._ulabels_numeric):
            ...
            pl.nsamples[il] = N_float = float(np.sum(pb.nsamples[bis_il]))
            nsamples += N_float
            if N_float == 0.0:
                pl.variances[il] = pl.sums[il] = pl.means[il] = pl.sums2[il] = 0.0
            else:
    ...
    def _sl_call(self, dataset, roi_ids, nproc):
        ...
        splitter = (
            Splitter(attr=generator.get_space(), attr_values=[1, 2])
            if self._splitter is None
            else self._splitter
        )

        partitions = (
            list(generator.generate(dataset_indicies))
            if generator
        ...
        for ipartition, (split1, split2) in enumerate(splits):
            ...
            if not (
                len(np.unique(split1.samples[:, 0])) == len(split1)
                and len(np.unique(split2.samples[:, 0])) == len(split2)
            ):
                raise RuntimeError(
                    "%s needs a partitioner which does not reuse "
                    "the same the same samples more than once" % self.__class__
                )
        ...
        self.__sample2block = sample2block = np.array(
            [description2block[d] for d in descriptions]
        )

        # 3. Compute statistics per each block
        #
        if __debug__:
            debug("SLC", "Phase 3. Computing statistics for %i blocks" % (nblocks,))
        ...
        if not self.reuse_neighbors or self.__roi_fids is None:
            if __debug__:
                debug(
                    "SLC",
                    "Phase 4. Deducing neighbors information for %i ROIs" % (nrois,),
                )
            ...
            if __debug__:
                debug(
                    "SLC",
                    "Phase 4. Reusing neighbors information for %i ROIs" % (nrois,),
                )
        ...
        if indexsum == "sparse":
            if not self.reuse_neighbors or self.__roi_fids is None:
                ...
                roi_fids = inds_to_coo(roi_fids, shape=(dataset.nfeatures, nroi_fids))
            indexsum_fx = lastdim_columnsums_spmatrix
        elif indexsum == "fancy":
            indexsum_fx = lastdim_columnsums_fancy_indexing
        else:
            raise ValueError("Do not know how to deal with indexsum=%s" % indexsum)
        # Store roi_fids
        ...
        for isplit, split in enumerate(splits):
            ...
            if errorfx is mean_mismatch_error:
                results[isplit, :] = (predictions != targets[:, None]).sum(
                    axis=0
                ) / float(len(targets))
                all_cvfolds += [isplit]
            elif errorfx:
