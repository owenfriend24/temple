...
class GNBSearchlight(SimpleStatBaseSearchlight):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(GNBSearchlight, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["gnb"])
        )
    ...
    def _sl_call_on_a_split(
        ...
        training_nsamples, non0labels = self._compute_pl_stats(training_sis, pl)

        nlabels = len(pl.nsamples)

        if params.common_variance:
            pl.variances[:] = (
                np.sum(pl.sums2 - pl.sums * pl.means, axis=0) / training_nsamples
            )
        else:
            pl.variances[non0labels] = (pl.sums2 - pl.sums * pl.means)[
                non0labels
            ] / pl.nsamples[non0labels]

        # assign priors
        priors = gnb._get_priors(nlabels, training_nsamples, pl.nsamples)
        ...
        scaled_distances = -0.5 * (
            ((data - pl.means[:, np.newaxis, ...]) ** 2)
...
def sphere_gnbsearchlight(
    gnb, generator, radius=1, center_ids=None, space="voxel_indices", *args, **kwargs
):
    ...
    **kwargs
    ...
    return GNBSearchlight(gnb, generator, qe, roi_ids=center_ids, *args, **kwargs)
