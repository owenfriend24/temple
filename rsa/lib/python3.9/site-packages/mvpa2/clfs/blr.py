...
class BLR(Classifier):
    ...
    def _train(self, data):
        ...
        elif self.sigma_p.shape[1] != (
            data.samples.shape[1] + 1
        ...
        if type(self.sigma_p) != type(self.samples_train):  # if sigma_p is a number...
            self.sigma_p = (
                np.eye(self.samples_train.shape[1]) * self.sigma_p
            )  # convert in matrix
            pass
        ...
        self.w = (
            1.0
            / (self.sigma_noise ** 2)
            * np.dot(self.A_inv, np.dot(self.samples_train.T, train_labels))
        )
        pass
