...
class Warehouse(object):
    ...
    def __getitem__(self, *args):
        ...
        dargs = set([str(x).lstrip("!") for x in args]).difference(self._known_tags)
    ...
    def __iadd__(self, item):
        if is_sequence_type(item):
            ...
            if not hasattr(item, "__tags__"):
                raise ValueError(
                    "Cannot register %s " % item + "which has no __tags__ defined"
                )
            if item.descr in self.__descriptions:
                raise ValueError(
                    "Cannot register %s, " % item
                    + "an item with descriptions '%s' already exists" % item.descr
                )
            if len(item.__tags__) == 0:
                raise ValueError(
                    "Cannot register %s " % item + "which has empty __tags__"
                )
...
clfswh += [
    MulticlassClassifier(SMLR(lm=0.1), descr="Pairs+maxvote multiclass on SMLR(lm=0.1)")
]
...
if externals.exists("libsvm"):
    ...
    clfswh += [
        ...
        libsvm.SVM(svm_impl="NU_SVC", descr="libsvm.LinNuSVM(nu=def)", probability=1),
    ]
    clfswh += [
        libsvm.SVM(kernel=RbfLSKernel(), descr="libsvm.RbfSVM()"),
        libsvm.SVM(
            kernel=RbfLSKernel(), svm_impl="NU_SVC", descr="libsvm.RbfNuSVM(nu=def)"
        ),
        libsvm.SVM(kernel=PolyLSKernel(), descr="libsvm.PolySVM()", probability=1),
    ...
    regrswh += [
        libsvm.SVM(svm_impl="EPSILON_SVR", descr="libsvm epsilon-SVR"),
...
if externals.exists("shogun"):
    ...
    from mvpa2.kernels.sg import RbfSGKernel
    ...
    for impl in sg.SVM._KNOWN_IMPLEMENTATIONS:
        ...
        clfswh += [
            ...
            sg.SVM(C=-10.0, descr="sg.LinSVM(C=10*def)/%s" % impl, svm_impl=impl),
            sg.SVM(C=1.0, descr="sg.LinSVM(C=1)/%s" % impl, svm_impl=impl),
        ]
        if not impl in ["svmocas"]:  # inherently linear only
            clfswh += [
                sg.SVM(
                    kernel=RbfSGKernel(), descr="sg.RbfSVM()/%s" % impl, svm_impl=impl
                ),
    ...
    for impl in ["libsvr"] + _optional_regressions:  # \
        ...
        regrswh += [
            sg.SVM(svm_impl=impl, descr="sg.LinSVMR()/%s" % impl),
...
if externals.exists("skl"):
    ...
    clfswh += SKLLearnerAdapter(
        sklLDA(), tags=["lda", "linear", "multiclass", "binary"], descr="skl.LDA()"
    )
    ...
    if externals.versions["skl"] >= "0.6.0":
        sklLars = _skl_import("linear_model", _skl_api09 and "Lars" or "LARS")
        sklLassoLars = _skl_import(
            "linear_model", _skl_api09 and "LassoLars" or "LassoLARS"
        )
        sklElasticNet = _skl_import("linear_model", "ElasticNet")
        _lars_tags = ["lars", "linear", "regression", "does_feature_selection"]

        _lars = SKLLearnerAdapter(sklLars(), tags=_lars_tags, descr="skl.Lars()")

        _lasso_lars = SKLLearnerAdapter(
            sklLassoLars(alpha=0.01), tags=_lars_tags, descr="skl.LassoLars()"
        )

        _elastic_net = SKLLearnerAdapter(
            sklElasticNet(
                alpha=0.01,
                **{"l1_ratio" if externals.versions["skl"] >= "0.13" else "rho": 0.3}
            ),
            tags=[
                "enet",
                "regression",
                "linear",  # 'has_sensitivity',
                "does_feature_selection",
            ],
    ...
    if _skl_version >= "0.10":
        ...
        _lasso_lars_ic = SKLLearnerAdapter(
            sklLassoLarsIC(), tags=_lars_tags, descr="skl.LassoLarsIC()"
        )
        regrswh += [_lasso_lars_ic]
        clfswh += [RegressionAsClassifier(_lasso_lars_ic, descr="skl.LassoLarsIC_C()")]
...
clfswh += FeatureSelectionClassifier(
    kNN(),
    SensitivityBasedFeatureSelection(
        OneWayAnova(), FixedNElementTailSelector(50, mode="select", tail="upper")
    ),
...
if externals.exists("scipy"):
    ...
    regrswh += GPR(kernel=SquaredExponentialKernel(), descr="GPR(kernel='sqexp')")
    ...
    clfswh += MulticlassClassifier(
        RegressionAsClassifier(GPR(kernel=GeneralizedLinearKernel())),
...
if len(clfswh["linear", "svm"]) > 0:
    ...
    linearSVMC = clfswh[
        "linear", "svm", cfg.get("svm", "backend", default="libsvm").lower()
    ][0]
    ...
    clfswh += FeatureSelectionClassifier(
        ...
        SplitRFE(
            _rfeclf,
            OddEvenPartitioner(),
            fselector=FractionTailSelector(0.2, mode="discard", tail="lower"),
        ),
        descr="LinSVM with nested-CV RFE",
    )

    _clfs_splitrfe_svm_anova = FeatureSelectionClassifier(
        linearSVMC.clone(),
        SplitRFE(
            linearSVMC.clone(),
            OddEvenPartitioner(),
