...
class BoostedClassifier(Classifier):
    ...
    raw_predictions = ConditionalAttribute(
        enabled=False, doc="Predictions obtained from each classifier"
    )

    raw_estimates = ConditionalAttribute(
        enabled=False, doc="Estimates obtained from each classifier"
    )

    def __init__(self, clfs=None, propagate_ca=True, **kwargs):
    ...
    def _predict(self, dataset):
        ...
        if self.ca.is_enabled("estimates"):
            if np.array([x.ca.is_enabled("estimates") for x in self.__clfs]).all():
                estimates = [clf.ca.estimates for clf in self.__clfs]
                self.ca.raw_estimates = estimates
            else:
                warning(
                    "One or more classifiers in %s has no 'estimates' state" % self
                    + "enabled, thus BoostedClassifier can't have"
    ...
    def get_sensitivity_analyzer(self, **kwargs):
        ...
        return BoostedClassifierSensitivityAnalyzer(self, **kwargs)

    clfs = property(
        fget=lambda x: x.__clfs, fset=_set_classifiers, doc="Used classifiers"
    )
...
class ProxyClassifier(Classifier):
    ...
    def __str__(self, *args, **kwargs):
        return super(ProxyClassifier, self).__str__(str(self.__clf), *args, **kwargs)
    ...
    def _set_retrainable(self, value, force=False):
        ...
        if value and not (self.ca["retrained"] is self.clf.ca["retrained"]):
            if __debug__:
                debug(
                    "CLFPRX",
                    "Rebinding conditional attributes from slave clf %s",
                    (self.clf,),
                )
    ...
    def get_sensitivity_analyzer(self, slave_kwargs, **kwargs):
        ...
        **kwargs
        ...
        return self.__sa_class__(
            self, analyzer=self.__clf.get_sensitivity_analyzer(**slave_kwargs), **kwargs
        )
...
class MaximalVote(PredictionsCombiner):
    ...
    predictions = ConditionalAttribute(enabled=True, doc="Voted predictions")
    ...
    def __call__(self, clfs, dataset):
        ...
        for clf in clfs:
            ...
            for i in range(len(predictions)):
                ...
                if isinstance(prediction, str) or not is_sequence_type(prediction):
        ...
        for i in range(len(all_label_counts)):
            ...
            maxv = -1
            for k, v in list(label_counts.items()):
                if v > maxv:
                    maxk = [k]
                    maxv = v
                elif v == maxv:
            ...
            if len(maxk) > 1:
                warning(
                    "We got multiple labels %s which have the " % maxk
...
class MeanPrediction(PredictionsCombiner):
    ...
    predictions = ConditionalAttribute(enabled=True, doc="Mean predictions")

    estimates = ConditionalAttribute(
        enabled=True, doc="Predictions from all classifiers are stored"
    )
...
class ClassifierCombiner(PredictionsCombiner):
    ...
    predictions = ConditionalAttribute(enabled=True, doc="Trained predictions")
    ...
    def combiner(self):
        ...
        if self._combiner is None:
            if isinstance(self.__combiner, str) and self.__combiner == "auto":
                self._combiner = (MaximalVote, MeanPrediction)[
                    int(self.clfs[0].__is_regression__)
                ]()
            else:
    ...
    def _predict(self, dataset):
        ...
        if ca.is_enabled("estimates") and cca is not None:
            if cca.is_active("estimates"):
                ...
                if __debug__:
                    warning(
                        "Boosted classifier %s has 'estimates' state enabled,"
                        " but combiner doesn't have 'estimates' active, thus "
                        " .estimates cannot be provided directly, access .clfs" % self
                    )
        return predictions
...
class TreeClassifier(ProxyClassifier):
    ...
    def __init__(self, clf, groups, **kwargs):
        ...
        self.clfs = dict([(gk, c) for gk, (ls, c) in list(groups.items())])
    ...
    def __str__(self, *args, **kwargs):
        return super(TreeClassifier, self).__str__(
            ", ".join(["%s: %s" % i for i in list(self.clfs.items())]), *args, **kwargs
        )
    ...
    def _train(self, dataset):
        ...
        for gi, gk in enumerate(index2group):
            ...
            if len(known_already):
                raise ValueError(
                    ...
                    "Got labels %s already among known in %s. " % (known_already, known)
                )
        ...
        if known.intersection(dsul) != dsul:
            raise ValueError(
                ...
                "Known are %s" % (dataset, dsul.difference(known), known)
            )
        ...
        ds_group.sa[targets_sa_name].value = [label2index[l] for l in targets_sa.value]

        # train primary classifier
        if __debug__:
            debug(
                "CLFTREE",
                "Training primary %s on %s with targets %s",
            ...
            if clf is None:  # Trailing node
                if len(group_labels) != 1:
                    raise ValueError(
                        ...
                        "group %r of %s" % (group_labels, gk, self)
                    )
            else:
    ...
    def _predict(self, dataset):
        ...
        if __debug__:
            debug("CLFTREE", "Predictions %s", clf_predictions)
        ...
        # corresponding one
        predictions = np.zeros((len(dataset),), dtype=self.ca.trained_targets.dtype)
        for pred_group in set(clf_predictions):
            try:
                gk = index2group[pred_group]
            except IndexError:
                raise IndexError(
                    "%s: Cannot obtain value for index %r among %r"
...
class BinaryClassifier(ProxyClassifier):
    ...
    def __init__(self, clf, poslabels, neglabels, **kwargs):
        ...
        if len(overlap) > 0:
            raise ValueError(
                "Sets of positive and negative labels for "
                + "BinaryClassifier must not overlap. Got overlap " % overlap
            )
    ...
    def __repr__(self, prefixes=None):
        ...
        prefix = "poslabels=%s, neglabels=%s" % (
            repr(self.__poslabels),
            repr(self.__neglabels),
        )
    ...
    def _train(self, dataset):
        ...
        idlabels = [
            (x, +1)
            for x in get_samples_by_attr(dataset, targets_sa_name, self.__poslabels)
        ] + [
            (x, -1)
            for x in get_samples_by_attr(dataset, targets_sa_name, self.__neglabels)
        ]
        ...
        if len(idlabels) == dataset.nsamples and [x[0] for x in idlabels] == list(
            range(dataset.nsamples)
        ):
        ...
        if __debug__:
            assert set(datasetselected.sa[targets_sa_name].unique) == {-1, 1}
    ...
    def _predict(self, dataset):
        ...
        predictions = [
            {-1: self.__predictneg, +1: self.__predictpos}[x]
...
class MulticlassClassifier(CombinedClassifier):
    ...
    raw_predictions_ds = ConditionalAttribute(
        enabled=False,
    ...
    def __repr__(self, prefixes=None):
        if prefixes is None:
            prefixes = []
        prefix = "bclf_type=%s, clf=%s" % (repr(self.__bclf_type), repr(self.__clf))
    ...
    def _get_binary_pairs(self, dataset):
        ...
        if __debug__:
            debug(
                "CLFMC",
                "Created %d label pairs for original %d labels",
    ...
    def _train(self, dataset):
        ...
        for neglabels, poslabels in self._get_binary_pairs(dataset):
            biclfs.append(
                BinaryClassifier(
                    self.__clf.clone(), poslabels=poslabels, neglabels=neglabels
                )
            )
    ...
    def _predict(self, dataset):
        ...
        if ca.is_enabled("raw_predictions_ds") or self.combiner is None:
            ...
            ca.raw_predictions_ds = raw_predictions_ds = Dataset(
                np.array(raw_predictions).T, fa={self.space: asobjarray(pairs)}
            )
        if self.combiner is None:
...
class SplitClassifier(CombinedClassifier):
    ...
    def __init__(
        ...
        splitter=Splitter("partitions", count=2),
        **kwargs
    ):
        """Initialize the instance
    ...
    def _train(self, dataset):
        ...
        for i, pset in enumerate(self.__partitioner.generate(dataset)):
            if __debug__:
                debug("CLFSPL_", "Deepcopying %s for %s", (clf_template, self))
            ...
            if ca.is_enabled("stats"):
                ...
                self.ca.stats.add(
                    split[1].sa[targets_sa_name].value,
                    predictions,
    ...
    partitioner = property(
        fget=lambda x: x.__partitioner, doc="Partitioner used by SplitClassifier"
    )
    splitter = property(
        fget=lambda x: x.__splitter, doc="Splitter used by SplitClassifier"
    )
...
class MappedClassifier(ProxyClassifier):
    ...
    def _train(self, dataset):
        ...
        if __debug__:
            debug("CLF", "Training %s having mapped dataset into %s", (self, wdataset))
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(MappedClassifier, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["mapper"])
        )

    def __str__(self, *args, **kwargs):
        return super(MappedClassifier, self).__str__(str(self.mapper), *args, **kwargs)
...
class RegressionAsClassifier(ProxyClassifier):
    ...
    distances = ConditionalAttribute(
        enabled=False, doc="Distances obtained during prediction"
    )
    ...
    def _train(self, dataset):
        ...
        self._trained_attrmap = AttributeMap(
            map=dict([(l, i) for i, l in enumerate(ul)]), mapnumeric=True
        )
        ...
        dataset_relabeled.sa[targets_sa_name].value = self._trained_attrmap.to_numeric(
            targets_sa.value
        )
    ...
    def _predict(self, dataset):
        ...
        self.ca.estimates = regr_predictions = ProxyClassifier._predict(self, dataset)
        ...
        # Compute distances
        self.ca.distances = distances = np.array(
            [[distance_measure(s, c) for c in centers] for s in regr_predictions]
        )
        ...
        if __debug__:
            debug(
                "CLF_",
                "Converted regression distances %s " "into labels %s for %s",
                (distances, predictions, self),
            )
