...
class SummaryStatistics(object):
    ...
    def add(self, targets, predictions, estimates=None):
        ...
        if len(targets) != len(predictions):
            raise ValueError(
                "Targets[%d] and predictions[%d]" % (len(targets), len(predictions))
        ...
        if isinstance(estimates, Collectable):
            try:
                ...
                if __debug__:
                    debug(
                        "CM", "Cannot get yet uncollected estimates from %s", estimates
                    )

        if estimates is not None and len(targets) != len(estimates):
            raise ValueError(
                "Targets[%d] and estimates[%d]" % (len(targets), len(estimates))
                + " have different number of samples"
            )
    ...
    def as_string(self, short=False, header=True, summary=True, description=False):
    ...
    def __str__(self):
        ...
        return self.as_string(
            short=False, header=True, summary=True, description=description
        )
...
class ROCCurve(object):
    ...
    def _compute(self):
        ...
        if Nlabels < 2:
            warning(
                "ROC was asked to be evaluated on data with %i"
            ...
            for v in x:
                try:
                    ...
                    if isinstance(v, dict) or (  # not dict for pairs
                        (Nlabels >= 2) and len(v) != Nlabels
                    ):  # 1 per each label for multiclass
                        return False
        ...
        if 0 < Nsets_wv != len(sets):
            warning(
                "Only %d sets have estimates assigned from %d sets. "
                "ROC estimates might be incorrect." % (Nsets_wv, len(sets))
            )
...
class ConfusionMatrix(SummaryStatistics):
    ...
    _STATS_DESCRIPTION = (
        ...
        (
            "TPR",
            "true positive rate (AKA hit rate, recall, sensitivity)",
            "TPR = TP / P = TP / (TP + FN)",
        ),
        (
            "FPR",
            "false positive rate (AKA false alarm rate, fall-out)",
            "FPR = FP / N = FP / (FP + TN)",
        ),
        ...
        ("F1", "F1 score", "F1 = 2TP / (P + P') = 2TP / (2TP + FP + FN)"),
    ...
    def __call__(self, predictions, targets, estimates=None, store=False):
        ...
        if labels is None or not len(labels):
            raise RuntimeError(
                "ConfusionMatrix must have labels assigned prior" "__call__()"
            )
        # verify that we know all the labels
        labels_set = set(labels)
        if not (labels_set.issuperset(targets) and labels_set.issuperset(predictions)):
            raise ValueError(
                "Known labels %r does not include some labels "
    ...
    def matrices(self):
        ...
        return [
            self.__class__(labels=self.labels, labels_map=self.labels_map, sets=[x])
            for x in self.sets
        ]
    ...
    def _compute(self):
        ...
        if __debug__:
            if self.__matrix is not None:
                debug(
                    "LAZY",
                    "Have to recompute %s#%s" % (self.__class__.__name__, id(self)),
                )
        ...
        try:
            ...
            labels = list(
                reduce(
                    lambda x, y: x.union(set(y[0]).union(set(y[1]))),
                    self.sets,
                    set(self.__labels),
                )
            )
        ...
        if labels_map is not None:
            ...
            if not map_labels_set.issuperset(labels_set):
                warning(
                    "Provided labels_map %s is not coherent with labels "
        ...
        if labels_map is not None:
            labels_map_rev = {}
            for k, v in list(labels_map.items()):
        ...
        if linregress and Nsets > 3:
            ...
            ACC_per_set = [
                np.sum(np.diag(m)) / np.sum(m).astype(float) for m in mat_all
            ]
            (
                stats["LOE(ACC):slope"],
                stats["LOE(ACC):inter"],
            ...
            TPRs_per_set = [
                np.diag(m) / np.sum(m, axis=0).astype(float) for m in mat_all
            ]
            # Confusion ratios (both TPs or FPs)
            # we want to divide each column but sum in the column
            CM_per_set = [
                np.ravel(m / np.sum(m, axis=0).astype(float)[None, :]) for m in mat_all
            ]
    ...
    def as_string(self, short=False, header=True, summary=True, description=False):
        ...
        if labels_map_rev is not None:
            labels_rev = [",".join([str(x) for x in labels_map_rev[l]]) for l in labels]
        ...
        stats = self._stats
        if short:
            return "%(# of sets)d sets %(# of labels)d labels " " ACC:%(ACC).2f" % stats
        ...
        if header:
            ...
            # underscores
            printed.append(["@l            `------"] + underscores + stats_perpredict)
        ...
        if summary:
            ...
            for stat in stats_pertarget:
                printed.append([stat] + [_p2(stats[stat][i]) for i in range(Nlabels)])

            # compute mean stats
            # XXX refactor to expose them in stats as well, as
            #     mean(FCC)
            mean_stats = np.mean(np.array([stats[k] for k in stats_perpredict]), axis=1)
            printed.append(
                ["@lSummary \ Means:"]
                + underscores
                + [_p2(stats["mean(%s)" % x]) for x in stats_perpredict]
            )

            if "CHI^2" in self.stats:
                chi2t = stats["CHI^2"]
                printed.append(["CHI^2"] + [_p2(chi2t[0])] + ["@wp=%.2g" % chi2t[1]])
            ...
            if "LOE(ACC):inter" in stats:
                ...
                printed[-1] += [
                    "@w ACC(i) = %(LOE(ACC):inter).2g%(LOE(ACC):slope)+.2g*i" % stats
                    + " p="
                    + _p2(stats["LOE(ACC):p"])
                    + " r="
                    + _p2(stats["LOE(ACC):r"])
                    + " r^2="
                    + _p2(stats["LOE(ACC):r"] ** 2)
                ]
    ...
    def plot(
        ...
        **kwargs
        ...
        if labels_order is not None:
            ...
            elif len(labels_rev) and set(labels_rev) == labels_order_filtered_set:
        ...
        reordered_indexes = [labels_indexes[i] for i in labels_plot if i is not None]
...
class ConfusionMatrixError(object):
    ...
    def __call__(self, predictions, targets):
        cm = ConfusionMatrix(
            labels=list(self.labels), targets=targets, predictions=predictions
        )
        # print cm.matrix
...
class Confusion(Node):
    ...
    def __init__(self, attr="targets", labels=None, add_confusion_obj=False, **kwargs):
        """
        Parameters
        ----------
        attr : str
    ...
    def _call(self, ds):
        if not len(ds.shape) == 2 and ds.shape[1] == 1:
            raise ValueError(
                "Confusion cannot deal with multi-dimensional "
        ...
        out = Dataset(cm.matrix, sa={sa_attr: cm.labels}, fa={fa_attr: cm.labels})
    ...
    def _call(self, ds):
        ...
        if hypotheses is None:
            ...
            if self._labels_attr in ds.sa:
                ...
                # underlying API
                recode = dict(
                    [(e, i) for i, e in enumerate(ds.sa[self._labels_attr].value)]
                )
                partitions = [
                    [[recode[label] for label in class_] for class_ in hyp]
        ...
        out = Dataset(
            out,
            sa={self.get_space(): hypotheses, "prior": prior_Hs},
            fa={"stat": statfa},
        )
        return out
...
class RegressionStatistics(SummaryStatistics):
    ...
    def plot(
        ...
        if splot:
            ...
            sps.append(pl.subplot(nplots, 1, nplot))
            for s in self.sets:
                pl.plot(s[0], s[1], "o", markeredgewidth=0.2, markersize=2)
    ...
    def as_string(self, short=False, header=True, summary=True, description=False):
        ...
        if short:
            if short == "very":
                ...
                return (
                    "%(# of sets)d sets CCe=%(CCe).2f p=%(CCp).2g"
                    " RMSE:%(RMSE).2f"
                    " Summary (stacked data): "
                    "CCe=%(Summary CCe).2f p=%(Summary CCp).2g" % stats
                )
            else:
                return (
                    "%(# of sets)d sets CCe=%(CCe).2f+-%(CCe_std).3f"
                    " RMSE=%(RMSE).2f+-%(RMSE_std).3f"
                    " RMSE/RMP_t=%(RMSE/RMP_t).2f+-%(RMSE/RMP_t_std).3f" % stats
                )
        ...
        printed.append(
            ["CCe", _p2(stats["Summary CCe"]), "", "p=", "%g" % stats["Summary CCp"]]
        )
...
class ClassifierError(ClassWithCollections):
    ...
    training_stats = ConditionalAttribute(
        enabled=False, doc="Proxy training_stats from underlying classifier."
    )
        ...
        if trainingdataset is not None:
            if self.__train:
                ...
                if self.ca.is_enabled("training_stats"):
                    self.__clf.ca.change_temporarily(enable_ca=["training_stats"])
                self.__clf.train(trainingdataset)
                if self.ca.is_enabled("training_stats"):
                    self.ca.training_stats = self.__clf.ca.training_stats
                    self.__clf.ca.reset_changed_temporarily()

        if (
            self.__clf.ca.is_enabled("trained_targets")
            and not self.__clf.__is_regression__
            and testdataset is not None
        ):
            newlabels = set(testdataset.sa[self.clf.get_space()].unique) - set(
                self.__clf.ca.trained_targets
            )
            if len(newlabels) > 0:
                warning(
                    "Classifier %s wasn't trained to classify labels %s"
    ...
    def __call__(self, testdataset, trainingdataset=None):
        ...
        if __debug__:
            debug("CERR", "Classifier error on %s: %.2f" % (testdataset, error))
        return error
...
class ConfusionBasedError(ClassifierError):
    ...
    def __init__(self, clf, labels=None, confusion_state="training_stats", **kwargs):
        """Initialization.
        ...
        if not clf.ca.is_enabled(confusion_state):
            if __debug__:
                debug(
                    "CERR",
                    "Forcing state %s to be enabled for %r" % (confusion_state, clf),
                )
            clf.ca.enable(confusion_state)

    __doc__ = enhanced_doc_string("ConfusionBasedError", locals(), ClassifierError)
