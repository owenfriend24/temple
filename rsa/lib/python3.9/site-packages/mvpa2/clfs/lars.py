...
# local imports
from mvpa2.clfs.base import Classifier, accepts_dataset_as_samples, FailedToPredictError
...
class LARS(Classifier):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not model_type in known_models:
            raise ValueError(
                "Unknown model %s for LARS is specified. Known" % model_type
                + "are %s" % repr(known_models)
            )
    ...
    def __repr__(self):
        ...
        return (
            "LARS(type='%s', normalize=%s, intercept=%s, trace=%s, "
    ...
    def _train(self, data):
        ...
        self.__weights = np.asanyarray(Rrx2(trained_model, "beta"))[lowest_Cp_step]
    ...
    def _predict(self, data):
        ...
        try:
            res = r.predict(
                self.__trained_model, data, mode="step", s=self.__lowest_Cp_step
            )
