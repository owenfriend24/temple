...
if not _svm_backend in _VALID_BACKENDS:
    raise ValueError(
        "Configuration option svm.backend got invalid value %s."
...
if externals.exists("libsvm"):
    ...
    if _svm_backend == "libsvm" or SVM is None:
        if __debug__ and _svm_backend != "libsvm" and SVM is None:
            debug("SVM", "SVM backend %s was not found, so using libsvm" % _svm_backend)
        SVM = libsvm.SVM
...
if SVM is None:
    ...
    _edocs += [
        ...
        (RbfCSVMC, SVM, "C-SVM classifier using a radial basis function kernel"),
    ]
