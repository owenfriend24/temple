...
class GNB(Classifier):
    ...
    __tags__ = ["gnb", "linear", "non-linear", "binary", "multiclass", "oneclass"]

    common_variance = Parameter(
        False, constraints="bool", doc="""Use the same variance across all classes."""
    )
    ...
    def _get_priors(self, nlabels, nsamples, nsamples_per_class):
        ...
        if prior == "uniform":
            ...
            priors = (1 + squash(nsamples_per_class)) / (float(nsamples) + nlabels)
        elif prior == "ratio":
    ...
    def _train(self, dataset):
        ...
        self.means = means = np.zeros((nlabels,) + s_shape)
        self.variances = variances = np.zeros((nlabels,) + s_shape)
        ...
        if __debug__ and "GNB" in debug.active:
            debug(
                "GNB",
                "training finished on data.shape=%s " % (X.shape,)
    ...
    def _predict(self, data):
        ...
        if np.any(variances0_mask):
            prob1_csf = np.logical_and(
                distances == 0, variances0_mask[:, None, :]  # class x sample x feature
            )
        else:
            prob1_csf = None
        ...
        if params.logprob:
            ...
            prob_csfs = self._norm_weight[:, np.newaxis, ...] * np.exp(scaled_distances)

            # Naive part -- just a product of probabilities across features
            ## First we need to reshape to get class x samples x features
            prob_csf = prob_csfs.reshape(prob_csfs.shape[:2] + (-1,))
        ...
        if params.normalize:
            if params.logprob:
                ...
                underflow_offset = (
                    -np.ceil(np.max(prob_cs_cp, axis=0)) if guard_overflows else 0
                )
    ...
    def get_sensitivity_analyzer(self, **kwargs):
        ...
        if params.common_variance:
            ...
            raise NotImplementedError(
                "Sensitivity calculation is only sensible for a linear GNB, which is true when "
...
class GNBWeights(Sensitivity):
    ...
    def _call(self, dataset):
        ...
        pairs = list(itertools.combinations(list(range(len(clf.ulabels))), 2))
        ...
        for idx, pair in enumerate(pairs):
            ...
            # "positive one"
            weights[idx, nonzero_vars0] = (
                means[pair[1], nonzero_vars0] - means[pair[0], nonzero_vars0]
