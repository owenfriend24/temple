...
class kNN(Classifier):
    ...
    distances = ConditionalAttribute(
        enabled=False, doc="Distances computed for each sample"
    )

    __tags__ = ["knn", "non-linear", "binary", "multiclass", "oneclass"]

    def __init__(
        self, k=2, dfx=squared_euclidean_distance, voting="weighted", **kwargs
    ):
        """
        Parameters
        ----------
    ...
    def _train(self, data):
        ...
        if __debug__:
            if str(data.samples.dtype).startswith("uint") or str(
                data.samples.dtype
            ).startswith("int"):
                warning(
                    "kNN: input data is in integers. "
        ...
        if self.__voting == "weighted":
            ...
            weights = [
                1.0 - ((labels == label).sum() / Nlabels) for label in uniquelabels
            ]
        ...
        self.__votes_init = dict(list(zip(uniquelabels, [0] * Nuniquelabels)))
    ...
    def _predict(self, data):
        ...
        if __debug__:
            ...
            if not data.shape[1] == self.__data.nfeatures:
                raise ValueError(
                    "Length of data samples (features) does "
        ...
        for inns, nns in enumerate(knns):
            ...
            if self.__voting == "majority":
                ...
                raise ValueError(
                    "kNN told to perform unknown voting '%s'." % self.__voting
                )
            ...
            votes_reversed = sorted([(v, k) for k, v in list(votes.items())], reverse=True)
            # check for ties
