...
class SVMModel:
    ...
    def __repr__(self):
        ...
        try:
            ...
            ret += " number of classes = %d (%s), " % (self.nr_class, repr(self.labels))
        except:
            pass
        return ret + ">"
    ...
    def predict_probability(self, x):
        ...
        if self.svm_type == NU_SVR or self.svm_type == EPSILON_SVR:
            raise TypeError(
                "call get_svr_probability or get_svr_pdf "
    ...
    def __del__(self):
        if __debug__ and debug:
            ...
            debug(
                "SVM_",
                "Destroying libsvm v. %s SVMModel %s",
                (
                    svmc is not None
                    and hasattr(svmc, "__version__")
                    and svmc.__version__
                    or "unknown",
                    repr(self),
                ),
            )
    ...
    def get_n_sv(self):
        ...
        return [svmc.int_getitem(self.model.nSV, i) for i in range(self.nr_class)]
    ...
    def get_sv(self):
        ...
        return svmc.svm_node_matrix2numpy_array(
            self.model.SV, self.get_total_n_sv(), self.prob.maxlen
        )
    ...
    def get_sv_coef(self):
        ...
        return svmc.doubleppcarray2numpy_array(
            self.model.sv_coef, self.nr_class - 1, self.get_total_n_sv()
        )

    ##REF: Name was automagically refactored
    def get_rho(self):
        """Return constant(s) in decision function(s) (if multi-class)"""
        return double_array_to_list(
            self.model.rho, self.nr_class * (self.nr_class - 1) // 2
        )
