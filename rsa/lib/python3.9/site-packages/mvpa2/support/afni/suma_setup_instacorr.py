...
def afni_niml_zscore_makefull(fnin, fnout, pad_to_ico_ld=None, pad_to_node=None):
    ...
    if pad_to_ico_ld or pad_to_node:
        dset_z_full = afni_niml_dset.sparse2full(
            dset_z, pad_to_ico_ld=pad_to_ico_ld, pad_to_node=pad_to_node
        )
    else:
        dset_z_full = dset_z
...
def afni_niml_zscore_makefull_wizard(cfg):
    ...
    if pad_to_node is None:
        ...
        if pad_to_ico_ld is None:
            ...
            if pad_to_ico_ld is not None:
                ...
                print((
                    "Using automatic pad_to_ico_ld=%r, pad_to_node=%r"
                    % (pad_to_ico_ld, pad_to_node)
                ))
    ...
    for fn in fns:
        ...
        if os.path.exists(fnout) and not overwrite:
            print(
                (
                    "Output file %s already exists, skipping (use '--overwrite' to override)"
                    % fnout
                )
            )
        else:
    ...
    if any(map(os.path.exists, groupfnsout)):
        if overwrite:
            ...
            print(
                (
                    "Some or all of output files (%s) already exists (use '--overwrite' to override)"
                    % (" ".join(groupfnsout))
                )
            )
    ...
    msg = (
        ...
        "- in SUMA, select a node while holding down ctrl+shift" % (pth, fullprefix)
    )
...
def _get_options():
    ...
    parser = argparse.ArgumentParser(
        "Pre-processing for group-based connectivity/similarity analysis using AFNI and SUMA\nNikolaas N. Oosterhof Jan 2012"
    )
