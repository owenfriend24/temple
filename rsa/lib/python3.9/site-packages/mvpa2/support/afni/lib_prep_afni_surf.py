...
def augmentconfig(c):
    ...
    if surfdir:
        ...
        if not (os.path.exists(surfdir) or os.path.split(surfdir)[1] == "surf"):
            print(
                (
                    "Warning: surface directory %s not does exist "
    ...
    if c.get("steps", "all") == "all":
        c["steps"] = (
            "toafni+mapico+moresurfs+skullstrip+"
    ...
    if "expvol" in c:
        ...
        if c.get("outvol_space", None) is None:
            ...
            if (
                len(outvol_space)
                and not c["notemplate"]
                and outvol_space.lower() != "orig"
            ):
                ...
                if o == "+orig":
                    ...
                    print(
                        (
                            "Template space '%s' detected: '"
                            "output has extension %s" % (outvol_space, o)
                        )
                    )
                c["template"] = True
            else:
...
def _ensure_expvol_is_plump(expvol, func=None):
    ...
    for line in info_lines:
        ...
        if has_match:
            ...
            if "oblique" in value:
                ...
                raise ValueError('Unable to find orientation in: "%s"' % info_lines)
    ...
    if not is_plump:
        raise ValueError(
            'The volume "%s" is oblique (not plump), for which '
...
def run_toafni(config, env):
    ...
    if config["overwrite"] or not filesexist:
        ...
        print((
            "SUMA conversion appears to have been "
            "performed already for %s in %s" % (sid, sd)
        ))
    ...
    for icold in icolds:
        ...
        for hemi in hemis:
            if not config["overwrite"]:
                ...
                if os.path.exists(lastsurffn):
                    print(
                        (
                            "Seems MapIcosahedron was already run "
                            "for %sh with ld=%d" % (hemi, icold)
                        )
                    )
                    continue

            cmd = (
                "MapIcosahedron -overwrite -spec %s_%sh.spec "
                "-ld %d -fix_cut_surfaces -prefix %s"
        ...
        if len(spherefns) == 2 and "l" in hemis and "r" in hemis:
            ...
            if config["overwrite"] or not os.path.exists(mappathfn):
                ...
                print((
                    (
                        "Computing bijection between nodes (ico=%d) - '"
    ...
    for icold in icolds:
        ...
        for hemi in hemis:
            for surftriple in moresurfs:
                fns = [
                    "%s/%s%sh.%s%s" % (d, icoprefix, hemi, surfname, ext)
...
def run_skullstrip(config, env):
    ...
    surfvol_trgs = [
        "%s/%s" % (refdir, fn)
        for fn in ["%s_SurfVol_ss%s" % (sid, fullext), "%s_SurfVol%s" % (sid, fullext)]
    ]
    ...
    for src, trg in zip(surfvol_srcs, surfvol_trgs):
        if os.path.exists(trg) and not overwrite:
            ...
            cmds.append(
                'cd "%s"; 3dresample -overwrite -orient '
                "LPI -inset %s -prefix ./%s" % (refdir, src, trg_short)
            )
    ...
    if overwrite or not utils.afni_fileexists(expvol_trg):
        if do_ss:
            ...
            cmds.append(
                'cd "%s";3dbucket -overwrite -prefix ./%s%s %s'
                % (refdir, expvol_trg_tmp_prefix, ext, expvol_src)
            )
        cmds.append(
            'cd "%s"; 3dresample -overwrite -orient LPI '
            "-prefix %s -inset %s%s"
...
def run_alignment(config, env):
    ...
    surfvol = "%(refdir)s/%(sid)s_" "SurfVol%(outvol_fullext)s" % config
    surfvol_ss = "%(refdir)s/%(sid)s_" "SurfVol%(sssuffix)s%(outvol_fullext)s" % config
    ...
    aloutfns = [
        "%s%s" % (ssalprefix, fullext),
        fullmatrixfn,
    ]  # expected output files if alignment worked
    if config["overwrite"] or not all(
        ...
        if config["identity"]:
            ...
            twovolpat = (
                (
                    "-anat %s -epi %s -anat2epi -epi_base 0 "
                    "-anat_has_skull no "
                    "-epi_strip None"
                )
                if config["isepi"]
    ...
    if overwrite or not os.path.exists("%s/%s" % (refdir, matrixfn)):
        cmds.append(
            'cd "%s"; cat_matvec %s > %s || exit 1' % (refdir, fullmatrixfn, matrixfn)
        )
    ...
    if overwrite or not os.path.exists(svalignedfn):
        ...
        cmds.append(
            'cd "%s";3dWarp -overwrite -newgrid %f '
    ...
    for fn in headernukefns:
        for field in headernukefields:
            ...
            cmd = (
                'cd "%s"; m=`grep "%s" %s | wc -w`; if [ $m -eq 0 ]; '
                "then %s; "
    ...
    if config["AddEdge"]:
        ...
        orig_ext = "+orig"
        addedge_rootfns = [
            "%s_%s%%s" % (ae_e_n, postfix) for postfix in ["e3", "ec", ae_s_n + "_ec"]
        ]
        ...
        addegde_pathfns_orig = [
            pathjoin(refdir, x % "+orig") for x in addedge_fns_pat
        ] + addedge_fns
        ...
        if overwrite or not all(addegde_exists):
            ...
            cmds.extend(
                [
                    'if [ -e "%s" ]; then rm "%s"; fi' % (fn, fn)
                    for fn in addegde_pathfns_orig + addegde_pathfns_ext
                ]
            )
            cmds.append(
                ";".join(
            ...
            for fn in set_space_fns:
                if fn.endswith(".log"):
                    continue
                cmds.append(
                    "if [ -e %s ]; then %s; fi" % (fn, _set_vol_space_cmd(fn, config))
                )
            ...
            for sumafile in sumafiles:
                if fnmatch.fnmatch(sumafile, pat):
                    ...
                    if config["overwrite"] or not os.path.exists(
                        ...
                        cmd = (
                            'cd "%s";ConvertSurface -overwrite -i_fs '
...
def run_makespec_bothhemis(config, env):
    ...
    for icold in icolds:
        ...
        for add_state, is_req in zip(add_states, add_states_required):
            has_state = all(
                [len(spec.find_surface_from_state(add_state)) == 1 for spec in specs]
            )
            ...
            specs = afni_suma_spec.hemi_pairs_add_views(
                specs, add_state, ext, refdir, overwrite=overwrite
            )
        ...
        hemimerged = "m"
        specfn = afni_suma_spec.canonical_filename(
            icold, hemimerged, config["alsuffix"]
        )
        specpathfn = pathjoin(refdir, specfn)
        ...
        if config["overwrite"] or not os.path.exists(specpathfn):
            ...
            for fn_out, fns_in in list(surfs_to_join.items()):
...
def run_makesurfmasks(config, env):
    ...
    s2v_cmd = (
        '3dSurf2Vol -map_func mask2 -data_expr "a*%%d" '
        "-spec %%s %%s -sv %s"
    ...
    for hemi in hemis:
        ...
        for infix, val in list(infix2val.items()):
...
def suma_makerunsuma(fnout, specfn, surfvol):
    ...
    lines = [
        "export SUMA_AllowDsetReplacement=YES",
        "killall afni",
        "afni -niml &" "suma -spec %s -sv %s" % (shortspecfn, surfvol),
    ]
    ...
    for surfname in usesurfs:
        if surfname in surfname2filename:
            ldp = (
                "SAME"
                if (
                    not localdomainparent
                    or localdomainparent == surfname2filename[surfname]
                )
...
def _get_hemis_icolds(config):
    ...
    icolds = [
        int(v) for v in config["ld"].split("+")
    ]  # linear divisions for MapIcosehedron
...
remove and/or overwrite existing files."""
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="prep_afni_surf %s" % _VERSION
    )
    parser.add_argument(
        "-s", "--sid", required=False, help="subject id used in @SUMA_Make_Spec_FS "
    )
    parser.add_argument(
        "-d", "--surfdir", required=False, help="FreeSurfer surf/ directory"
    )
    parser.add_argument(
        "-a",
        "--anatvol",
    ...
    parser.add_argument(
        ...
        required=False,
        choices=yesno,
        help="Is the experimental volume an EPI (yes) " "or anatomical (no)",
    )
    parser.add_argument(
        "-r",
        "-o",
    ...
    parser.add_argument(
        "--aea_opts",
        default="-cmass cmass+xyz -big_move",
        help="Options given to align_epi_anat " "([-cmass cmass+xyz -big_move])",
    )
    parser.add_argument(
        "-I",
        "--identity",
    ...
    parser.add_argument(
        ...
        default="ascii",
        choices=["gifti", "ascii"],
        help="Output format of surfaces: 'ascii' " "(default - for now) or 'gifti'",
    )
    parser.add_argument(
        "-T",
        "--template",
    ...
    for i, ref in enumerate(refs):
        ...
        c.update(
            dict(
                ...
                steps="all",
                ld="4+32",
                aea_opts="-cmass cmass+xyz -big_move",
                alsuffix="_al",
                verbose=True,
            )
        )
