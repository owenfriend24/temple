...
class SplitterTests(unittest.TestCase):
    ...
    def test_n_group_split(self):
        ...
        for isreversed, splitter in enumerate((hs, hs)):
            ...
            assert_array_equal(
                splits[0][1 - isreversed].sa["chunks"].unique, [0, 1, 2, 3, 4]
            )
            assert_array_equal(
                splits[0][isreversed].sa["chunks"].unique, [5, 6, 7, 8, 9]
            )
            assert_array_equal(
                splits[1][1 - isreversed].sa["chunks"].unique, [5, 6, 7, 8, 9]
            )
            assert_array_equal(
                splits[1][isreversed].sa["chunks"].unique, [0, 1, 2, 3, 4]
            )
        ...
        for isreversed, s5splitter in enumerate((s5, s5)):
            ...
            # check split content
            assert_array_equal(splits[0][1 - isreversed].sa["chunks"].unique, [0, 1])
            assert_array_equal(
                splits[0][isreversed].sa["chunks"].unique, [2, 3, 4, 5, 6, 7, 8, 9]
            )
            assert_array_equal(splits[1][1 - isreversed].sa["chunks"].unique, [2, 3])
            assert_array_equal(
                splits[1][isreversed].sa["chunks"].unique, [0, 1, 4, 5, 6, 7, 8, 9]
            )
            # ...
            assert_array_equal(splits[4][1 - isreversed].sa["chunks"].unique, [8, 9])
            assert_array_equal(
                splits[4][isreversed].sa["chunks"].unique, [0, 1, 2, 3, 4, 5, 6, 7]
            )
    ...
    def test_nfold_random_counted_selection_partitioner(self):
        ...
        split_partitions = [
            tuple(x.sa.partitions) for x in NFoldPartitioner(0.5).generate(self.data)
        ]
        ...
        assert_equal(
            set(split_partitions).intersection(split10_partitions),
            set(split10_partitions),
        )
        assert_equal(
            set(split_partitions).intersection(split10_partitions_),
    ...
    def test_nfold_random_counted_selection_partitioner_huge(self):
        ...
        ds = dataset_wizard(
            np.arange(1000).reshape((-1, 1)),
            targets=list(range(1000)),
            chunks=list(range(500)) * 2,
        )
        split_partitions_random = [
            tuple(x.sa.partitions)
            for x in NFoldPartitioner(
                100, selection_strategy="random", **kwargs
            ).generate(ds)
        ]
    ...
    def test_custom_split(self):
        ...
        splitters = (
            Splitter(attr="partitions", attr_values=[1, 2]),
    ...
    def test_counted_splitting(self):
        ...
        for strategy in Partitioner._STRATEGIES:
            for count, target in [
                ...
                nfs = NFoldPartitioner(
                    cvtype=1, count=count, selection_strategy=strategy
                )
                ...
                if strategy == "first":
                    ...
                    raise RuntimeError("Add unittest for strategy %s" % strategy)
