...
class MiscDatasetFxTests(unittest.TestCase):
    ...
    def test_coarsen_chunks(self):
        ...
        ds = dataset_wizard(
            samples=np.arange(len(chunks)).reshape((len(chunks), 1)),
            targets=[1] * 8,
            chunks=chunks,
        )
    ...
    def test_binds(self):
        ...
        for f in [
            ...
            res = eval("ds.%s()" % f)
            self.assertTrue(
                res is not None, msg="We should have got result from function %s" % f
            )
            self.assertTrue(
    ...
    def test_sequence_stat(self):
        ...
        self.assertAlmostEqual(rp["sumabscorr"], 1.0)
        self.assertAlmostEqual(np.max(rp["corrcoef"] * (len(sp) - 1) + 1.0), 0.0)
    ...
    @sweepargs(ds=iter(list(datasets.values())))
