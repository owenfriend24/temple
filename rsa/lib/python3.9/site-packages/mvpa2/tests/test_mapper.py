...
def test_flatten():
    ...
    for fm in [
        FlattenMapper(space="voxel"),
        ChainMapper(
            [FlattenMapper(space="voxel"), StaticFeatureSelection(slice(None))]
        ),
    ]:
        # not working if untrained
        assert_raises(RuntimeError, fm.forward1, np.arange(np.sum(samples_shape) + 1))
        ...
        assert_array_equal(fm.reverse1(target[0]), _verified_reverse1(fm, target[0]))
...
def test_chainmapper():
    ...
    if __debug__:
        ...
        assert_equal(
            "#".join(repr(cm_clone).split("#")[:-1]), "#".join(repr(cm).split("#")[:-1])
        )
    else:
        cm_clone = eval(repr(cm))
