...
class SearchlightHyperalignmentTests(unittest.TestCase):
    ...
    def get_testdata(self):
        ...
        while len(dss_rotated_clean) < n:
            ds_ = random_affine_transformation(ds_orig, scale_fac=1.0, shift_fac=0.0)
    ...
    def test_hyperalignment_measure(self):
        ...
        for noisy, dss in ((False, dss_rotated_clean), (True, dss_rotated)):
            ...
            mappers = fsha(dss)
            mappers = [StaticProjectionMapper(proj=m, recon=m.T) for m in mappers]
            idhashes_ = [idhash(ds.samples) for ds in dss]
            idhashes_targets_ = [idhash(ds.targets) for ds in dss]
            self.assertEqual(
                idhashes, idhashes_, msg="Hyperalignment must not change original data."
            )
            self.assertEqual(
                idhashes_targets,
                idhashes_targets_,
            ...
            ds_orig_Rref = np.dot(ds_orig.samples, Rs[ref_ds]) * np.sign(
                dss_rotated_clean[ref_ds].a.random_scale
            )
            ...
            # normed differences
            self.assertTrue(
                np.all(np.array(nddss) <= (0.2, 3)[int(noisy)]),
                msg="Should have reconstructed original dataset more or"
                " less for all. Got normed differences %s in %s case."
                % (nddss, snoisy),
            )
            self.assertTrue(
                nddss[ref_ds] <= (0.1, 0.4)[int(noisy)],
        ...
        fsha_fsf_same = FeatureSelectionHyperalignment(
            featsel=0.5, use_same_features=True
        )
        ...
        assert_true(
            np.alltrue([np.sum(m[:4, :4].std(0) > 0) == 4 for m in mappers_fsf_same])
        )
    ...
    def test_searchlight_hyperalignment(self, tempfile):
        ...
        while len(dss) < nds - 1:
            sd = local_random_affine_transformations(
                ...
                scatter_neighborhoods(
                    Sphere(1), ds_orig.fa[space].value, deterministic=True
                )[1],
                Sphere(2),
                space=space,
                scale_fac=1.0,
            ...
            if np.sum(np.isnan(sd.samples) + np.isinf(sd.samples)) == 0 and np.all(
                sd.samples.std(0)
            ):
                dss.append(sd)
        ...
        slhyp = SearchlightHyperalignment(ref_ds=2, exclude_from_model=[1], featsel=0.7)
        projs1 = slhyp(dss)
        ...
        for i in [0, 2, 3, 4]:
            assert_array_almost_equal(
                projs1[i].proj.todense(), projs2[i].proj.todense()
            )
            assert_array_almost_equal(aligned1[i].samples, aligned2[i].samples)
        assert_false(np.all(projs1[1].proj.todense() == projs1[2].proj.todense()))
        ...
        for midx in range(nds):
            ...
            assert_array_almost_equal(
                projs[3][midx].proj.todense(), projs[4][midx].proj.todense()
            )
            # recon check
            assert_array_almost_equal(
                projs[0][midx].proj.todense(),
        ...
        for proj in projs:
            ...
            for i in range(1, nds - 1):
                assert (
                    np.sum(
                        proj[i].proj.toarray().max(1).squeeze()
                        == proj[i].proj.diagonal()
                    )
                    / float(proj[i].proj.shape[0])
                    < 0.80
                )
        ...
        for ds_hyper in dss_hyper:
            ndcs = np.diag(
                np.corrcoef(ds_hyper.samples.T, ds_orig.samples.T)[nf:, :nf], k=0
            )
            ndcss += [ndcs]
    ...
    def test_custom_qas(self):
        ...
        assert_array_equal(
            projs[0], tprojs_shifted[0]
        )  # must be identity since we made them so
        assert_array_equal(
            projs[1], tprojs_shifted[1]
        )  # pretty much incorporating that shift
