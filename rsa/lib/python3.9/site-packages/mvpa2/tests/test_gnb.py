...
def test_gnb_sensitivities(logprob):
    ...
    assert_equal(
        s.shape,
        (((len(ds.uniquetargets) * (len(ds.uniquetargets) - 1)) / 2), ds.nfeatures),
    )
...
def test_gnb_overflow():
    ...
    assert_equal(
        sorted(np.unique(probs)), [0, 1]
    )  # quantized into 0, 1 given this many samples
...
def _test_gnb_overflow_haxby():  # pragma: no cover
    ...
    haxby = load_tutorial_data(
        datapath,
        roi="vt",
        add_fa={
            "vt_thr_glm": os.path.join(
                datapath, "haxby2001", "sub001", "masks", "orig", "vt.nii.gz"
            )
        },
    )
    ...
    haxby = haxby[
        np.array(
            [l in ["rest", "scrambled"] for l in haxby.targets],  # ''house', 'face']
            dtype="bool",
        )
    ]
    ...
    cv = CrossValidation(
        clf, HalfPartitioner(attr="chunks"), postproc=None, enable_ca=["stats"]
    )
