...
class SurfTests(unittest.TestCase):
    ...
    def test_surf(self, temp_fn):
        ...
        vf_checks = [(v_check, lambda x: x.vertices), (f_check, lambda x: x.faces)]
        ...
        for _ in [0, 1]:
            nbrs = s.neighbors
            n_check = [(0, 96, 0.284629), (40, 39, 0.56218349), (100, 99, 0.1741202)]
        ...
        low2high = s.map_to_high_resolution_surf(h, 0.1)
        if on_windows:
            raise SkipTest(
                """Known failure:
        ...
        for k, v in list(partmap.items()):
            assert_true(low2high[k] == v)

        # ensure that slow implementation gives same results as fast one
        low2high_slow = s.map_to_high_resolution_surf(h, 0.1)
        for k, v in list(low2high.items()):
            assert_true(low2high_slow[k] == v)

        #  should fail if epsilon is too small
        assert_raises(ValueError, lambda x: x.map_to_high_resolution_surf(h, 0.01), s)
        ...
        assert_array_almost_equal(s3.vertices[-1, :], np.array([18.0, 19, 0.0]))
    ...
    def test_surf_border(self):
        ...
        vb = reduce(
            np.logical_or, [v[:, 0] == 0, v[:, 1] == 0, v[:, 0] == 9, v[:, 1] == 9]
        )
    ...
    def test_surf_fs_asc(self, temp_fn):
        ...
        for side_facing in sides_facing:
            l, r = surf.reposition_hemisphere_pairs(s + 10.0, t + (-10.0), side_facing)

            m = surf.merge(l, r)

            # not sure at the moment why medial rotation
            # messes up - but leave for now
            eps = 666 if side_facing == "m" else 0.001
    ...
    def test_volgeom(self, temp_fn):
        ...
        d = 2.0  # voxel size
        xo, yo, zo = -6.0, -12.0, -20.0  # origin
        mx = np.identity(4, float) * d  # affine transformation matrix
        mx[3, 3] = 1
        mx[0, 3] = xo
        mx[1, 3] = yo
        mx[2, 3] = zo
        ...
        for other_sz, (eq_shape, eq_nvoxels) in list(eq_shape_nvoxels.items()):
        ...
        deltas = [-0.51, -0.49, 0.0, 0.49, 0.51]
        ...
        for delta, r in zip(deltas, should_raise):
            ...
            if r:
                assert_raises(AssertionError, assert_array_almost_equal, lin_d, lin)
            else:
    ...
    def test_volgeom_masking(self):
        ...
        for dilate in dilates:
            covers_full_volume = dilate * 2 >= maskstep * 3 ** 0.5 + 1
    ...
    def test_volsurf(self):
        ...
        density = 40
        outer = surf.generate_sphere(density) * 25.0 + 25
        inner = surf.generate_sphere(density) * 20.0 + 25
        ...
        for sp, sa, so in steps_start_stop:
            vs = volsurf.VolSurfMaximalMapping(
                vg, outer, inner, (outer + inner) * 0.5, sp, sa, so
            )
            ...
            counter = 0
            for k, v2pos in list(n2v.items()):
                for v, pos in list(v2pos.items()):
                    # should be close to grey matter

                    assert_true(-1.0 <= pos <= 2.0)
                    counter += 1
    ...
    def test_volume_mask_dict(self):
        ...
        outer = surf.generate_sphere(density) * 10.0 + 5
        inner = surf.generate_sphere(density) * 5.0 + 5

        intermediate = outer * 0.5 + inner * 0.5
        xyz = intermediate.vertices

        radius = 50

        outside_node_margins = [None, 0, 100.0, np.inf, True]
    ...
    def test_surf_voxel_selection(self):
        ...
        outer = surf.generate_sphere(density) * 25.0 + 15
        inner = surf.generate_sphere(density) * 20.0 + 15
        ...
        for param in params:
            ...
            sel = surf_voxel_selection.voxel_selection(
                vs, radius, source_surf_nodes=srcs, distance_metric=distance_metric
            )
            ...
            if test_full(param):
                ...
                sel3cmp_difference_ratio = [(sel, 0.2), (sel4, 0.0)]
    ...
    def test_agreement_surface_volume(self):
        ...
        assert_array_equal(msk.ravel() + 0.0, node_msk.ravel() + 0.0)
    ...
    def test_surf_queryengine(self, qefn):
        ...
        # add second layer
        s2 = surf.merge(s, (s + (0.01, 0, 0)))

        ds = Dataset(
            samples=np.arange(20)[np.newaxis],
        ...
        for distance_metric in ("euclidean", "dijkstra", "<illegal>", None):
            ...
            for node in np.arange(-1, s2.nvertices + 1):
                ...
                assert_equal(
                    set(feature_ids),
                    set((base_ids[np.newaxis].T + [0, 20, 40]).ravel()),
                )

                node_indices = list(
                    s2.circlearound_n2d(node, radius, distance_metric or "dijkstra")
                )
    ...
    def test_surf_ring_queryengine(self):
        ...
        assert_raises(
            ValueError,
            lambda: queryengine.SurfaceRingQueryEngine(
...
0 2 3
        ...
        f = np.asarray([[0, 1, 2], [1, 2, 3], [0, 1, 3], [0, 2, 3]], dtype=np.int32)
