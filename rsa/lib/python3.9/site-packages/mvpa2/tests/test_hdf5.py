...
def test_directaccess(fname):
    ...
    assert_array_equal(h5load(fname).samples, datasets["uni4medium"].samples)
...
@sweepargs(
    obj=(
        ...
        np.array([[{"d": np.empty(shape=(2, 3)), "k": 33}] * 2] * 3, dtype=object),
        # Swaroop's use-case
        ...
        np.array(
            [np.array([{"d": np.empty(shape=(2, 3))}], dtype=object)], dtype=object
        ),
...
if hasattr(collections, "OrderedDict"):
    _python_objs.append(
        [collections.OrderedDict(), collections.OrderedDict(a9=1, a0=2)]
    )
...
_unicode_arrays = [
    np.array([["a", "мама", "x"], ["ы", "a", "z"]], order=o) for o in "CF"
]
...
_nested_d = {0: 2}
_nested_d[1] = {0: {3: 4}, 1: _nested_d}  # to ease comprehension of the dump
...
def test_reg_load_hyperalignment_example_hdf5():
    ...
    if not os.path.exists(filepath):
        raise SkipTest("No hyperalignment tutorial data available under %s" % filepath)
...
def test_regress_fmri_dataset(tempfile=None, testfile=None):
    ...
    if not (exists(testfile) and exists(realpath(testfile))):
        raise SkipTest(
            "File %s seems to be missing -- 'git annex get .' "
    ...
    if os.stat(testfile).st_size < 1000:
        raise SkipTest(
            "File %s seems to be small/empty -- 'git annex get .' "
            "to fetch all test files first" % testfile
        )
    ...
    assert_equal(
        sorted(ds.sa.keys()), ["chunks", "targets", "time_coords", "time_indices"]
    )
    ...
    if externals.versions["nibabel"] >= ds.a.versions["nibabel"]:
        ...
        raise SkipTest(
            ...
            "with: %s"
            % (externals.versions["nibabel"], testfile, ds.a.versions["nibabel"])
        )
