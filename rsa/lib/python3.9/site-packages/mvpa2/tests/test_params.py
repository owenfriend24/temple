...
class ParametrizedClassifierExtended(ParametrizedClassifier):
    def __init__(self):
        ...
        self.kernel_params["kp2"] = KernelParameter(200.0, doc="Very useful param")


class ChoiceClass(ClassWithCollections):
    C = Parameter(
        "choice1", constraints=EnsureChoice("choice1", "choice2"), doc="documentation"
    )
...
class ChoiceSubClass(ChoiceClass):
    C = Parameter(
        "choice3", constraints=EnsureChoice("choice2", "choice3"), doc="documentation"
    )
...
class SimpleClass(ClassWithCollections):
    C = Parameter(
        1.0,
        constraints=Constraints(EnsureFloat(), EnsureRange(min=0.0, max=10.0)),
        doc="C parameter",
    )
    ...
    def test_incorrect_parameter_error(self):
        ...
        try:
            ...
            raise AssertionError(
                "Must have failed with an exception here " "due to incorrect parameter"
            )
        except Exception as e:
            estr = str(e)
        self.assertTrue(
            not "calling_time" in estr,
