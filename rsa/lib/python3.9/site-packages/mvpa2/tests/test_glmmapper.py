...
def get_bold():
    ...
    baseline = 800
    wsignal = baseline + 8.0 * model_lr + np.random.randn(int(samples / tr / 10)) * 4.0
    nsignal = baseline + np.random.randn(int(samples / tr / 10)) * 4.0

    ds = Dataset(samples=np.array([wsignal, nsignal]).T, sa={"model": model_lr})
...
def test_glm_mapper():
    ...
    for klass in implementations:
        ...
        pest = klass([], add_regs=(("trend", (np.linspace(-1, 1, len(bold)))),))(bold)
        assert_equal(pest.shape, (1, bold.nfeatures))
        assert_array_equal(pest.sa.regressor_names, ["trend"])
        # full monty
        pest = klass(
            ["model"],
            add_regs=(("trend", (np.linspace(-1, 1, len(bold)))),),
            add_constant=True,
            space="conditions",
            return_design=True,
            return_model=True,
