...
class TransformerTests(unittest.TestCase):
    ...
    def test_first_axis_sum_not_zero(self):
        src = [[1, -22.9, 6.8, 0], [-0.8, 7, 0, 0.0], [88, 0, 0.0, 0], [0, 0, 0, 0.0]]
    ...
    def test_rank_order(self):
        ...
        self.assertTrue(
            (uout == np.arange(nelements)).all(),
            msg="We should get all indexes. Got just %s" % uout,
        )
        self.assertTrue(
            (uoutr == np.arange(nelements)).all(),
    ...
    def test_dist_p_value(self):
        ...
        if cfg.getboolean("tests", "labile", default="yes"):
            ...
            self.assertTrue(
                (
                    np.array(distPValue.ca.positives_recovered)
                    + np.array(distPValue.ca.nulldist_number)
                    == ndb + ndu
                ).all()
            )
