...
def collect_unit_tests(verbosity=1):
    ...
    for external, testname in __optional_tests:
        if externals.exists(external):
            ...
            print(
                (
                    'T: Tests from "test_%s" are skipped due to missing externals: %s'
                    % (testname, external)
                )
            )
...
def run_tests_using_nose(limit=None, verbosity=1, exit_=False):
    ...
    if not externals.exists("nose"):
        warning(
            "You do not have python-nose installed.  Some unittests were "
    ...
    config = nose.config.Config(
        verbosity=max(0, verbosity - 1), plugins=nose.plugins.DefaultPluginManager()
    )
    if limit is None:
        ...
        if verbosity:
            ...
            testfiles = glob.glob(
                "%s%stest_*.py" % (os.path.dirname(__file__), os.path.sep)
            )
            not_tested = set([os.path.basename(f) for f in testfiles]) - set(
                ["%s.py" % f for f in tests]
            )
            if len(not_tested):
                print(
                    (
        ...
        config.testNames = ["mvpa2.tests." + nt for nt in tests if nt[5:] in limit]
