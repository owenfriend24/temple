...
def timesegments_classification(
    ...
    while True:
        ...
        dss_train, dss_test = list(
            zip(*[list(Splitter("partitions").generate(ds)) for ds in dss_partitioned])
        )
        ...
        for ds_test_part in part2.generate(ds_test):
            ...
            if distance == "correlation":
                ...
                dist = (
                    1
                    - np.corrcoef(ds_train_, ds_test_)[len(ds_test_) :, : len(ds_test_)]
                )
            else:
    ...
    if __debug__:
        debug(
            "BM",
            "Finished with %s array of errors. Mean error %.2f"
