...
class FeatureSelection(SliceMapper):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(FeatureSelection, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["filler"], default=0)
        )
    ...
    def reverse1(self, data):
        ...
        if not data.shape[: len(self._oshape)] == self._oshape:
            raise ValueError(
                "Data shape does not match training "
                "(trained: %s; got: %s)" % (self._dshape, data.shape)
            )
    ...
    def _reverse_data(self, data):
        ...
        # features
        mapped.resize(data.shape[:1] + self._dshape + data.shape[2:], refcheck=False)
...
class StaticFeatureSelection(FeatureSelection):
    ...
    def __repr__(self, prefixes=None):
        if prefixes is None:
            prefixes = []
        return super(FeatureSelection, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["dshape", "oshape"])
        )
...
class SensitivityBasedFeatureSelection(FeatureSelection):
    ...
    def _get_selected_ids(self, dataset):
        ...
        if __debug__:
            debug(
                "FS_", "Sensitivity: %s Selected ids: %s" % (sensitivity, selected_ids)
            )
...
class IterativeFeatureSelection(FeatureSelection):
    ...
    errors = ConditionalAttribute(doc="History of errors")
    nfeatures = ConditionalAttribute(doc="History of # of features left")
...
class SplitSamplesProbabilityMapper(SliceMapper):
    ...
    def _train(self, ds):
        ...
        splits = split_by_sample_attribute(ds_copy, self._split_by_labels)
    ...
    def _forward_dataset(self, ds):
        sliced_ds = [
            ds[sample_ids, feature_ids]
            for sample_ids, feature_ids in zip(
                *(self._slice_sample_ids, self._slice_feature_ids)
            )
        ]
    ...
    sensitivity_analyzer = property(
        fget=lambda self: self._sensitivity_analyzer,
        doc="Measure which was used to do selection",
    )
    selector = property(
        fget=lambda self: self._selector, doc="Function used to do selection"
    )
