...
class ConnectivityHyperalignment(SearchlightHyperalignment):
    ...
    def _get_seed_means(self, measure, queryengine, dataset, seed_indices):
        ...
        seed_data = Searchlight(
            measure,
            queryengine=queryengine,
            nproc=self.params.nproc,
            roi_ids=seed_indices,
        )
        seed_data = seed_data(dataset)
        zscore(seed_data, chunks_attr=None)
        return seed_data

    def _get_sl_connectomes(
        self, seed_means, qe_all, datasets, inode, connectivity_mapper
    ):
    ...
    def _get_connectomes(self, datasets):
        ...
        seed_means = [
            self._get_seed_means(MeanFeatureMeasure(), qe, ds, params.seed_indices)
        ...
        if params.npcs is None:
            ...
            for inode in roi_ids:
                ...
                local_common_model = (
                    sl_common_models[inode][:, : params.npcs]
