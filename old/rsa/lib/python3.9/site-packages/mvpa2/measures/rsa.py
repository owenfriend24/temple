...
class CDist(Measure):
    ...
    def _call(self, ds):
        ...
        if test_ds.nsamples != self._train_ds.nsamples:
            raise ValueError(
                "Datasets should have same sample size for dissimilarity, "
                "nsamples for train: %d, test: %d"
                % (self._train_ds.nsamples, test_ds.nsamples)
            )
        # Call actual distance metric
        distds = cdist(
            self._train_ds.samples,
            test_ds.samples,
        ...
        for k in self._train_ds.sa:
            if k in test_ds.sa:
                sa_dict[k] = list(
                    product(self._train_ds.sa.get(k).value, test_ds.sa.get(k).value)
                )
...
class PDist(Measure):
    ...
    def _call(self, ds):
        ...
        if self.params.square:
            ...
            out = Dataset(squareform(dsm), sa=ds.sa)
        else:
            # add some attributes
            out = Dataset(
                dsm, sa=dict(pairs=list(combinations(list(range(len(ds))), 2)))
            )
        return out
...
class PDistConsistency(Measure):
    ...
    def _call(self, dataset):
        ...
        if nchunks < 2:
            raise Exception(
                "This measure calculates similarity consistency across "
        ...
        if self.params.square:
            ...
            ds = Dataset(
                squareform(corrmat, checks=False),
...
class Regression(Measure):
    ...
    alpha = Parameter(
        1.0, constraints="float", doc="alpha parameter for lasso" "regression"
    )

    fit_intercept = Parameter(
        True, constraints="bool", doc="whether to fit the" "intercept"
    )
    ...
    def __init__(self, predictors, keep_pairs=None, **kwargs):
        ...
        if len(predictors.shape) == 1:
            raise ValueError(
                "predictors have shape {0}. Make sure the array "
    ...
    def _call(self, dataset):
        ...
        # first run PDist
        compute_dsm = PDist(
            pairwise_metric=self.params.pairwise_metric,
        ...
        if dsm_samples.shape[0] != predictors.shape[0]:
            raise ValueError(
                "computed dsm has {0} rows, while predictors have"
                "{1} rows. Check that predictors have the right"
                "shape".format(dsm_samples.shape[0], predictors.shape[0])
            )
