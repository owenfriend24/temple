...
def test_query_engine():
    ...
    ds = Dataset(
        [data, data],
        fa={"s_ind": np.concatenate((ind, ind)), "t_ind": np.repeat([0, 1], 27)},
    )
    ...
    assert_array_equal(qe(s_ind=(0, 0, 0), t_ind=[0, 1, 10]), qe(s_ind=(0, 0, 0)))
    ...
    assert_array_equal(qe_lit(s_ind=(0, 0, 0), lit="roi1"), [0, 3, 9, 27, 30, 36])
    assert_array_equal(
        qe_lit(s_ind=(0, 0, 0), lit=["roi1", "ro2"]), [0, 1, 3, 9, 27, 28, 30, 36]
    )
...
def test_scattered_neighborhoods():
    ...
    scoords, sidx = ne.scatter_neighborhoods(sphere, coords, deterministic=False)
