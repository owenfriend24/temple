...
from mvpa2.misc.surfing.volume_mask_dict import (
...
from mvpa2.misc.surfing.queryengine import (
...
class SurfVoxelSelectionTests(unittest.TestCase):
    def test_voxel_selection(self):
        ...
        """
        surf_voxsel = surf_voxel_selection.voxel_selection(
            vsm, radius, distance_metric="e"
        )
        ...
        if run_slow:
            meas = CrossValidation(
                GNB(), OddEvenPartitioner(), errorfx=lambda p, t: np.mean(p == t)
            )
        ...
        dataset = fmri_dataset(
            samples=pathjoin(pymvpa_dataroot, "bold.nii.gz"),
        ...
        dataset = dataset[
            np.array([l in ["face", "house"] for l in dataset.sa.targets], dtype="bool")
        ]
    ...
    def test_voxel_selection_alternative_calls(self):
        ...
        for s, fn in zip(
            [outer, inner, intermediate], [outerfn, innerfn, intermediatefn]
        ):
        ...
        for i in range(0, len(combis), combistep):
            ...
            if call_method_ == "qe":
                ...
                sel = surf_voxel_selection.voxel_selection(
                    vs, radius, source_surf=s_m, source_surf_nodes=center_nodes_
                )
    ...
    def test_volsurf_projections(self):
        ...
        assert_equal(n2vs, dict((i, {i: 0.0, i + 100: 1.0}) for i in range(100)))
    ...
    def test_surface_outside_volume_voxel_selection(self, fn):
        ...
        radii = [10.0, 10]  # fixed and variable radii
            ...
            for radius in radii:
                ...
                if type(radius) is int and outside_node_margin is True:
                    ...
                    if outside_node_margin is True:
                        ...
                        # all be empty
                        assert_array_equal(
                            list(sel.keys()), list(range(inner.nvertices))
                        )
                    ...
                    if (
                        outside_node_margin is True
                        and externals.versions["hdf5"] < "1.8.7"
                    ):
                        raise SkipTest(
                            "Versions of hdf5 before 1.8.7 have "
    ...
    def test_queryengine_io(self, fn):
        ...
        for setstate_use_legacy, reduce_use_legacy in true_false_combis:
            reducer = (
                VolumeMaskDictionary._reduce_legacy
            ...
            setstater = (
                VolumeMaskDictionary._setstate_legacy
            ...
            for vmd_load_method in vmd_load_methods:
                ...
                for key, v in zip(add_fa, expected_values):
                    for id in qe.ids:
                        assert_array_equal(
                            sel.get_aux(id, key), sel_copy.get_aux(id, key)
                        )
    ...
    def test_surface_minimal_lowres_voxel_selection(self, fn):
        ...
        outer = surf.generate_plane((0, 0, 4), (0, 0.4, 0), (0, 0, 0.4), 14, 14)
        inner = outer + 2

        source = surf.generate_plane((0, 0, 4), (0, 0.8, 0), (0, 0, 0.8), 7, 7) + 1

        for i, nvm in enumerate(("minimal", "minimal_lowres")):
            qe = disc_surface_queryengine(
                radius, vg, inner, outer, source, node_voxel_mapping=nvm
            )
            ...
            if i == 0:
                ...
                for k in list(voxsel.keys()):
                    ...
                    assert_true(len(delta) < 0.8 * (len(p) + len(q)))
    ...
    def test_mask_with_keys(self):
        ...
        for constructor, ids2nfeatures in list(qe_ids2nvoxels.items()):
            qe = constructor(sel)
            qe.train(ds)
            img = qe.get_masked_nifti_image()
            assert_array_equal(
                img.get_data(), qe.get_masked_nifti_image(qe.ids).get_data()
            )

            img_getter = qe.get_masked_nifti_image
            for ids, nfeatures in list(ids2nfeatures.items()):
                ...
                if nfeatures is None and ids is not None:
                    ...
                    if constructor is SurfaceVerticesQueryEngine:
                        ...
                        check_image_func = lambda x: check_mask_func(
                            x.get_data()
                        ) and assert_array_equal(x.affine, expected_image.affine)
    ...
    def test_volume_mask_dictionary_different_value_types(self):
        ...
        at_expected = dict(
            foo=(
                np.asarray([0, 1]),
                np.asarray([2, 2]),
                np.asarray([1.1, 2.0, 1.3, 2.0]),
            )
        )
        for i, (at_elem, at_expected_elem) in enumerate(
            zip(at["foo"], at_expected["foo"])
        ):
