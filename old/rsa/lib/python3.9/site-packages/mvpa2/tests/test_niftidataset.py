...
from mvpa2.datasets.mri import fmri_dataset, _load_anyimg, map2nifti, strip_nibabel
...
def test_nifti_dataset():
    ...
    mask[40, 20, 12] = True
    nddata = fmri_dataset(
        samples=pathjoin(pymvpa_dataroot, "example4d.nii.gz"), targets=[1, 2], mask=mask
    )
...
def test_nifti_mapper(filename):
    ...
    data = fmri_dataset(
        samples=pathjoin(pymvpa_dataroot, "example4d.nii.gz"), targets=[1, 2]
    )
...
def test_multiple_calls():
    ...
    data = fmri_dataset(
        samples=pathjoin(pymvpa_dataroot, "example4d.nii.gz"), targets=1, sprefix="abc"
    )
    data2 = fmri_dataset(
        samples=pathjoin(pymvpa_dataroot, "example4d.nii.gz"), targets=1, sprefix="abc"
    )
...
def test_er_nifti_dataset():
    ...
    ds_alt = preprocessed_dataset(
        tssrc, nibabel.load, fmri_dataset, preproc_ds=toevents
    )
    assert_datasets_almost_equal(ds, ds_alt)
    ...
    for i, onset in enumerate(
        [value2idx(e["onset"], ds_orig.sa.time_coords, "floor") for e in evs]
    ):
        ...
        for evattr in [
            ...
            assert_array_equal(
                evs[i]["_".join(evattr.split("_")[2:])], ds.sa[evattr].value[i]
            )
    ...
    # and they have been broadcasted through all boxcars
    assert_array_equal(
        ds.fa.voxel_indices[:nnonzero], ds.fa.voxel_indices[nnonzero : 2 * nnonzero]
    )
...
def test_er_nifti_dataset_mapping():
    ...
    tds = fmri_dataset(
        nibabel.Nifti1Image(samples.T, None), mask=nibabel.Nifti1Image(dsmask.T, None)
    )
    ds = eventrelated_dataset(
        tds,
        events=[
            Event(onset=0, duration=2, label=1, chunk=1, features=[1000, 1001]),
            Event(onset=1, duration=2, label=2, chunk=1, features=[2000, 2001]),
        ],
    )
    ...
    # reproducability
    assert_array_equal(
        ds_sel.samples, ds_sel.a.mapper.forward(np.rollaxis(samples.T, -1))
    )
...
def test_assumptions_on_nibabel_behavior(filename):
    ...
    for h, t, d in ((hdr, "int16", 2), (hdr64, "float64", 166)):
