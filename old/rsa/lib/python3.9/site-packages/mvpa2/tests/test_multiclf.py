...
def test_multiclass_ties(clf):
    ...
    te = CrossValidation(
        clf, NFoldPartitioner(), postproc=mean_sample(), enable_ca=["stats"]
    )
    te_ = CrossValidation(
        clf_, NFoldPartitioner(), postproc=mean_sample(), enable_ca=["stats"]
    )
    ...
    if len({"libsvm", "sg", "skl", "gpr", "blr"}.intersection(clf.__tags__)):
    ...
    if not (
        "random_tie_breaking" in clf.__tags__
...
def test_multiclass_classifier_pass_ds_attributes():
    ...
    assert_array_equal(
        res.sa.cvfolds, np.repeat(list(range(len(ds.UC))), len(ds) / len(ds.UC))
    )
...
def test_multiclass_without_combiner():
    ...
    if mcv.ca.is_enabled("training_stats"):
        ...
        assert_equal(
            set(training_stats.keys()), {("L0", "L1"), ("L0", "L2"), ("L1", "L2")}
        )
        for pair, cm in list(training_stats.items()):
