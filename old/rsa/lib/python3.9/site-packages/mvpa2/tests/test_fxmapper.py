...
def test_samplesgroup_mapper_test_order_occurrence():
    ...
    ds = dataset_wizard(samples=data, targets=[1, 0] * 4, chunks=[0] * 4 + [1] * 4)
...
def test_features01():
    ...
    #       help in mappers/fx
    aov = OneWayAnova(postproc=FxMapper("features", lambda x: x / x.max(), attrfx=None))
...
def test_uniquemerge2literal():
    assert_equal(u2l(list(range(3))), ["0+1+2"])
...
def test_bin_prop_ci():
    ...
    assert_true(maxdist > 0 or maxdist <= 1.0 / n)
    assert_true(mindist > 0 or mindist <= 1.0 / n)
...
def test_mean_removal():
    ...
    for function in functions:
        assert_true(
            np.array_equal(function(test_array.copy()), desired_result), function
        )

    for function in functions:
        assert_true(
            np.array_equal(function(test_dataset.copy()).samples, desired_result)
        )

    random_array = np.random.rand(50, 1000)
    assert_true(np.array_equal(mr_fx(random_array.copy()), mr(random_array.copy())))
    assert_true(
        np.array_equal(mr_fx(random_array.copy()), mr_inplace(random_array.copy()))
    )
