...
from mvpa2.misc.transformers import Absolute, DistPValue
...
class SensitivityAnalysersTests(unittest.TestCase):
    ...
    def test_basic(self, dsm):
        ...
        self.assertTrue(
            abs(f.samples[0, 1]) <= 1e-12,  # some small value
    ...
    @sweepargs(
        clfds=[(c, datasets["uni2large"]) for c in clfswh["has_sensitivity", "binary"]]
        + [(c, datasets["uni4large"]) for c in clfswh["has_sensitivity", "multiclass"]]
    )
    def test_analyzer_with_split_classifier(self, clfds):
        ...
        mclf = SplitClassifier(
            clf=clf, partitioner=partitioner, enable_ca=["training_stats", "stats"]
        )
        sana = mclf.get_sensitivity_analyzer(  # postproc=absolute_features(),
            pass_attr=["fa.nonbogus_targets"], enable_ca=["sensitivities"]
        )
        ...
        if "5%" in clf.descr or (nlabels > 2 and "regression_based" in clf.__tags__):
        ...
        if True:
            ...
            for sens1 in sensm:
                ...
                if not isinstance(label, tuple):
                    ...
                    ilabel2 = [
                        np.where(ds.fa.nonbogus_targets == l)[0][0] for l in label
                    ]
                    self.assertEqual(
                        set(maxsensi2),
                        set(ilabel2),
    ...
    def test_mapped_classifier_sensitivity_analyzer(self, clf):
        ...
        sana = mclf.get_sensitivity_analyzer(
            postproc=sumofabs_sample(), enable_ca=["sensitivities"]
        )
    ...
    def test_linear_svm_weights(self, svm):
        ...
        self.assertRaises(NotImplementedError, svmnl.get_sensitivity_analyzer)
    ...
    def test_linear_svm_weights_per_class(self, svm):
        ...
        sana_full = svm.get_sensitivity_analyzer(force_train=False, **kwargs)
        ...
        dmap = (-1 * senssplit.samples[1] + senssplit.samples[0]) - sensfull.samples
        ...
        warning.handlers = []
        self.assertRaises(NotImplementedError, sana_split, datasets["uni3medium"])
    ...
    def test_split_featurewise_dataset_measure(self):
        ...
        assert_equal(
            sens.shape,
            (len(ds.sa["chunks"].unique) * len(ds.sa["targets"].unique), ds.nfeatures),
        )
        ...
        assert_equal(sens.shape, (2 * len(ds.sa["targets"].unique), ds.nfeatures))
    ...
    def __test_fspipeline_with_split_classifier(self, basic_clf):
        ...
        rfe = RFE(
            sensitivity_analyzer=sclf.get_sensitivity_analyzer(
                enable_ca=["sensitivities"]
            ),
            transfer_error=trans_error,
            feature_selector=FeatureSelectionPipeline(
                [FractionTailSelector(0.5), FixedNElementTailSelector(1)]
            ),
            train_pmeasure=True,
        )
    ...
    def test_union_feature_selection(self):
        ...
        fss = [
            ...
            SensitivityBasedFeatureSelection(
                SMLRWeights(SMLR(lm=1, implementation="C"), postproc=sumofabs_sample()),
    ...
    def test_clf_transfer_measure(self):
        ...
        tm = TransferMeasure(clf, Splitter("chunks", count=2), enable_ca=["stats"])
        res = tm(self.dataset)
        manual_error = np.mean(res.samples.squeeze() != res.sa.targets)
        postproc_error = enode(res)
        tm_err = TransferMeasure(clf, Splitter("chunks", count=2), postproc=enode)
    ...
    def test_pseudo_cv_measure(self):
        ...
        cv = CrossValidation(
            clf, cvgen, enable_ca=["stats", "training_stats", "datasets"]
        )
        res = cv(self.dataset)
        assert_equal(res.shape, (len(self.dataset.sa["chunks"].unique), 1))
    ...
    def test_custom_combined_selectors(self):
        ...
        fs = SensitivityBasedFeatureSelection(OneWayAnova(), custom_tail_selector)
        ds = datasets["3dsmall"]
    ...
    def test_combined_node(self):
        ds = datasets["3dsmall"]
        axis2nodes = dict(h=(mean_feature, mean_feature), v=(mean_sample, mean_sample))
    ...
    def test_split_samples_probability_mapper(self):
        ...
        element_selector = FractionTailSelector(0.4, mode="select", tail="upper")
        common = True
        m = SplitSamplesProbabilityMapper(
            analyzer,
            "subjects",
