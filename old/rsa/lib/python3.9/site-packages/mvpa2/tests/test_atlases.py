...
def test_atlases(name):
    ...
    for res in [atlas(coord), atlas.label_point(coord)]:
        ok_(
            res.get("coord_queried", None) == coord,
...
def test_fsl_hox_queries():
    ...
    assert_equal(len(atl.find(re.compile("Fusiform"), unique=False)), 4)
    ...
    ok_(
        r_point["voxel_atlas"]
        == r_point["voxel_queried"]
        == list(r_voxel["voxel_queried"])
        == [138, 51, 91]
    )
    ...
    atl2 = Atlas(name="HarvardOxford-Cortical", image_file=atl._image_file)
...
def test_pymvpa_talairach():
    ...
    atld = Atlas(name="talairach-dist", reference_level="Closest Gray", distance=10)
