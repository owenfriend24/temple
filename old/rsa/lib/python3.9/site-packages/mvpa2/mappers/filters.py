...
class FFTResampleMapper(Mapper):
    ...
    def __repr__(self):
        ...
        return s.replace("(", "(chunks_attr=%s, " % (repr(self.__chunks_attr),), 1)
    ...
    def _forward_dataset_helper(self, ds):
        ...
        if self.__position_attr is not None:
            ...
            rsamples, pos = resample(
                ds.samples, self.__num, t=pos, window=self.__window_args
            )
        else:
            # we know nothing about samples position
            rsamples = resample(
                ds.samples, self.__num, t=None, window=self.__window_args
            )
        ...
        if self.__attr_strategy == "remove":
            ...
            for k in ds.sa:
                v = ds.sa[k].value
                if pos is None:
                    sa[k] = resample(v, self.__num, t=None, window=self.__window_args)
                else:
                    if k == self.__position_attr:
                        # position attr will be handled separately at the end
                        continue
                    sa[k] = resample(v, self.__num, t=pos, window=self.__window_args)[0]
            # inject them all
            mds.sa.update(sa)
        else:
            raise ValueError(
                "Unkown attribute handling strategy '%s'." % self.__attr_strategy
            )
...
class IIRFilterMapper(Mapper):
    ...
    def _forward_data(self, data):
        ...
        try:
            ...
            if params.axis > 1:
                raise ValueError(
                    "this version of scipy does not " "support nd-arrays for filtfilt()"
                )
            if not (params["padlen"].is_default and params["padtype"].is_default):
                warning(
                    "this version of scipy.signal.filtfilt() does not "
                    "support `padlen` and `padtype` arguments -- ignoring "
                    "them"
                )
            mapped = [filtfilt(self.__iir_num, self.__iir_denom, x) for x in data]
