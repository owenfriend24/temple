...
class ProcrusteanMapper(ProjectionMapper):
    ...
    def _train(self, source):
        ...
        if sn != tn:
            raise ValueError(
                "Data for both spaces should have the same "
                "number of samples. Got %d in source and %d in target space" % (sn, tn)
            )
        ...
        for i in range(2):
            if np.all(
                ssqs[i] <= np.abs((np.finfo(datas[i].dtype).eps * sn * means[i]) ** 2)
            ):
        ...
        if sm > tm:
            if params.reduction:
                ...
                raise ValueError(
                    "reduction=False, so mapping from "
        ...
        if params.oblique:
            ...
            if params.svd == "numpy":
                U, s, Vh = np.linalg.svd(np.dot(target.T, source), full_matrices=False)
                ...
                U, s, Vh = scipy.linalg.svd(
                    np.dot(target.T, source), full_matrices=False
                )
            elif params.svd == "dgesvd":
                from mvpa2.support.lapack_svd import svd as dgesvd

                U, s, Vh = dgesvd(
                    np.dot(target.T, source), full_matrices=True, algo="svd"
                )
            else:
    ...
    def _compute_recon(self):
        ...
        if self.params.oblique:
            ...
            return (
                np.transpose(self._proj / self._scale ** 2)
                if self.params.scaling
                else np.transpose(self._proj)
            )
