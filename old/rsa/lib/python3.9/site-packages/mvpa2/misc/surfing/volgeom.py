...
class VolGeom(object):
    ...
    def __init__(self, shape, affine, mask=None):
        ...
        if mask is not None:
            if mask.size != self.nvoxels:
                raise ValueError(
                    "%d voxels, but mask has %d" % (self.nvoxels, mask.size)
                )
    ...
    def same_geometry(self, other):
        ...
        return self.same_shape(other) and np.all(self.affine == other.affine)
    ...
    def __eq__(self, other):
        return self.same_geometry(other) and np.all(self.affine == other.affine)
    ...
    def __repr__(self, prefixes=None):
        if prefixes is None:
            prefixes = []
        prefixes_ = [
            "shape=(%s)" % ",".join(["%r" % i for i in self._shape]),
            "affine=%r" % self._affine,
        ] + prefixes
    ...
    def xyz2ijk(self, xyz):
        ...
        ijk = np.array(ijkfloat + 0.5, dtype=int)
    ...
    def contains_ijk(self, ijk, apply_mask=True):
        ...
        return np.logical_not(self._outside_vol(ijk, lin, apply_mask=apply_mask))
    ...
    def get_masked_array(self, nt=None, dilate=None):
        ...
        if dilate is not None and self.nvoxels_mask != self.nvoxels:
    ...
    try:
        ...
        if isinstance(mask_volume, int):
            ...
            if ndim <= 3:
                ...
                if mask_volume > 0:
                    warning(
                        "There is no 4th dimension (t) to select "
        ...
        try:
            ...
            if isinstance(mask_volume, int):
                ...
                if mask_volume is None and (
                    hasattr(s, "fa") and hasattr(s.fa, "voxel_indices")
                ):
            ...
            raise ValueError(
                ...
                "(filename of) Nifti image, or (mri-)Dataset: %s" % (s, e)
            )
...
def to_vector(v):
    ...
    if len(v.shape) > 1:
        if v.shape[0] != 1 and v.shape(1) != 1:
            raise ValueError(
                "Matrix of shape %d x %d: cannot make linear" % (v.shape[0], v.shape[1])
            )
        v = v.ravel()
    return v
