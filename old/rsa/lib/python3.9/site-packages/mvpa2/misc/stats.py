...
def binomial_proportion_ci(n, X, alpha=0.05, meth="jeffreys"):
    ...
    if meth == "wald":
        ...
        L = stats.beta.ppf(alpha / 2, X + 0.5, n - X + 0.5)
        U = stats.beta.ppf(1 - alpha / 2, X + 0.5, n - X + 0.5)
...
def binomial_proportion_ci_from_bool(arr, axis=0, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    """
    return binomial_proportion_ci(
        arr.shape[axis], np.sum(arr, axis=axis), *args, **kwargs
    )
...
def compute_ts_boxplot_stats(
    data,
    outlier_abs_minthresh=None,
    outlier_thresh=3.0,
    ...
    *args :
    ...
    if outlier_thresh > 0.0:
        ...
        outlier = np.logical_and(
            np.logical_not(np.isnan(data)),
            np.ma.greater((np.absolute(data - meand)), outlier_thresh * stdd),
        )

        if outlier_abs_minthresh is not None:
            # apply absolute filter in addition
            outlier = np.logical_and(
                outlier, np.ma.greater(data, outlier_abs_minthresh)
            )
        if greedy_outlier:
        ...
        data = np.ma.masked_array(data.data, mask=np.logical_or(data.mask, outlier))
        outlierd = np.ma.masked_array(data.data, mask=np.logical_not(outlier))
    ...
    res = {
        ...
        "n": len(data) - data.mask.sum(axis=0),
    }
    return res, outlierd
