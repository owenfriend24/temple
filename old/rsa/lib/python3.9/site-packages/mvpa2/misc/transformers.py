...
def rank_order(x, reverse=False):
    ...
    rankorder[[x[0][1] for x in ztosort]] = [x[1] for x in ztosort]
    return rankorder
...
class OverAxis(object):
    ...
    def __call__(self, x, *args, **kwargs):
        ...
        if axis >= len(shape):
            raise ValueError(
                "Axis given in constructor %d is higher "
...
class DistPValue(ClassWithCollections):
    ...
    nulldist_number = ConditionalAttribute(
        enabled=True, doc="Number of features within the estimated null-distribution"
    )
    ...
    def __init__(self, sd=0, distribution="rdist", fpp=None, nbins=400, **kwargs):
        ...
        if not (distribution in ["rdist"]):
            raise ValueError(
                "Actually only rdist supported at the moment" " got %s" % distribution
            )
    ...
    def __call__(self, x):
        ...
        if __debug__:
            if nd < x.shape[0]:
                warning(
                    "Number of features in DistPValue lower than number of"
            ...
            if fpp is not None:
                ...
                while True:
                    ...
                    if __debug__:
                        debug(
                            "TRAN_",
                            "empirical p=%.3f for theoretical p=%.3f" % (pN_emp, fpp),
                        )
                    ...
                    keep = np.logical_and(
                        xxx > bins[0], xxx < bins[-1]  # + bins_halfstep,
                    )  # - bins_halfstep)
                    if __debug__:
                        debug(
                            "TRAN_",
                            "Keeping %d out of %d elements" % (np.sum(keep), Nxxx),
                        )
