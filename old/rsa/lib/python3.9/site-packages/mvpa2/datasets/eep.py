...
def eep_dataset(samples, targets=None, chunks=None):
    ...
    if isinstance(samples, str):
        ...
        raise ValueError(
            "eep_dataset takes the filename of an "
...
class EEPBin(DataReader):
    ...
    def __init__(self, source):
        ...
        self._data = np.reshape(
            np.fromfile(infile, dtype="f"),
            (nsamples, self._props["nchannels"], self._props["ntimepoints"]),
        )
    ...
    nchannels = property(
        fget=lambda self: self._props["nchannels"], doc="Number of channels"
    )
    ntimepoints = property(
        fget=lambda self: self._props["ntimepoints"], doc="Number of data timepoints"
    )
    nsamples = property(
        fget=lambda self: self._data.shape[0], doc="Number of trials/samples"
    )
    t0 = property(
        fget=lambda self: self._props["t0"],
        doc="Relative start time of sampling interval",
    )
    dt = property(
        fget=lambda self: self._props["dt"],
        doc="Time difference between two adjacent samples",
    )
    channels = property(
        fget=lambda self: self._props["channels"], doc="List of channel names"
    )
