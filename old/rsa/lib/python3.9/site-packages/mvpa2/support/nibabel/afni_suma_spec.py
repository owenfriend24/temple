...
class SurfaceSpec(object):
    ...
    def as_string(self):
        ...
        for surface in self.surfaces:
            ...
            lines.extend("    %s = %s" % kv for kv in list(surface.items()))
            lines.append("")
            ...
            for arg in args:
                ...
                for func in funcs:
                    ...
                    if not surfs_filter:
                        ...
                        return pathjoin(self.directory, surfs_filter[0]["SurfaceName"])
...
def hemi_pairs_add_views(spec_both, state, ext, directory=None, overwrite=False):
    ...
    if state == "inflated":
        views = dict(
            m="medial", s="superior", i="inferior", a="anterior", p="posterior"
        )
        viewkeys = ["m", "s", "i", "a", "p"]
    else:
        views = dict(m="medial")
...
def combine_left_right(leftright):
    ...
    mergeable = lambda x: (
        (x["Anatomical"] == "Y") or x["SurfaceState"].startswith(_COMPREFIX)
    )
...
def merge_left_right(both):
    ...
    m = SurfaceSpec(
        m_surfaces, states=m_states, groups=m_groups, directory=both.directory
    )
...
def find_file(directory, icold=None, hemi=None, suffix=None):
    fn = pathjoin(directory, canonical_filename(icold=icold, hemi=hemi, suffix=suffix))
    if not os.path.exists(fn):
        suffix = "*"
        pat = pathjoin(
            directory, canonical_filename(icold=icold, hemi=hemi, suffix=suffix)
        )
        fn = glob.glob(pat)
