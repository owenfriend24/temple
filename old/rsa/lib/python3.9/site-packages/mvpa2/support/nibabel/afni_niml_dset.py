...
def _string2list(s, SEP=";", warn_if_no_string=True):
    ...
    if isinstance(s, (list, tuple)):
        if warn_if_no_string and any(isinstance(s_elem, (list, tuple)) for s_elem in s):
            # a short representation of the offending structure
            s_str = "%r" % s if len(s) <= 1 else "[%s ... %s]" % (s[0], s[-1])
            warning(
                "Two-dimensional string structure %s found - "
...
def _dset2rawniml_data(s):
    return dict(data_type="Node_Bucket_data", name="SPARSE_DATA", data=s["data"])
...
def _dset2rawniml_nodeidxs(s):
    ...
    node_idxs = (
        s.get("node_indices")
        if "node_indices" in s
        else np.arange(nrows, dtype=np.int32)
    )
    ...
    if node_idxs is not None:
        ...
        if node_idxs.size != nrows:
            raise ValueError(
                "Size mismatch for node indices (%r) and data (%r)"
                % (node_idxs.size, nrows)
            )

        if node_idxs.shape != (nrows, 1):
            node_idxs = np.reshape(
                node_idxs, (nrows, 1)
...
def _dset2rawniml_datarange(s):
    ...
    try:
        ...
        for i in range(len(minpos)):
            ...
            mxpos = maxpos[i]
            r.append(
                "%s %s %d %d" % (f(data[mnpos, i]), f(data[mxpos, i]), mnpos, mxpos)
            )

        # range of data in each column
        return dict(atr_name="COLMS_RANGE", data=r)
    except:
        return dict(atr_name="COLMS_RANGE", data=None)
...
def _dset2rawniml_labels(s):
    ...
    if len(labels) != ncols:
        raise ValueError(
            "Wrong number of labels (%s): found %d but expected %d"
            % (labels, len(labels), ncols)
        )
    return dict(atr_name="COLMS_LABS", data=labels)
...
def _dset2rawniml_history(s):
    ...
    if history and not history.endswith("\n"):
        history += "\n"
    history += "%s Saved by %s:%s" % (
        logprefix,
        __file__,
        sys._getframe().f_code.co_name,
    )

    return dict(atr_name="HISTORY_NOTE", data=history)
...
def _dset2rawniml_datatypes(s):
    ...
    datatype = [
        "Generic_Int" if types.numpy_data_isint(data) else "Generic_Float"
    ] * ncols
    return dict(atr_name="COLMS_TYPE", data=datatype)
...
def _dset2rawniml_stats(s):
    ...
    if stats is None:
        stats = ["none"] * ncols
    return dict(atr_name="COLMS_STATSYM", data=stats)
    ...
    if tp == "mixed":
        ...
        for d in data:
            if isinstance(d, str) or (
                type(d) is list and all(isinstance(di, str) for di in d)
            ):
                tpstr = "String"
...
def sparse2full(
    ...
    pad_to_ico_ld=None,
    pad_to_node=None,
    ico_ld_surface_count=1,
    set_missing_values=0,
):
    """
    ...
    if nrows != n:
        raise ValueError(
            "element count mismatch between data (%d) and "
            "node indices (%d)" % (nrows, n)
        )

    if n > pad_to_node:
        raise ValueError(
            "data has more rows (%d) than pad_to_node (%d)", (n, pad_to_node)
        )

    full_node_indices_vec = np.arange(pad_to_node)
    full_node_indices = np.reshape(full_node_indices_vec, (pad_to_node, 1))

    full_data = np.zeros((pad_to_node, ncols), dtype=data.dtype) + set_missing_values
...
def label2index(dset, label):
    ...
    if type(label) is int:
        ...
        if label < 0 or label >= sh[1]:
            raise ValueError("label index %d out of bounds (0.. %d)" % (label, sh[1]))
        return label
...
def ttest(dsets, sa_labels=None, return_values="mt", set_NaN_to=0.0, compare_to=0.0):
    ...
    for i, dset in enumerate(dsets):
        ...
        if i == 0:
            ...
            data = np.zeros(
                (nn, nc, ns), dset_data.dtype
            )  # number of nodes, columns, subjects
        ...
        if i == 0:
            ...
            if set(node_idxs0) != set(node_idxs):
                raise ValueError("non-matching node indices for %d and %d" % (0, i))
    ...
    if do_t:
        ...
        t = stats.ttest_1samp(data - compare_to, 0.0, axis=2)[0]
