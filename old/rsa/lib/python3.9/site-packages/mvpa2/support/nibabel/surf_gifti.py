...
def _get_single_array(g, intent):
    ...
    if n != 1:
        ...
        raise ValueError("Found %s arrays matching %s, expected 1" % (len_str, intent))
    return ar[0]
...
def filename2vertices_faces_metadata(fn):
    ...
    vertex_map = dict(
        AnatomicalStructurePrimary=dict(
            lh="CortexLeft", rh="CortexRight", mh="CortexRightLeft"
        ),
        AnatomicalStructureSecondary=dict(
            pial="Pial", smoothwm="GrayWhite", intermediate="MidThickness"
        ),
        GeometricType=dict(
            pial="Anatomical",
            smoothwm="Anatomical",
    ...
    def just_one(dict_, fn=fn):
        vs = [v for k, v in list(dict_.items()) if k in fn]
        return vs[0] if len(vs) == 1 else None

    v_meta = [gifti.GiftiNVPairs("Name", fn)]

    for key, dict_ in list(vertex_map.items()):
        v = just_one(dict_)
        if v is not None:
...
def to_xml(img, meta_fn_hint=None):
    ...
    if meta_fn_hint is not None:
        ...
        vertices.meta, faces.meta = filename2vertices_faces_metadata(meta_fn_hint)
    ...
    fix_odd_even = (
        lambda x, i: x.replace(b"\n", b"")
        if i % 2 == 1
        else x.replace(b'Endian="GIFTI_ENDIAN_LITTLE"', b'Endian="LittleEndian"')
    )
...
def write(fn, s, overwrite=True):
    ...
    if not fn.endswith(EXT):
        raise ValueError(
            "Filename %s does not end with required" " extension %s" % (fn, EXT)
        )
