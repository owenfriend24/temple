...
if [[ $# -lt 1 ]]; then
    echo "Usage:  sl_to_mni.sh sub fmriprep_dir comp roi (e.g., sl_to_mni.sh temple024 FM AB b_hip"
    exit 1
fi
...
WarpImageMultiTransform 3 "${fmriprep_dir}/searchlight/prepost_${comp}/${sub}_prepost_${roi}_z.nii.gz" "${fmriprep_dir}/searchlight/mni_${comp}/${sub}_${comp}_${roi}_mni.nii.gz" -R "/home1/09123/ofriend/analysis/temple/bin/templates/MNI152_T1_func_brain.nii.gz" "${fmriprep_dir}/sub-${sub}/transforms/native_to_MNI_Warp.nii.gz" "${fmriprep_dir}/sub-${sub}/transforms/native_to_MNI_Affine.txt"
