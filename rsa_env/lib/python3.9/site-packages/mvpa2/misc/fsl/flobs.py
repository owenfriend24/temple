...
def make_flobs(
    ...
    if nsecs < 0:
        nsecs = int(math.ceil(pre[1] + rise[1] + fall[1] + undershoot[1] + resolution))
    else:
    ...
    tochild, fromchild, childerror = os.popen3(
        "halfcosbasis"
        + " --hf="
        + pfilename
        + " --nbfs="
    ...
    hrfs = np.fromfile(pathjoin(wdir, "out", "hrfsamps.txt"), sep=" ")

    # reshape array to get one sample per row and 1d array only
    # for one sample hrf
    hrfs = hrfs.reshape(len(hrfs) / rnsamples, rnsamples).T[:nsamples].squeeze()
