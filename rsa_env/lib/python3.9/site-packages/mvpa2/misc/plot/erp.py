...
def _make_centeredaxis(
    ax, loc, offset=5, ai=0, mult=1.0, format="%4g", label=None, **props
):
    ...
    **props
    ...
    xlocs = [l for l in ax.xaxis.get_ticklocs() if xmin <= l <= xmax]
    ylocs = [l for l in ax.yaxis.get_ticklocs() if ymin <= l <= ymax]
    ...
    args = [(locs, [loc] * len(locs)), (vrange, [loc, loc]), [locs, (loc,) * len(locs)]]
...
def plot_erp(
    ...
    *args, **kwargs
    ...
    if onsets is not None:  # if we need to extract ERPs
        ...
        bcm = BoxcarMapper(onsets, boxlength=int(SR * duration), offset=-int(SR * pre))
        erp_data = bcm(data)
    ...
    ax.plot(time_points, erp_mean, lw=2, color=color, zorder=4, *args, **kwargs)
    ...
    **kwargs
    ...
    for erp_def in erps:
        ...
        if isinstance(erp_def, tuple) and len(erp_def) == 3:
            params.update(
                {"label": erp_def[0], "color": erp_def[1], "onsets": erp_def[2]}
            )
        ...
        if plot_data is None:
            raise ValueError(
                "Channel %s got no data provided" % params.get("label", "UNKNOWN")
            )

        plot_erp(plot_data, pre=pre, pre_onset=pre_onset, post=post, ax=ax, **params)
