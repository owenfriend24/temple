...
def unstructured_xy2grid_xy_vectors(x, y, min_nsteps):
    ...
    xi = (np.arange(xsteps) + 0.5) * delta + xmin
    yi = (np.arange(ysteps) + 0.5) * delta + ymin
...
def flat_surface2xy(surface, max_deformation):
    ...
    if len(too_deformed) > 0:
        raise ValueError(
            "Surface is not sufficiently flat with "
...
def _scale(xs, target_min=0.0, target_max=1.0, source_min=None, source_max=None):
...
def flat_surface_data2rgba(data, range_="2_98%", threshold=None, color_map=None):
    ...
    scaled = _scale(data, 0.0, 1.0, mn, mx)
    rgba = cmap(scaled)
...
def curvature_from_any(c):
    ...
    if isinstance(c, str):
        ...
        if (
            len(c.shape) > 1
            and c.shape[1] == 2
            and set(c[:, 0]) == set(range(1 + int(max(c[:, 0]))))
        ):
            cc = c
            n = cc.shape[0]
            c = np.zeros((n,))
...
class FlatSurfacePlotter(object):
        ...
        if curvature is None:
            ...
            if self._surface.nvertices != len(self._curvature):
                raise ValueError(
                    ...
                    "has %d values" % (self._surface.nvertices, self._curvature.size)
                )
    ...
    def _pre_setup(self):
        if self._grid_def is None:
            self._grid_def = flat_surface2grid_mask(
                self._surface, self._min_nsteps, self._max_deformation
            )
    ...
    def __call__(self, data):
        ...
        if data.shape != expected_shape:
            raise ValueError(
                "data shape was expected to be %s based on "
        ...
        olay[nan_msk] = np.nan
        o_rgba = flat_surface_data2rgba(
            olay, self._range_, self._threshold, self._color_map
        )
