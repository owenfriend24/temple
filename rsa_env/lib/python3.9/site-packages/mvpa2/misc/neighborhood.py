...
from mvpa2.base.dochelpers import borrowdoc, _repr_attrs, _repr
...
class Sphere(object):
    ...
    def _get_increments(self, ndim):
        ...
        if element_sizes is None:
            ...
            if ndim != len(element_sizes):
                raise ValueError(
                    ...
                    "coordinate had %i" % (element_sizes, len(element_sizes), ndim)
                )
        ...
        tentative_increments = (
            np.array(list(np.ndindex(tuple(erange * 2 + 1)))) - erange
        )
        # Filter out the ones beyond the "sphere"
        return array(
            [
                x
                for x in tentative_increments
                if self._distance_func(x * element_sizes, center) <= self._radius
            ]
        )
    ...
    def __call__(self, coordinate):
        ...
        if self._increments is None or self._increments_ndim != ndim:
            if __debug__:
                debug(
                    "NBH", "Recomputing neighborhood increments for %dD Sphere" % ndim
                )
        ...
        if __debug__:
            if coordinate.dtype.char not in np.typecodes["AllInteger"]:
                raise ValueError(
                    "Sphere must be called on a sequence of "
                    "integers of length %i, you gave %s " % (ndim, coordinate)
                )
            # if dataset is None:
...
class HollowSphere(Sphere):
    ...
    def __init__(self, radius, inner_radius, include_center=False, **kwargs):
        ...
        **kwargs
        ...
        if inner_radius > radius:
            raise ValueError(
                "inner_radius (got %g) should be smaller "
    ...
    def _get_increments(self, ndim):
        ...
        return (
            np.vstack([np.zeros(ndim, dtype="int"), res])
...
class QueryEngine(QueryEngineInterface):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(QueryEngine, self).__repr__(
            prefixes=prefixes + ["%s=%r" % v for v in list(self._queryobjs.items())]
        )
    ...
    def query_byid(self, fid):
        ...
        kwargs = dict([(space, queryattrs[space][fid]) for space in queryattrs])
        return self.query(**kwargs)
...
class IndexQueryEngine(QueryEngine):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(IndexQueryEngine, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["sorted"], default=True)
        )
    ...
    def _train(self, dataset):
        ...
        for space in self._spaceorder:
            ...
            # in searcharray
            lookups[space] = lookup = dict([(u, i) for i, u in enumerate(uqattr)])
        ...
        if np.prod(dims) < dataset.nfeatures:
            raise ValueError(
                "IndexQueryEngine has insufficient information "
                ...
                "#actual features: %i)."
                % (str(self._spaceorder), np.prod(dims), dataset.nfeatures)
            )
        ...
        if len(self._searcharray.nonzero()[0]) != dataset.nfeatures:
            ...
            #        function in numpy
            raise ValueError(
                "Multiple features carry the same set of "
    ...
    def query(self, **kwargs):
        ...
        if len(kwargs):
            raise ValueError(
                "Do not know how to treat space(s) %s given "
...
class CachedQueryEngine(QueryEngineInterface):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(CachedQueryEngine, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["queryengine"])
        )
    ...
    def query(self, **kwargs):
        def to_hashable(x):
            ...
            if isinstance(x, dict):
                ...
                return tuple((k, to_hashable(v)) for (k, v) in sorted(x.items()))
...
def scatter_neighborhoods(neighbor_gen, coords, deterministic=False):
    ...
    try:
        ...
        lookup = hasher()
        _ = [
            lookup.setdefault(tuple(c), list()).append(i) for i, c in enumerate(coords)
        ]
