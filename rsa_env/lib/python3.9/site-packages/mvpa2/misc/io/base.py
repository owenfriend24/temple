...
class ColumnData(dict):
    ...
    def __init__(
        self, source, header=True, sep=None, headersep=None, dtype=float, skiplines=0
    ):
        ...
        if isinstance(source, str):
            ...
            raise ValueError("Unkown source for ColumnData [%r]" % type(source))
        ...
        for k in list(self.keys()):
            if not k in classdict:
                ...
                if __debug__:
                    debug(
                        "IOH", "Registering property %s for ColumnData key %s" % (k_, k)
                    )
                # make sure to import class directly into local namespace
                # otherwise following does not work for classes defined
                # elsewhere
                exec("from %s import %s" % (self.__module__, self.__class__.__name__))
                exec(
                    "%s.%s = property(fget=%s)" % (self.__class__.__name__, k_, getter)
                )
    ...
    def __str__(self):
        ...
        if len(list(self.keys())) > 0:
            s += " %d rows, %d columns [" % (self.nrows, self.ncolumns)
        ...
        for k in list(self.keys()):
            if length is None:
                ...
                if not len(self[k]) == length:
                    raise ValueError(
                        "Data integrity lost. Columns do not " "have equal length."
                    )
    ...
    def _from_file(self, filename, header, sep, headersep, dtype, skiplines):
        ...
        with open(filename, "r") as file_:
                ...
                for i, v in enumerate(l):
                    if dtype[i] is not None:
                        try:
                            ...
                            warning(
                                "Can't convert %r to desired datatype %r." % (v, dtype)
            ...
            if not len(tbl) == len(hdr):
                raise RuntimeError(
                    "Number of columns read from file does not "
    ...
    def __iadd__(self, other):
        ...
        for k, v in list(other.items()):
            ...
            if not isinstance(v, list):
                raise ValueError("Can only merge list data, but got [%r]." % type(v))
        ...
        with open(filename, "w") as file_:
            ...
            if header_order is None:
                if self._header_order is None:
                    ...
                    col_hdr = self._header_order + list(
                        set(self.keys()).difference(set(self._header_order))
                    )
            else:
                if not len(header_order) == self.ncolumns:
                    raise ValueError("Header list does not match number of " "columns.")
...
class SampleAttributes(ColumnData):
    ...
    def __init__(self, source, literallabels=True, header=None):
        ...
        ColumnData.__init__(self, source, header=header, sep=None, dtype=dtypes)

    def tofile(self, filename):
        """Write sample attributes to a text file."""
        ColumnData.tofile(
            self, filename, header=False, header_order=["targets", "chunks"], sep=" "
        )
...
class XAVRSensorLocations(SensorLocations):
    ...
    def __init__(self, source):
        ...
        SensorLocations.__init__(
            ...
            sep=None,
            dtype=[str, int, float, float, float],
        )
...
class TuebingenMEGSensorLocations(SensorLocations):
    ...
    def __init__(self, source):
        ...
        SensorLocations.__init__(
            self,
            source,
            header=["names", "pos_y", "pos_x", "pos_z", "pos_y2", "pos_x2", "pos_z2"],
            sep=None,
            dtype=[str, float, float, float, float, float, float],
        )


def design2labels(columndata, baseline_label=0, func=lambda x: x > 0.0):
    ...
    for row in range(columndata.nrows):
        ...
        if nselected > 1:
            ...
            raise ValueError(
                "Row #%i with items %s has multiple entries "
                "meeting the criterion. Cannot decide on the label" % (row, entries)
            )
        elif nselected == 1:
            label = selected[0][0]
        else:
...
def labels2chunks(labels, method="alllabels", ignore_labels=None):
    ...
    if method == "alllabels":
        ...
        for method, descr in list(__known_chunking_methods.items()):
            errmsg += "  %s : %s\n" % (method, descr)
        raise ValueError(errmsg)
    return chunks


labels2chunks.__doc__ = """Automagically decide on chunks based on labels
...
def safe_write(fnout, s, mode="wb"):
    ...
    if n != len(s):
        raise ValueError(
            "%d bytes out of %d were not written to %s" % (len(s) - n, len(s), fnout)
        )
    return n
