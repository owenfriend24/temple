...
class OptionGroups(object):
    ...
    def _get_group(self, name):
        ...
        try:
            ...
            print((
                "Problem addition options to the group '%s'. Most probably"
...
def split_comma_semicolon_lists(s, dtype=None):
    ...
    for x in s.split(";"):
        if not ":" in x:
            raise ValueError(
                "Each entry must be in the form key:values," " e.g. 'targets:rest'"
            )
...
parser = OptionParser(
    version=mvpa2.__version__,  # "%prog"
...
opt.help = Option(
    "-h", "--help", "--sos", action="help", help="Show this help message and exit"
)
...
if __debug__:
    ...
    def _debug_callback(option, optstr, value, parser):
        ...
        if value == "list":
            ...
            print(
                "Use python regular expressions to select group. CLF.* will"
...
opt.clf = Option(
    ...
    type="choice",
    dest="clf",
    choices=["gnb", "knn", "svm", "ridge", "gpr", "smlr"],
    default="svm",
...
opt.crossfolddegree = Option(
    "-c",
    "--crossfold",
    action="store",
    type="float",
    dest="crossfolddegree",
    default=1,
...
opt.baseline_conditions = Option(
    ...
    + _EXAMPLE("targets:rest")
    + _DEF,
)

opt.exclude_conditions = Option(
    "-e",
    "--exclude-conditions",
    action="callback",
    nargs=1,
    ...
    + _EXAMPLE("targets:rest;trials:bad")
    + _DEF,
)

opt.include_conditions = Option(
    "-i",
    "--include-conditions",
    action="callback",
    nargs=1,
...
opt.targets_sa = Option(
    "-T",
    "--targets-sa",
    action="store",
    dest="targets_sa",
    default="targets",
    help="Which sample attribute would be used for (classification)" " analysis" + _DEF,
)

opt.chunks_sa = Option(
    "--chunks-sa",
    action="store",
    dest="chunks_sa",
    default="chunks",
...
if externals.exists("pywt"):
    ...
    opt.wavelet_family = Option(
        "-w",
        "--wavelet-family",
        callback=_wavelet_family_callback,
        action="callback",
        type="string",
        dest="wavelet_family",
        default="-1",
    ...
    opt.wavelet_decomposition = Option(
        "-W",
        "--wavelet-decomposition",
        action="store",
        type="choice",
        dest="wavelet_decomposition",
        default="dwt",
        choices=["dwt", "dwp"],
    ...
    opts.add(
        "wavelet", [opt.wavelet_family, opt.wavelet_decomposition], "Wavelets mappers"
    )

# Box options

opt.boxlength = Option(
    "--boxlength",
    action="store",
    dest="boxlength",
    default=1,
...
opts.add(
    "chunk", [opt.chunk, opt.chunkLimits], "Chunk options AKA Sample attributes XXX"
)
