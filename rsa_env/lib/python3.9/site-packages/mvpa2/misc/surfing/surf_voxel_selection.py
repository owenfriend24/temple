...
class VoxelSelector(object):
    ...
    def __init__(
        ...
        if tp is int:  # fixed number of voxels
            ...
            initradius_mm = 0.001 + 1.5 * float(radius) ** 0.5
    ...
    def _select_approx(self, voxprops, count=None):
        ...
        if not distkey in voxprops:
            raise KeyError("No distance key %s in it - cannot select voxels" % distkey)
    ...
    def disc_voxel_attributes(self, src):
        ...
        if not node_in_vol(src) and not outside_node_margin is True:
            ...
            if outside_node_margin is not None:
                if math.isinf(outside_node_margin):
                    if __debug__:
                        ...
                        debug(
                            "SVS",
                            "node %s is outside - considering all other "
                    ...
                    node_distances = dist_surf.circlearound_n2d(
                        src, radius=outside_node_margin, metric=self._distance_metric
                    )
                    ...
                    if __debug__:
                        ...
                        debug(
                            "SVS",
                            "node %s is outside - considering %d distances"
                            " to other nodes that may be inside."
                            % ((src,), len(node_distances)),
                        )
                    for nd, d in list(node_distances.items()):
                        if (
                            nd in n2v
                            and n2v[nd] is not None
                            and d <= outside_node_margin
                        ):
                            if __debug__:
                                debug(
                                    "SVS",
                                    "node #%s is distance %s <= %s from #%d "
                                    " and kept" % ((src,), d, outside_node_margin, nd),
                                )
                            skip = False
                            break
            ...
            if skip:
                ...
                if __debug__:
                    debug(
                        "SVS",
                        "Skipping node %s (no voxels associated)" % (src,),
                        cr=True,
                    )
        ...
        for counter in range(maxiter):
            if radius_mm == 0:
                ...
                around_n2d = dist_surf.circlearound_n2d(
                    src, radius_mm, self._distance_metric
                )
        ...
        if counter + 1 >= maxiter:
            raise ValueError(
                "Failure to increase radius to get %d voxels for "
                " node #%d" % (radius, src)
            )
    ...
    def nodes2voxel_attributes(self, n2d, n2v, distancesummary=min):
            ...
            if nd in n2v:
                ...
                if vps is not None:
                    for vx, pos in list(vps.items()):
                        v2dps[vx].add(
                            (d, pos)
                        )  # associate voxel with tuple of distance and relative position
        ...
        def unpack_dp(vx, dp, distancesummary=distancesummary):
            d, p = distancesummary(
                dp
            )  # implicit sort by first elemnts first, i.e. distance
            return vx, d, p

        # make triples of (voxel index, distance to center node, relative position in grey matter)
        vdp = [unpack_dp(vx, dp) for vx, dp in list(v2dps.items())]
...
def voxel_selection(
    ...
    intermediate_surf = (vol_surf_mapping.pial_surface * 0.5) + (
        vol_surf_mapping.white_surface * 0.5
    )
    ...
    if _debug():
        debug(
            "SVS",
            "Generated high-res intermediate surface: "
            "%d nodes, %d faces"
            % (intermediate_surf.nvertices, intermediate_surf.nfaces),
        )
        debug(
            "SVS",
            "Mapping source to high-res surface:"
            " %d nodes, %d faces" % (source_surf.nvertices, source_surf.nfaces),
        )
    ...
    if distance_metric[0].lower() == "e" and outside_node_margin:
        ...
        src2intermediate = source_surf.map_to_high_resolution_surf(intermediate_surf)
    ...
    if nproc > 1:
        ...
        for i, block in enumerate(blocks):
            ...
            if _debug():
                debug(
                    "SVS",
                    "  starting block %d/%d: %d centers" % (i + 1, nproc, len(src_trg)),
                    cr=True,
                )
            ...
            if node2volume_attributes is None:
                ...
                if _debug():
                    ...
                    debug(
                        ...
                        "processes using '%s' backend" % (len(blocks), results_backend),
                    )
            else:
                # merge new with current data
                node2volume_attributes.merge(result)
            if _debug():
                debug("SVS", "  merged result block %d/%d" % (i + 1, nproc), cr=True)
        ...
        if _debug():
            ...
            debug(
                "SVS",
                "Merged results from %d child processed - "
        ...
        node2volume_attributes = _reduce_mapper(
            empty_dict, attribute_mapper, src_trg_nodes, eta_step=eta_step
        )
        debug("SVS", "")
        ...
        if node2volume_attributes is None:
            ...
            msgs = [
                ...
                % (
                    nvox_selected,
                    vg.nvoxels_mask,
                    100.0 * nvox_selected / vg.nvoxels_mask,
                ),
            ]
...
def run_voxel_selection(
    ...
    mapper_dict = dict(
        maximal=volsurf.VolSurfMaximalMapping,
