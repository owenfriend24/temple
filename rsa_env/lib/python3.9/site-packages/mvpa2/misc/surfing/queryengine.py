...
class SurfaceQueryEngine(QueryEngineInterface):
    ...
    def __init__(
        self, surface, radius, distance_metric="dijkstra", fa_node_key="node_indices"
    ):
        ...
        if not self.distance_metric in allowed_metrics:
            raise ValueError(
                "distance_metric %s has to be in %s"
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(SurfaceQueryEngine, self).__repr__(
            ...
            + _repr_attrs(self, ["distance_metric"], default="dijkstra")
            + _repr_attrs(self, ["fa_node_key"], default="node_indices")
        )

    def __reduce__(self):
        return (
            self.__class__,
            (self.surface, self.radius, self.distance_metric, self.fa_node_key),
    ...
    def train(self, ds):
        ...
        if fa_key not in list(ds.fa.keys()):
        ...
        if len(delta):
            raise ValueError(
                "Vertex id '%s' found that is not in "
        ...
        self._vertex2feature_map = v2f = dict(
            (vertex_id, list()) for vertex_id in range(nvertices)
        )
    ...
    def query_byid(self, vertex_id):
        ...
        if (
            vertex_id < 0
            or vertex_id >= self.surface.nvertices
            or round(vertex_id) != vertex_id
        ):
            raise KeyError(
                "vertex_id should be integer in range(%d)" % self.surface.nvertices
            )

        nearby_nodes = self.surface.circlearound_n2d(
            vertex_id, self.radius, self.distance_metric
        )
...
class SurfaceVerticesQueryEngine(QueryEngineInterface):
    ...
    def __reduce__(self):
        return (
            self.__class__,
            (self.voxsel, self.space, self._add_fa),
            dict(_map_voxel_coord=self._map_voxel_coord),
        )

    def __str__(self):
        return "%s(%s, space=%s, add_fa=%s)" % (
            self.__class__.__name__,
            self.voxsel,
            self.space,
            self.add_fa,
    ...
    def train(self, dataset):
        ...
        if vg_ds:
            ...
            if np.max(np.abs(vg_ds.affine - vg.affine)) > eps:
                raise ValueError(
                    "Mismatch in affine matrix: %r !+ %r" % (vg_ds.affine, vg.affine)
                )
            if not vg_ds.same_shape(vg):
                raise ValueError(
                    "Mismatch in shape: (%s,%s,%s) != "
                    "(%s,%s,%s)" % (vg_ds.shape[:3], vg.shape[:3])
                )
        else:
    ...
    def query_byid(self, vertexid):
        ...
        # map into dataset
        voxel_dataset_ids = [self._map_voxel_coord[i] for i in voxel_unmasked_ids]
        ...
        if self._add_fa is not None:
            ...
            for n in self._add_fa:
                ...
                ds.fa[n] = sum(
                    [[x] * len(ids) for x, ids in zip(fa_values, voxel_dataset_ids)], []
                )
            return ds
    ...
    def feature_id2linear_voxel_ids(self, feature_id):
        ...
        return [i for i, j in list(self._map_voxel_coord.items()) if feature_id in j]

    def feature_id2nearest_vertex_id(
        self, feature_id, fallback_euclidean_distance=False
    ):
    ...
    def _set_add_fa(self, add_fa):
        if add_fa is not None:
            if not set(self.voxsel.aux_keys()).issuperset(add_fa):
                raise ValueError(
                    "add_fa should list only those known to voxsel %s" % self.voxsel
                )
        self._add_fa = add_fa
...
class SurfaceVoxelsQueryEngine(SurfaceVerticesQueryEngine):
    ...
    def __init__(
        ...
        fallback_euclidean_distance: bool (default: True)
            ...
            searchlight.
        """
        super(SurfaceVoxelsQueryEngine, self).__init__(
            voxsel=voxsel, space=space, add_fa=add_fa
        )
    ...
    def __repr__(self, prefixes=None):
        if prefixes is None:
            prefixes = []
        prefixes_ = prefixes + _repr_attrs(
            self, ["fallback_euclidean_distance"], default=False
        )
        return super(SurfaceVoxelsQueryEngine, self).__repr__(prefixes=prefixes_)

    def __reduce__(self):
        return (
            self.__class__,
            (self.voxsel, self.space, self._add_fa, self.fallback_euclidean_distance),
    ...
    def train(self, ds):
        ...
        if fallback:
            ...
            feature_ids = set()
            for v in list(self._map_voxel_coord.values()):
        ...
        # output
        self._feature_id2vertex_id = dict((f, v) for f, v in fv if v is not None)
    ...
    def get_masked_nifti_image(self, center_ids=None):
        ...
        vertex_ids = [self._feature_id2vertex_id[center_id] for center_id in center_ids]
        parent = super(SurfaceVoxelsQueryEngine, self)
        return parent.get_masked_nifti_image(center_ids=vertex_ids)


def disc_surface_queryengine(
    radius,
    volume,
    white_surf,
    pial_surf,
    ...
    modality2class = dict(
        surface=SurfaceVerticesQueryEngine, volume=SurfaceVoxelsQueryEngine
    )

    if not output_modality in modality2class:
        raise KeyError(
            "Illegal modality %s: should be in %s"
