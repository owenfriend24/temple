...
class VolumeMaskDictionary(Mapping):
    ...
    def __repr__(self, prefixes=None):
        if prefixes is None:
            prefixes = []
        prefixes_ = ["vg=%r" % self._volgeom, "source=%r" % self._source] + prefixes
        ...
        if self._src2aux is not None:
            ...
            prefixes_.append("src2aux=%s(%s)" % (type(self._src2aux), dict_summary))

        return "%s(%s)" % (self.__class__.__name__, ",".join(prefixes_))

    def __str__(self):
        return "%s(%d centers, volgeom=%s)" % (
    ...
    def add(self, src, nbrs, aux=None):
        ...
        if aux:
            ...
            if expected_keys and (set(aux) != expected_keys):
                raise ValueError(
                    "aux label mismatch: %s != %s" % (set(aux), expected_keys)
                )
            for k, v in list(aux.items()):
                ...
                if len(self._src2aux[k]) == 0:
                    ...
                    v_dtype = next(iter(list(self._src2aux[k].values()))).dtype
                ...
                if len(v_arr) not in (n, 1):
                    raise ValueError(
                        "size mismatch: size %d != %d or 1" % (len(v_arr), n)
                    )
    ...
    def get_aux(self, src, label):
        ...
        if not label in self._src2aux:
            msg = "Mismatch for key %r" if src in self._src2nbr else "Unknown key %r"
    ...
    def _check_has_keys(self, keys=None, raise_=True):
        ...
        if has_missing and raise_:
            raise KeyError(
                '%d keys (including "%s") not present' % (n_missing, missing_keys.pop())
            )
        return has_missing
    ...
    def get_dataset_feature_mask(self, ds, keys=None):
        ...
        if not_in_ds:
            raise ValueError(
                "Found %d voxel indices selected that were "
                "not in dataset, first one is %s" % (len(not_in_ds), not_in_ds.pop())
            )
    ...
    @deprecated(
        "should be used for testing compatibility only - "
        "otherwise use .__reduce__ instead"
    )
    def _reduce_legacy(self):
        return self.__class__, (self._volgeom, self._source), self._getstate_legacy()
    ...
    def merge(self, other):
        ...
        if set(aks) != set(other.aux_keys()):
            if len(list(self.keys())) == 0:
                ...
                raise ValueError(
                    "Different keys in merge: %s != %s" % (aks, other.aux_keys())
                )
...
def _dict_with_arrays2array_tuple(d):
    ...
    if all(type(v) is dict for v in list(d.values())):
        ...
        return dict((k, _dict_with_arrays2array_tuple(v)) for k, v in list(d.items()))
    ...
    for i, (key, length) in enumerate(zip(keys, lengths)):
        ...
        if i == 0:
            ...
            if common_dtype == np.dtype("O"):
                raise TypeError(
                    'Elements for auxilery attribute "%s" ' "have different types" % key
                )
...
def _array_tuple2dict_with_arrays(kld):
    ...
    if type(kld) is dict:
        if all(type(v) in (tuple, dict) for v in list(kld.values())):
            # probably src2aux, so run recursively
            return dict((k, _array_tuple2dict_with_arrays(v)) for k, v in list(kld.items()))
    ...
    if pos != data.size:
        raise ValueError("data size mismatch: expected %s, found %s" % (data.size, pos))
    return d
