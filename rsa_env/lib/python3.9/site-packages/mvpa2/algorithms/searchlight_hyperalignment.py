...
class FeatureSelectionHyperalignment(ClassWithCollections):
    ...
    def __init__(
        self,
        ref_ds=0,
        hyperalignment=Hyperalignment(ref_ds=0),
        featsel=1.0,
        full_matrix=True,
        use_same_features=False,
    ...
    def __call__(self, datasets):
        ...
        if self.featsel != 1.0:
            ...
            fselector = FixedNElementTailSelector(
                nfeatures_sel, tail="upper", mode="select", sort=False
            )
            ...
            if self.use_same_features:
                ...
                datasets = [
                    sd[:, fsel] for fsel, sd in zip(features_selected, datasets)
                ]
        elif bigger_ds_idxs:
            ...
            fselector = FixedNElementTailSelector(
                nfeatures, tail="upper", mode="select", sort=False
            )
        ...
        try:
            ...
            if self.featsel != 1.0:
                ...
                for selected_features, isub in zip(features_selected, bigger_ds_idxs):
                    ...
                    mapper_full[
                        np.ix_(selected_features, list(range(nfeatures)))
                    ] = mappers[isub]
...
class SearchlightHyperalignment(ClassWithCollections):
    ...
    def __init__(self, **kwargs):
        ...
        if (
            self.params.nproc is not None
            and self.params.nproc > 1
            ...
            raise RuntimeError(
                ...
                "to 1 (got nproc=%i) or set to default None" % self.params.nproc
            )
        if not externals.exists("scipy"):
            raise RuntimeError(
                "The 'scipy' module is required for " "searchlight hyperalignment."
            )
        if self.params.results_backend == "native":
            raise NotImplementedError(
                "'native' mode to handle results is still a " "work in progress."
            )
            # warning("results_backend is set to 'native'. This has been known"
            #        "to result in longer run time when working with big datasets.")
        if self.params.results_backend == "hdf5" and not externals.exists("h5py"):
            raise RuntimeError(
                "The 'hdf5' module is required for "
                "when results_backend is set to 'hdf5'"
            )

    def _proc_block(
        self, block, datasets, featselhyper, queryengines, seed=None, iblock="main"
    ):
        if seed is not None:
            mvpa2.seed(seed)
        if __debug__:
        ...
        bar = ProgressBar()
        projections = [
            csc_matrix((self.nfeatures, self.nfeatures), dtype=self.params.dtype)
        ...
        for i, node_id in enumerate(block):
            ...
            for isub in range(len(roi_feature_ids_all)):
                if is_datasetlike(roi_feature_ids_all[isub]):
                    ...
                    roi_feature_ids_all[isub] = (
                        roi_feature_ids_all[isub].samples[0, :].tolist()
                    )
            ...
            for isub, roi_feature_ids in enumerate(roi_feature_ids_all):
                ...
                proj = coo_matrix(
                    (V, (I, J)),
                    shape=(
                        max(self.nfeatures, max(I) + 1),
                        max(self.nfeatures, max(J) + 1),
                    ),
        ...
        if self.params.results_backend == "native":
            ...
            results_file = mktemp(
                prefix=self.params.tmp_prefix, suffix="-%s.hdf5" % iblock
            )
            if __debug__:
    ...
    def __call__(self, datasets):
        ...
        _shpaldebug(
            "SearchlightHyperalignment %s for %i datasets" % (self, self.ndatasets)
        )

        selected = [_ for _ in range(ndatasets) if _ not in params.exclude_from_model]
        ref_ds_train = selected.index(params.ref_ds)
        params.hyperalignment.params.ref_ds = ref_ds_train
        warning(
            "Using %dth dataset as the reference dataset (%dth after "
            "excluding datasets)" % (params.ref_ds, ref_ds_train)
        )
        if len(params.exclude_from_model) > 0:
            warning(
                "These datasets will not participate in building common "
        ...
        if __debug__:
            ...
            for ids, ds in enumerate(datasets):
                for f, fname, tval in ((np.mean, "means", 0), (np.std, "stds", 1)):
                    vals = f(ds, axis=0)
        ...
        if params.sparse_radius is None:
            roi_ids = (
                self._get_verified_ids(queryengines)
        ...
        if params.nproc is not None and params.nproc > 1:
            ...
            params.nblocks = nproc_needed if params.nblocks is None else params.nblocks
