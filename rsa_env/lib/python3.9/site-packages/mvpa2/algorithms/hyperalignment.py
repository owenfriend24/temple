...
def mean_xy(x, y, weights=(0.5, 0.5)):
...
class Hyperalignment(ClassWithCollections):
    ...
    ref_ds = Parameter(
        None,
        constraints=(EnsureRange(min=0) & EnsureInt() | EnsureNone()),
    ...
    joblib_backend = Parameter(
        None,
        constraints=EnsureChoice("multiprocessing", "threading") | EnsureNone(),
    ...
    def train(self, datasets):
        ...
        if ca["training_residual_errors"].enabled:
            ...
            ca.training_residual_errors = Dataset(
                samples=residuals,
                sa={"levels": ["1"] + ["2:%i" % i for i in range(params.level2_niter)]},
            )

        if __debug__:
            debug("HPAL", "Hyperalignment %s for %i datasets" % (self, ndatasets))
        ...
        if params.ref_ds is None:
            ...
            if ref_ds >= ndatasets:
                raise ValueError(
                    ...
                    "bounds. We have only %i datasets provided" % (ref_ds, ndatasets)
                )
        ...
        if len(datasets) < 2:
            ...
            #
            lvl1_projdata = self._level1(
                datasets, commonspace, ref_ds, mappers, residuals
            )
            #
            # Level 2 -- might iterate multiple times
            #
            # this is the final common space
            self.commonspace = self._level2(datasets, lvl1_projdata, mappers, residuals)
    ...
    def _regularize(self, datasets, alpha):
        if __debug__:
            debug("HPAL", "Using regularized hyperalignment with alpha of %d" % alpha)
        wmappers = []
        for ids in range(len(datasets)):
    ...
    def _level1(self, datasets, commonspace, ref_ds, mappers, residuals):
        ...
        for i, (m, ds_new) in enumerate(zip(mappers, datasets)):
            ...
            if params.level1_equal_weight:
                commonspace = params.combiner1(
                    ds_, commonspace, weights=(float(counts), 1.0)
                )
            else:
        ...
        if params.nproc == 1:
            ...
            for i, (m, ds_new) in enumerate(zip(mappers, datasets)):
                ...
                m, residual = get_trained_mapper(
                    ds_new, self.commonspace, m, self.ca["residual_errors"].enabled
                )
            ...
            verbose_level_parallel = 20 if (__debug__ and "HPAL" in debug.active) else 0
            ...
            if params.joblib_backend is None:
                params.joblib_backend = (
                    "threading" if sys.platform == "darwin" else "multiprocessing"
                )
            res = Parallel(
                n_jobs=params.nproc,
