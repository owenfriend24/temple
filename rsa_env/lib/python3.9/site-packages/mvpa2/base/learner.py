...
class Learner(Node):
    ...
    training_time = ConditionalAttribute(
        enabled=True, doc="Time (in seconds) it took to train the learner"
    )
    ...
    def __call__(self, ds):
        ...
        if self.is_trained:
            ...
            if self.force_train:
                if __debug__:
                    debug("LRN", "Forcing training of %s on %s", (self, ds))
                # but retraining is enforced
                self.train(ds)
            elif __debug__:
                debug(
                    "LRN", "Skipping training of already trained %s on %s", (self, ds)
                )
        else:
            # not trained
            if self.auto_train:
                ...
                if __debug__:
                    debug("LRN", "Auto-training %s on %s", (self, ds))
                self.train(ds)
            else:
                # we always have to have trained before using a learner
                raise RuntimeError(
                    "%s needs to be trained before it can be "
                    "used and auto training is disabled." % str(self)
                )
        return super(Learner, self).__call__(ds)

    is_trained = property(
        fget=lambda x: x.__is_trained,
        fset=_set_trained,
        doc="Whether the Learner is currently trained.",
    )
    auto_train = property(
        fget=lambda x: x.__auto_train,
        doc="Whether the Learner performs automatic training" "when called untrained.",
    )
...
class CompoundLearner(Learner, CompoundNode):
    def __init__(self, learners, auto_train=False, force_train=False, **kwargs):
        ...
        """
        Learner.__init__(self, auto_train=auto_train, force_train=force_train, **kwargs)
        CompoundNode.__init__(self, learners, **kwargs)

    is_trained = property(
        fget=lambda x: all(y.is_trained for y in x),
        fset=lambda x: list(map(y._set_trained() for y in x)),
        doc="Whether the Learner is currently trained.",
    )
...
class ChainLearner(ChainNode, CompoundLearner):
    ...
    def __init__(self, learners, auto_train=False, force_train=False, **kwargs):
        ...
        CompoundLearner.__init__(
            self, learners, auto_train=auto_train, force_train=force_train, **kwargs
        )
