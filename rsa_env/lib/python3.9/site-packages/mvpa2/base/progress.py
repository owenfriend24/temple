...
def eta_string(
    start_time, progress, msg=None, progress_bar_width=18, show_percentage=True
):
    ...
    eta = (
        took * (1 - progress) / progress if legal_progress else progress * float("inf")
    )
    pct_str = "[%.0f%%]" % (progress * 100.0)

    formatter = seconds2prettystring

    if progress_bar_width:
        n_done = int(math.floor(min(progress, 1.0) * progress_bar_width))
        ...
        if show_percentage:
            ...
            if n <= n_done and (n > n_todo or n_done >= n_todo):
                ...
                done = done[:offset] + pct_str + done[: (n_done - offset - n_pct)]
            elif n <= n_todo:
                offset = (n_todo - (n_pct + margin) // 2) // 2
                todo = todo[:offset] + pct_str + todo[: (n_todo - offset - n_pct)]
            else:
...
class ProgressBar(object):
    ...
    def __init__(self, start_time=None, progress_bar_width=18, show_percentage=True):
        """
