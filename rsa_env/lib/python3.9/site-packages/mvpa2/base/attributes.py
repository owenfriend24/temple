...
class IndexedCollectable(Collectable):
    ...
    * Parameter: attribute with validity ranges.
    ...
    def __reduce__(self):
        ...
        res = (cr[0], (self._instance_index,) + cr[1], {"_isset": self._isset})
    ...
    def _set(self, val, init=False):
        ...
        if __debug__:  # Since this call is quite often, don't convert
            ...
            debug(
                "COL",
                "%s %s to %s ",
                ({True: "Initializing", False: "Setting"}[init], self, val),
            )
        self._value = val
        self._isset = True
...
class ConditionalAttribute(IndexedCollectable):
    ...
    def __reduce__(self):
        ...
        icr[2].update({"_defaultenabled": self._defaultenabled, "_value": self._value})
    ...
    def _set(self, val, init=False):
        if self.__enabled:
            ...
            debug("COL", "Not setting disabled %s to %s ", (self, val))
    ...
    def _set_enabled(self, value=False):
        if self.__enabled == value:
            ...
            return
        if __debug__:
            debug("STV", "%s %s", ({True: "Enabling", False: "Disabling"}[value], self))
