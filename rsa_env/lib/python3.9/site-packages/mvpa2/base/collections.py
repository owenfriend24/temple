...
class SequenceCollectable(Collectable):
    ...
    def __init__(self, value=None, name=None, doc="Sequence attribute", length=None):
        """
        Parameters
        ----------
        value : arbitrary (see derived implementations)
        ...
        if value is not None and not hasattr(value, "__len__"):
            raise ValueError(
                "%s only takes sequences as value." % self.__class__.__name__
            )
    ...
    def _set(self, val):
        ...
        # desired at all
        if self._target_length is not None and len(val) != self._target_length:
            raise ValueError(
                "Value length [%i] does not match the required "
                "length [%i] of attribute '%s'."
                % (len(val), self._target_length, str(self.name))
            )
        self._reset_unique()
...
class ArrayCollectable(SequenceCollectable):
    ...
    def __copy__(self):
        ...
        copied = self.__class__(
            name=self.name, doc=self.__doc__, length=self._target_length
        )
        # just get a view of the old data!
    ...
    def _set(self, val):
        if not hasattr(val, "view"):
            if is_sequence_type(val):
                ...
                raise ValueError(
                    "%s only takes ndarrays (or array-likes "
...
class Collection(dict):
        ...
        if isinstance(source, list):
            for a in source:
                ...
                if copyvalues is None:
                    ...
                    raise ValueError("Unknown value ('%s') for copy argument." % copy)
            ...
            raise ValueError(
                "Collection.update() cannot handle '%s'." % str(type(source))
            )
    ...
    def __setattr__(self, key, value):
        try:
            ...
            errmsg = "parameter '%s' cannot accept value `%r` (%s)" % (
                key,
                value,
                str(e),
            )
    ...
    def __repr__(self):
        return "%s(items=%r)" % (self.__class__.__name__, list(self.values()))
...
class UniformLengthCollection(Collection):
    ...
    def __setitem__(self, key, value):
        ...
        if not isinstance(value, ArrayCollectable):
            ...
            if is_sequence_type(value) and len(value) == 1 and ulength is not None:
                if ulength > 1:
        ...
        if ulength is None:
            ...
            raise ValueError(
                ...
                % (key, len(value.value), ulength, str(self))
            )
    ...
    def _compare_to_value(a, v, strict=True):
        ...
        if isinstance(r, bool):
            ...
            assert r is False
            raise ValueError("%r is not comparable to items among %s" % (v, a))
        ...
        if a.ndim > 1:
            ...
            if vshape != a.shape[1:]:
                raise ValueError(
                    "Value %r you are looking for is of %s "
        ...
        if not np.any(r):
            if strict:
                raise ValueError("None of the items matched %r among %s" % (v, a))
            else:
    ...
    attr_length = property(
        fget=lambda self: self._uniform_length,
