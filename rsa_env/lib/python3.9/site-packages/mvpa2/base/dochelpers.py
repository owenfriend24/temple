...
def _parse_parameters(paramdoc):
    ...
    if __debug__:
        debug(
            "DOCH", 'parseParameters: Given "%s", we split into %s' % (paramdoc, result)
        )
    return result
...
def enhanced_doc_string(item, *args, **kwargs):
    ...
    *args : list
    ...
    if len(kwargs):
        if set(kwargs.keys()).issubset({"force_extend"}):
            raise ValueError(
                "Got unknown keyword arguments (smth among %s)"
    ...
    if "__init__" in lcl:
        ...
        extend_args = force_extend or "kwargs" in (
            func.__code__.co_names + func.__code__.co_varnames
        )
        ...
        if "_paramsdoc" in lcl:
            params_list += [i for i in lcl["_paramsdoc"] if not (i[0] in known_params)]
            known_params = set([i[0] for i in params_list])
    ...
    if __add_init2doc and initdoc != "":
        docs += [
            _rst_underline(
                "Constructor information for `%s` class" % name, rst_lvlmarkup[2]
            ),
            initdoc,
        ]
    ...
    if False:  # len(args):
        bc_intro = _rst(
            "  "
        ) + "Please refer to the documentation of the " "base %s for more information:" % (
        ...
        docs += [
            ...
            + ",\n  ".join(
                [
                    "%s%s.%s%s%s"
                    % (_rst(":class:`~"), i.__module__, i.__name__, _rst("`"), _rst_sep)
                    for i in args
                ]
            ),
        ]
...
def table2string(table, out=None):
    ...
    col_width = [
        max([len(markup_strip.sub("", x)) for x in column]) for column in atable.T
    ]
    string = ""
    for i, table_ in enumerate(table):
        string_ = ""
...
def _repr(obj, *args, **kwargs):
    ...
    *args, **kwargs : str
    ...
    if max_length < 0:
        max_length = 0
    auto_repr = ", ".join(list(args) + ["%s=%s" % (k, v) for k, v in list(kwargs.items())])
...
def _str(obj, *args, **kwargs):
    ...
    *args, **kwargs : str
    ...
    if s is None:
        ...
        auto_descr = ", ".join(
            list(args) + ["%s=%s" % (k, v) for k, v in list(kwargs.items())]
        )
        if len(auto_descr):
            s = s + ": " + auto_descr
