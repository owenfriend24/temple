...
class _SingletonType(type):
    ...
    def __call__(cls, sid, instance, *args):
        if not sid in cls._instances:
            cls._instances[sid] = instance
...
verbose = __Singleton(
    "verbose",
    LevelLogger(handlers=cfg.get("verbose", "output", default="stdout").split(",")),
)
...
if __debug__:
    ...
    debug = __Singleton(
        "debug",
        DebugLogger(handlers=cfg.get("debug", "output", default="stdout").split(",")),
    )
    ...
    # CHECKs
    debug.register(
        "CHECK_DS_SELECT", "Check in dataset.select() for sorted and unique indexes"
    )
    debug.register("CHECK_DS_SORTED", "Check in datasets for sorted")
    debug.register("CHECK_IDS_SORTED", "Check for ids being sorted in mappers")
    debug.register(
        "CHECK_TRAINED", "Checking in checking if clf was trained on given dataset"
    )
    ...
    debug.register(
        "SG_GC", "For all entities enable highest level" " (garbage collector)"
    )
    debug.register(
        "SG_LINENO", "Enable printing of the file:lineno" " where SG_ERROR occurred."
    )
    ...
    debug.register(
        "ATTRREFER",
        "Debugging of top-level attribute referencing, "
