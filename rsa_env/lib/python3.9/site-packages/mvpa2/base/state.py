...
class Collection(BaseCollection):
    ...
    def _cls_repr(self):
        ...
        raise NotImplementedError(
            "Class %s should override _cls_repr" % self.__class__.__name__
        )
    ...
    def reset(self, key=None):
        ...
        if len(self):
            for key in keys:
                ...
                self._action(
                    key, list(self.values())[0].__class__.reset, missingok=False
                )
    ...
    def listing(self):
        ...
        items_.sort()
        return [
            "%s%s%s: %s" % (_def_sep, str(x[1]), _def_sep, x[1].__doc__) for x in items_
        ]
...
class ConditionalAttributesCollection(Collection):
    ...
    def _cls_repr(self):
        ...
        for name, invert in (("enable", False), ("disable", True)):
            ca = self._get_enabled(nondefault=False, invert=invert)
            if len(ca):
    ...
    def _initialize(self, key, value):
        ...
        if key == "enable_ca":
            ...
            raise ValueError(
                "ConditionalAttributesCollection can accept only enable_ca "
    ...
    def _copy_ca_(self, fromstate, key=None, deep=False):
        ...
        operation = {True: copy.deepcopy, False: copy.copy}[deep]
    ...
    def enable(self, key, value=True, missingok=False):
        ...
        self._action(
            key, ConditionalAttribute._set_enabled, missingok=missingok, value=value
        )

    def disable(self, key):
        """Disable conditional attribute defined by `key` id"""
        self._action(
            key, ConditionalAttribute._set_enabled, missingok=False, value=False
        )
    ...
    def change_temporarily(self, enable_ca=None, disable_ca=None, other=None):
        ...
        if other is not None:
            ...
            add_enable_ca = list(
                set(other.enabled)
                .difference(set(enable_ca))
                .intersection(list(self.keys()))
            )
    ...
    def reset_changed_temporarily(self):
        ...
        if len(self.enabled) > 0:
            ...
            raise ValueError("Trying to restore not-stored list of enabled " "ca")
    ...
    def _get_enabled(self, nondefault=True, invert=False):
        ...
        if nondefault:
            ...
            ffunc = lambda y: fmatch(y) and self[y]._defaultenabled != self.is_enabled(
                y
            )
...
class AttributesCollector(type):
    ...
    def __init__(cls, name, bases, dict):
        ...
        for base in bases:
            if hasattr(base, "__class__") and base.__class__ == AttributesCollector:
                ...
                for col, super_collection in list(newcollections.items()):
                    if col in collections:
                        if __debug__:
                            debug(
                                "COLR",
                                "Updating existing collection %s with the one from super class"
                                % col,
                            )
                        collection = collections[col]
                        # Current class could have overriden a parameter, so
                        # we need to keep it without updating
                        for pname, pval in list(super_collection.items()):
                            if pname not in collection:
                                collection[pname] = pval
                            elif __debug__:
                                debug(
                                    "COLR",
                                    "Not overriding %s.%s of cls %s from base %s"
                                    % (col, pname, cls, base),
                                )
                    else:
        ...
        if hasattr(cls, "_ATTRIBUTE_COLLECTIONS"):
            for col in cls._ATTRIBUTE_COLLECTIONS:
                if not col in _col2class:
                    raise ValueError(
                        "Requested collection %s is unknown to collector" % col
                    )
        ...
        for col, colitems in list(collections.items()):
        ...
        textwrapper = TextWrapper(
            subsequent_indent="    ", initial_indent="    ", width=70
        )
        ...
        for col in ("params", "kernel_params"):
            if col in collections:
                ...
                iparams = [(v._instance_index, k) for k, v in list(col_items.items())]
                iparams.sort()
                paramsdoc += [
                    (col_items[iparam[1]].name, col_items[iparam[1]]._paramdoc())
        ...
        if "ca" in collections:
            ...
            if len(collections["ca"]):
                cadoc += "\n".join(["* " + x for x in collections["ca"].listing])
                cadoc += (
...
class ClassWithCollections(object, metaclass=AttributesCollector):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if "_collections" not in s__dict__:
            ...
            for col, collection in list(collections.items()):
                if col in s__dict__:
                    raise ValueError("Object %s has already attribute %s" % (self, col))
        ...
        if __debug__:
            ...
            debug(
                "COL",
                "ClassWithCollections.__new__ was done " "for %s%s with descr=%s",
    ...
    def __init__(self, descr=None, **kwargs):
        ...
        if __debug__:
            debug(
                "COL",
                "ClassWithCollections.__init__ was done " "for %s%s with descr=%s",
    ...
    if __debug__ and _debug_references:
        ...
        def __debug_references_call(self, method, key):
            ...
            if key in known_attribs:
                ...
                if (
                    "mvpa2.datasets" in clsstr
                    and "Dataset" in clsstr
                    and (key in ["targets", "chunks", "samples", "mapper"])
                ):
                    return
    ...
    def __str__(self):
        ...
        if hasattr(self, "_collections"):
            for col, collection in list(self._collections.items()):
    ...
    descr = property(lambda self: self.__descr, doc="Description of the object if any")
