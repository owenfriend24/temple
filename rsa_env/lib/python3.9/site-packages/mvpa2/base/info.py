...
class WTF(object):
    ...
    __knownitems__ = {"process", "runtime", "externals", "system", "sources"}
    ...
    def _acquire_system(self, out):
        ...
        out.write(
            " OS:            %s\n"
            % " ".join([os.name, pl.system(), pl.release(), pl.version()]).rstrip()
        )
        out.write(
            " Distribution:  %s\n"
            % " ".join(
                [_t2s(pl.dist()), _t2s(pl.mac_ver()), _t2s(pl.win32_ver())]
            ).rstrip()
        )
    ...
    def _acquire_externals(self, out):
        ...
        try:
            ...
            out.write(
                ' Failed to determine backend of matplotlib due to "%s"' % str(exc)
            )
