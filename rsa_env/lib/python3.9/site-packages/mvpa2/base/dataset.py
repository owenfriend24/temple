...
if not __REPR_STYLE__ in ("full", "str"):
    raise ValueError(
        "Incorrect value %r for option datasets.repr."
    ...
    *not* always congruent to array slicing.
    ...
    def __init__(self, samples, sa=None, fa=None, a=None):
        ...
        if not len(samples.shape):
            raise ValueError(
                "Only `samples` with at least one axis are "
    ...
    def init_origids(self, which, attr="origids", mode="new"):
        ...
        if not mode in legal_modes:
            raise ValueError("Incorrect mode %r. Known are %s." % (mode, legal_modes))
        if which in ("samples", "both"):
            ...
            ids = np.array(
                ["%s-%i" % (thisid, i) for i in range(self.samples.shape[0])]
            )
            if attr in self.sa:
        ...
        if which in ("features", "both"):
            ...
            ids = np.array(
                ["%s-%i" % (thisid, i) for i in range(self.samples.shape[1])]
            )
            if attr in self.fa:
    ...
    def __reduce__(self):
        return (
            self.__class__,
            (self.samples, dict(self.sa), dict(self.fa), dict(self.a)),
        )
    ...
    def copy(self, deep=True, sa=None, fa=None, a=None, memo=None):
        ...
        """
        if __debug__:
            debug("DS_", "Duplicating samples shaped %s" % str(self.samples.shape))
        if deep:
        ...
        if __debug__:
            debug(
                "DS_",
                "Return dataset copy %s of source %s" % (_strid(out), _strid(self)),
            )
        return out
    ...
    def append(self, other):
        ...
        if not self.nfeatures == other.nfeatures:
            raise DatasetError(
                "Cannot merge datasets, because the number of "
        ...
        for k, v in list(other.sa.items()):
    ...
    def from_hdf5(cls, source, name=None):
        ...
        if name is not None:
            ...
            if not name in hdf:
                raise ValueError(
                    "Cannot find '%s' group in HDF file %s.  "
                    "File contains groups: %s" % (name, source, list(hdf.keys()))
                )
            ...
            if not isinstance(res, AttrDataset):
                ...
                raise ValueError(
                    "%r in %s contains %s not a dataset.  "
            ...
            if not isinstance(res, AttrDataset):
                # TODO: unittest before committing
                raise ValueError(
                    "Failed to load a dataset from %s.  "
                    "Loaded %s instead." % (source, type(res))
                )
        if own_file:
            hdf.close()
        return res
    ...
    def from_npz(cls, filename):
        ...
        if (
            not lexists(filename)
            and not filename.endswith(".npz")
            and lexists(filename_npz)
        ):
        ...
        if skipped:
            warning(
                "Skipped following items since do not belong to any of "
...
def datasetmethod(func):
    ...
    if __debug__:
        debug("DS_", "Binding function %s to AttrDataset class" % func.__name__)
...
def vstack(datasets, a=None, fa="drop_nonunique"):
    ...
    if __debug__:
        ...
        if not np.all([sorted(ds.sa.keys()) == target for ds in datasets]):
            raise ValueError(
                "Sample attributes collections of to be stacked "
...
def hstack(datasets, a=None, sa="drop_nonunique"):
    ...
    if __debug__:
        ...
        if not np.all([sorted(ds.fa.keys()) == target for ds in datasets]):
            raise ValueError(
                "Feature attributes collections of to be stacked "
                "datasets have varying attributes."
            )
...
def _stack_add_equal_attributes(merged, datasets, strategy, colname):
    ...
    if strategy == "update":
        ...
        for ds in datasets[1:]:
            ...
            for attr, v in list(dscol.items()):
                if (attr not in ds0col) or np.any(ds0col[attr].value != v.value):
                    drop.add(attr)
        ...
        for ds in datasets:
            mergedcol.update(
                {
                    attr: v
                    for attr, v in list(getattr(ds, colname).items())
                    if attr not in drop
                }
            )
        ...
        raise ValueError(
            "Unknown strategy %s on how to deal with %s collection"
            % (strategy, colname)
        )
...
def _stack_add_equal_dataset_attributes(merged_dataset, datasets, a=None):
    ...
    if not a in allowed_values:
        raise ValueError("a should be an int or one of " "%r" % allowed_values)
        ...
        for i, dataset in enumerate(datasets):
            ...
            if a in ("drop_nonunique", "unique"):
                if not values:
                    ...
                    if a == "unique":
                        raise DatasetError(
                            "Not unique dataset attribute value "
                            " for %s: %s and %s" % (key, values[0], value)
                        )
                    else:
                        add_key = False
                        break
    ...
    if raise_unequal_count:
        ...
        for i, h in enumerate(hs):
            if i == 0:
                ...
                raise ValueError(
                    "Value mismatch between input 0 and %d:" " %s != %s" % (i, h, h0)
                )
    return unq, masks
...
def split_by_sample_attribute(ds, sa_label, raise_unequal_count=True):
    ...
    if type(sa_label) in (list, tuple):
        ...
        if len(sa_label) == 1:
            ...
            return sum(
                [
                    split_by_sample_attribute(sa, sa_label[1:], raise_unequal_count)
                    for sa in sas
                ],
                [],
            )
    ...
    _, masks = _get_unique_attribute_masks(
        ds.sa[sa_label].value, raise_unequal_count=raise_unequal_count
    )
...
def split_by_feature_attribute(ds, fa_label, raise_unequal_count=True):
    ...
    if type(fa_label) in (list, tuple):
        ...
        if len(fa_label) == 1:
            ...
            return sum(
                [
                    split_by_feature_attribute(fa, fa_label[1:], raise_unequal_count)
                    for fa in fas
                ],
                [],
            )
    ...
    _, masks = _get_unique_attribute_masks(
        ds.fa[fa_label].value, raise_unequal_count=raise_unequal_count
    )
...
def save(dataset, destination, name=None, compression=None):
    ...
    # look if we got an hdf file instance already
    if isinstance(destination, h5py.File):
        own_file = False
        hdf = destination
    else:
        own_file = True
