    ...
    if isinstance(hdf, h5py.Dataset):
        ...
        if __debug__:
            if "class" in hdf.attrs:
                debug("HDF5", "Found class info %s.%s" % (mod_name, hdf.attrs["class"]))
            else:
                debug("HDF5", "Found recon info %s.%s" % (mod_name, hdf.attrs["recon"]))
        ...
        if "recon" in hdf.attrs:
            ...
            if cls_name == "NoneType":
                ...
                raise RuntimeError(
                    "Unhandled reconstruction of built-in "
                ...
                raise RuntimeError(
                    ...
                    "is a conceptual bug in the parser. Please report." % hdf.name
                )
    #
...
def _recon_functype(hdf):
    ...
    if __debug__:
        debug("HDF5", "Load '%s.%s.%s' [%s]" % (mod_name, cls_name, ft_name, hdf.name))
...
def _recon_customobj_customrecon(hdf, memo):
    ...
    if __debug__:
        debug(
            "HDF5",
            "Load from custom reconstructor '%s.%s' [%s]"
    ...
    if "rcargs" in hdf:
        ...
        if __debug__:
            debug("HDF5", "Load reconstructor args in [%s]" % recon_args_hdf.name)
        if "objref" in hdf.attrs:
...
def _recon_customobj_defaultrecon(hdf, memo):
    ...
    if __debug__:
        debug(
            "HDF5",
            "Load class instance '%s.%s' instance [%s]"
...
def _hdf_dict_to_obj(hdf, memo, skip=None):
    ...
    if items_container.attrs.get("__keys_in_tuple__", 0):
        ...
        return dict(
            [
                (item, hdf2obj(items_container[item], memo=memo))
...
def _hdf_list_to_obj(hdf, memo, target_container=None):
    ...
    if "length" in hdf.attrs:
        ...
        if __debug__:
            debug("HDF5", "Found explicit sequence length setting (%i)" % length)
        hdf_items = hdf["items"]
    elif "items" in hdf:
        # not so legacy file, at least has an items container
        length = len(hdf["items"])
        if __debug__:
            debug("HDF5", "No explicit sequence length setting (guess: %i)" % length)
        hdf_items = hdf["items"]
    # legacy compat code
    else:
        length = len(hdf)
        if __debug__:
            debug("HDF5", "Ancient file, guessing sequence length (%i)" % length)
    ...
    if "objref" in hdf.attrs:
        ...
        if target_container is None:
            if __debug__:
                debug(
                    "HDF5",
                    "Track sequence with %i elements under objref '%s'"
                    % (length, objref),
                )
            memo[objref] = items
        else:
            if __debug__:
                debug(
                    "HDF5", "Track provided target_container under objref '%s'", objref
                )
    ...
    for i in range(length):
        ...
        if not got_obj:
            ...
            if objref is None:
                ...
                if objref in memo:
                    if __debug__:
                        debug("HDF5", "Use tracked object (%i)" % objref)
                    items[i] = memo[objref]
                else:
...
def _hdf_to_ndarray(hdf):
    ...
    if "is_a_view" in hdf.attrs:
        ...
        if "dtype_names" in hdf.attrs:
            ...
            dtypes = [x for x in hdf.attrs["dtype_types"]]
    ...
    **kwargs
    ...
    if is_scalar or is_ndarray:
        ...
        if __debug__:
            debug(
                "HDF5",
                "Store '%s' (ref: %i) in [%s/%s]" % (type(obj), obj_id, hdf.name, name),
            )
        # the real action is here
        if "compression" in kwargs and (
            is_scalar or (is_ndarray and not len(obj.shape))
        ):
            ...
            kwargs = dict([(k, v) for (k, v) in list(kwargs.items()) if k != "compression"])
        ...
        if is_a_view:
            ...
            if not (
                (obj.flags.c_contiguous or obj.flags.f_contiguous) and obj.flags.aligned
            ):
    ...
    if name is not None:
        ...
        if __debug__:
            debug(
                "HDF5", "Store '%s' (ref: %i) in [%s]" % (type(obj), obj_id, hdf.name)
            )
        grp = hdf
    ...
    if not hasattr(obj, "__reduce__"):
        raise HDF5ConversionError(
            "Cannot store class without __reduce__ " "implementation (%s)" % type(obj)
        )
    ...
    try:
        ...
        if __debug__:
            debug(
                "HDF5", "Reduced '%s' (ref: %i) in [%s]" % (type(obj), obj_id, hdf.name)
            )
    ...
    if pieces is None or pieces[0].__name__.endswith("_reconstructor"):
        ...
        if src_module != builtins_mod:
            if hasattr(obj, "__name__"):
                if obj.__name__ not in dir(
                    __import__(src_module, fromlist=[obj.__name__])
                ):
                    raise HDF5ConversionError(
                        "Cannot store locally defined " "function '%s'" % cls_name
                    )
            else:
                mod_content = dir(__import__(src_module, fromlist=[cls_name]))
                if cls_name not in mod_content:
                    ...
                    for stuff in mod_content:
                        ...
                        if isinstance(stuffobj, type) and stuffobj.__name__ == cls_name:
                            cls_name = stuff
                            found = True
                    if not found:
                        raise HDF5ConversionError(
                            "Cannot store locally defined " "class '%s'" % cls_name
                        )
        # store class info (fully-qualified)
        grp.attrs.create("class", cls_name.encode())
        grp.attrs.create("module", src_module.encode())
        if __debug__:
            debug("HDF5", "Stored class info: %s.%s" % (src_module, cls_name))
        ...
        if isinstance(obj, (list, tuple)):
            ...
            _seqitems_to_hdf(
                list(zip(list(obj.keys()), list(obj.values()))),
                grp,
                memo,
                noid=True,
                **kwargs
        ...
        if __debug__:
            debug(
                "HDF5",
                "Use custom __reduce__ for storage: (%i arguments)." % len(pieces[1]),
            )
    ...
    if pieces is not None and len(pieces) > 2:
        ...
        obj2hdf(grp, state, name="state", memo=memo, noid=True, **kwargs)
...
def h5load(filename, name=None):
    ...
    try:
        if name is not None:
            if not name in hdf:
                raise ValueError(
                    "No object of name '%s' in file '%s'." % (name, filename)
                )
            ...
            if not len(hdf) and not len(hdf.attrs):
                ...
                if isinstance(hdf, h5py.Dataset) or (
                    "class" in hdf.attrs or "recon" in hdf.attrs
                ):
