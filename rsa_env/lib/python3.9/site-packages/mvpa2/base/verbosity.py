    ...
    def _set_handlers(self, handlers):
        ...
        for handler in handlers:
            if isinstance(handler, str):
                try:
                    handler = {"stdout": stdout, "stderr": stderr}[handler.lower()]
                except:
                    try:
                        ...
                        raise RuntimeError(
                            "Cannot open file %s for writing by the logger" % handler
                        )
    ...
    def _set_level(self, level):
        ...
        if __debug__:
            try:
                ...
                debug(
                    "VERBOSE", "Setting verbosity to %r from %r", (self.__level, level)
                )
            except:
                pass
        ilevel = int(level)
        if ilevel < 0:
            raise ValueError(
                "Negative verbosity levels (got %d) are not supported" % ilevel
            )
        self.__level = ilevel
...
class SetLogger(Logger):
    ...
    def __init__(self, register=None, active=None, printsetid=True, *args, **kwargs):
        """
        Parameters
        ----------
        ...
        for item in list(set(active)):
            ...
            if isinstance(item, str):
                ...
                if len(toactivate) == 0:
                    ...
                    raise ValueError(
                        ...
                        " among known ids: %s" % (item, regexp_str, ids)
                    )
            else:
                toactivate = [item]
            ...
            for item_ in toactivate:
                if not (item_ in registered_keys):
                    raise ValueError(
                        "Unknown debug ID %s was asked to become active" % item_
                    )
    ...
    printsetid = property(fget=lambda self: self.__printsetid, fset=_set_printsetid)
...
if __debug__:
    ...
    def get_vmem_from_status():
        ...
        """
        rss, vms = [parse_status(field=x, value_only=True) for x in ["VmRSS", "VmSize"]]
    ...
    try:
        ...
        __pymvpa_memory_info = (
            __pymvpa_process__.memory_info
            if hasattr(__pymvpa_process__, "memory_info")
    ...
    class DebugLogger(SetLogger):
            ...
            if isinstance(func, str):
                if func in DebugLogger._known_metrics:
                    ...
                    if func in ["?", "list", "help"]:
                        ...
                        raise ValueError(
                            "Unknown name %s for metric in DebugLogger" % func
                            + " Known metrics are "
        ...
        def __call__(self, setid, msg, *args, **kwargs):
            ...
            if len(msg) > 0:
                ...
                if (
                    len(msg) > 250
                    and "DBG" in self.active
                    and not setid.endswith("_TB")
                ):
        ...
        offsetbydepth = property(
            fget=lambda x: x._offsetbydepth, fset=_set_offset_by_depth
        )

        metrics = property(fget=lambda x: x.__metrics, fset=register_metric)
