...
class Node(ClassWithCollections):
                ...
                if a in ca_keys:
                    ...
                    if rcol is None:
                        ...
                        if col_class is SampleAttributesCollection:
                            ...
                            raise ValueError(
                                "Cannot determine origin of %s collection" % col
                            )
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(Node, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["space", "pass_attr", "postproc"])
        )

    space = property(get_space, set_space, doc="Processing space name of this node")

    pass_attr = property(
        lambda self: self.__pass_attr,
        doc="Which attributes of the dataset or self.ca "
        "to pass into result dataset upon call",
    )

    postproc = property(
        get_postproc, set_postproc, doc="Node to perform post-processing of results"
    )
...
class CompoundNode(Node):
    ...
    def __init__(self, nodes, **kwargs):
        ...
        if not len(nodes):
            raise ValueError(
                "%s needs at least one embedded node." % self.__class__.__name__
            )
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(CompoundNode, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["nodes"])
        )
...
class CombinedNode(CompoundNode):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(CombinedNode, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["combine_axis", "a"])
        )
