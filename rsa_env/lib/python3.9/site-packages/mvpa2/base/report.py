    ...
    def figure(self, fig=None, name=None, savefig_kwargs=None, **kwargs):
        ...
        **kwargs
        ...
        if isinstance(fig, figure.Figure):
            ...
            if not (os.path.exists(self._filename) and os.path.isdir(self._filename)):
            ...
            # Save image
            fig_filename = pathjoin(self._filename, "%s.%s" % (name, self.fig_ext))
            if __debug__ and not self in debug.handlers:
                debug("REP_", "Saving figure '%s' into %s" % (fig, fig_filename))
    ...
    def save(self, add_preamble=True):
        ...
        def my_first_page(canvas, doc):
            ...
            canvas.drawCentredString(
                self.pagesize[0] / 2.0, self.pagesize[1] - 108, title
            )
            canvas.setFont(self.font, 9)
            canvas.drawString(rplu.inch, 0.75 * rplu.inch, "First Page / %s" % pageinfo)
            canvas.restoreState()
        ...
        def my_later_pages(canvas, doc):
            ...
            canvas.setFont(self.font, 9)
            canvas.drawString(
                rplu.inch, 0.75 * rplu.inch, "Page %d %s" % (doc.page, pageinfo)
            )
            canvas.restoreState()
