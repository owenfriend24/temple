...
def _hdr2dict(hdr):
    ...
    if scl_slope == scl_inter == "nan":
        warning(
            "Detected incorrect (nan) scl_ fields. Resetting to "
...
def _img2data(src):
    ...
    if isinstance(img, nibabel.spatialimages.SpatialImage):
        ...
        if len(img.shape) == 5 and img.shape[3] == 1:
            ...
            warning(
                "dataset with 5th dimension found but 4th is empty (AFNI "
...
def map2nifti(dataset, data=None, imghdr=None, imgtype=None):
    ...
    if imgtype is None:
        if "imgtype" in dataset.a:
            ...
            if __debug__:
                debug(
                    "DS_NIFTI",
                    "No image type found in %s. Using default Nifti1Image." % dataset.a,
                )
    ...
    if issubclass(imgtype, nibabel.spatialimages.SpatialImage) and (
        imghdr is None or hasattr(imghdr, "get_data_dtype")
    ):
...
def fmri_dataset(
    ...
    if tprefix is not None:
        ...
        ds.sa[tprefix + "_coords"] = np.arange(len(ds), dtype="float") * _get_dt(imghdr)
        # TODO extend with the unit
...
def _load_anyimg(src, ensure=False, enforce_dim=None):
    ...
    # first
    if (isinstance(src, list) or isinstance(src, tuple)) and len(src) > 0:
        # load from a list of given entries
        srcs = [_load_anyimg(s, ensure=ensure, enforce_dim=enforce_dim) for s in src]
        if __debug__:
            ...
            if not np.all([s == shapes[0] for s in shapes]):
                raise ValueError("Input volumes vary in their shapes: %s" % (shapes,))
    ...
    if imgdata is not None and enforce_dim is not None:
        ...
        if lshape < enforce_dim:
            ...
            if shape[:bogus_dims] != (1,) * bogus_dims:
                raise ValueError(
                    "Cannot enforce %dD on data with shape %s" % (enforce_dim, shape)
                )
        ...
        if new_shape is not None:
            if __debug__:
                debug(
                    "DS_NIFTI",
                    "Enforcing shape %s for %s data from %s" % (new_shape, shape, src),
                )
