...
def assign_conditionlabels(
    ds, events, label_attr="targets", time_attr="time_coords", **kwargs
):
...
def events2sample_attr(
    events, time_coords, noinfolabel=None, onset_shift=0.0, condition_attr="condition"
):
...
def find_events(**kwargs):
    ...
    **kwargs : sequences
    ...
    for r in range(len(list(kwargs.values())[0])):
        ...
        combo = dict([(k, v[r]) for k, v in list(kwargs.items())])
...
def _events2dict(events):
    ...
    for k in events[0]:
        try:
            ...
            raise ValueError(
                "Each event property must be present for all "
...
def extract_boxcar_event_samples(
    ...
    if __debug__:
        if not max(evvars["duration"]) == min(evvars["duration"]):
            warning(
                "Boxcar mapper will use maximum boxlength (%i) of all "
...
def fit_event_hrf_model(
    ...
    for event in events:
        ...
        compound_label = event[glm_condition_attr] = "glm_label_" + "+".join(
    ...
    glm_regs = [
        (reg, design_matrix.matrix[:, i]) for i, reg in enumerate(design_matrix.names)
    ]
    ...
    * :func:`~mvpa2.datasets.eventrelated.assign_conditionlabels`.
    ...
    if model == "boxcar":
        ...
        if condition_attr is None:
            raise ValueError("missing name of event attribute with condition names")
        if time_attr is None:
            raise ValueError("missing name of attribute with sample timing information")
...
def get_contrasts(
    hrf_estimates, contrasts=None, condition_attr="targets", fxname="z_score"
):
    ...
    if contrasts is None:
        contrasts = OrderedDict((c, {c: 1}) for c in conditions)

    out = Dataset(np.empty((len(contrasts), hrf_estimates.nfeatures)))
    ...
    for i, (contrast, condition_weights) in enumerate(contrasts.items()):
        ...
        for condition, weight in list(condition_weights.items()):
