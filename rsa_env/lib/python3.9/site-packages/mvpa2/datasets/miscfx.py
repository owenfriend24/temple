...
def coarsen_chunks(source, nchunks=4):
    ...
    for i, c in enumerate(chunks_unique):
        ...
        if (
            samples_counted + cur_chunk_nsamples >= (nchunks + 1) * avg_chunk_size
        ) or i == nchunks_orig - 1:
    ...
    if len(chunks_groups) != nchunks:
        warning(
            "Apparently logic in coarseChunks is wrong. "
            "It was desired to get %d chunks, got %d" % (nchunks, len(chunks_groups))
        )
    ...
    if __debug__:
        debug(
            "DS_",
            "Using dictionary %s to remap old chunks %s into new %s"
    ...
    if isinstance(source, Dataset):
        if __debug__:
            debug(
                "DS",
                "Coarsing %d chunks into %d chunks for %s"
...
##       operate on)
def get_samples_per_chunk_target(dataset, targets_attr="targets", chunks_attr="chunks"):
...
def get_samples_by_attr(dataset, attr, values, sort=True):
    ...
    if not is_sequence_type(values) or isinstance(values, str):
        values = [values]
    ...
    sa = dataset.sa
    for value in values:
        sel = np.concatenate((sel, np.where(sa[attr].value == value)[0]))
    ...
    if lstats:
        try:
            s += dataset.summary_targets(
                targets_attr=targets_attr, chunks_attr=chunks_attr, maxc=maxc, maxt=maxt
            )
...
@datasetmethod
def summary_targets(
    dataset, targets_attr="targets", chunks_attr="chunks", maxc=30, maxt=20
):
    ...
    def cl_stats(axis, u, name1, name2):
        ...
        for i, l in enumerate(u):
            ...
            table.append(
                [
                    ("%.3g", "%s")[isinstance(d[e], str) or d[e] is None] % d[e]
                    for e in entries
                ]
            )
...
class SequenceStats(dict):
    ...
    def _compute(self):
        ...
        sout = (
            "Sequence statistics for %d entries from set %s\n" % (len(seq), utargets)
...
def to_df(ds, sas=None, fas=None):
    ...
    def col_to_index(col, col_abbrev, names):
        ...
        return pd.MultiIndex.from_tuples(
            ...
            list(zip(*[list(map(str, col[a].value)) for a in attrs])),
            names=["%s.%s" % (col_abbrev, fa) for fa in attrs],
        )

    df = pd.DataFrame(
        np.asanyarray(ds.samples), columns=col_to_index(ds.fa, "fa", names=fas)
    )
