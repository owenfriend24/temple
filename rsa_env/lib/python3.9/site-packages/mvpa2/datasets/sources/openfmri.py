...
class OpenFMRIDataset(object):
    ...
    def get_bold_run_ids(self, subj, task):
        ...
        return _subdirs2ids(
            _opj(self.basedir, _sub2id(subj), "BOLD"),
            "%s_" % (task_prefix,),
    ...
    def _load_bold_task_run_data(self, subj, task, run, path, loadfx):
        ...
        return self._load_subj_data(subj, ["BOLD", _taskrun(task, run)] + path, loadfx)
        ...
        if len(zooms) > 3:
            ...
            if tr == 1.0:
                ...
                if prop_tr != tr:
                    ...
                    warning(
                        "Dataset file has time dimension set to 1.0 whenever "
    ...
    def get_bold_run_motion_estimates(self, subj, task, run, fname="bold_moest.txt"):
        ...
        array
          ...
          rotation in deg)
        """
        return self._load_bold_task_run_data(subj, task, run, [fname], np.loadtxt)
        ...
        for run in range(nruns):
            ...
            for subj in sorted(tbri.keys()):
                ...
                try:
                    ...
                    d = self._load_bold_task_run_data(
                        subj, task, run + 1, [fname], loadfx
                    )
                    if data[run] is None:
                        data[run] = {subj: d}
                    else:
            ...
            for subj in run_data:
                ...
                fixed_run = np.empty((max_vol, 6), dtype=float)
                fixed_run[:] = np.nan
                if len(d):
                    fixed_run[: len(d)] = d
    ...
    def get_bold_run_dataset(
        self, subj, task, run, flavor=None, preproc_img=None, add_sa=None, **kwargs
    ):
        ...
        **kwargs:
        ...
        if flavor is not None:
            path = _opj(
                self.basedir, _sub2id(subj), "BOLD", _taskrun(task, run), flavor
            )
        ...
        for sa in add_sa:
            ...
            # nsamples
            attrs = self._load_bold_task_run_data(subj, task, run, [sa], np.loadtxt)
    ...
    def get_model_conditions(self, model):
        ...
        def_data = self._load_data(
            ["models", _model2id(model), "condition_key.txt"], open
        )
        conds = []
    ...
    def get_model_contrasts(self, model):
        ...
        try:
            def_data = self._load_data(
                ["models", _model2id(model), "task_contrasts.txt"], open
            )
        except IOError:
            return props
    ...
    def get_bold_run_model(self, model, subj, run):
        ...
        for cond in conditions:
            ...
            try:
                ...
                warning(
                    "onset definition file not found; no information "
                    "about condition '%s' for run %i" % (cond["name"], run)
                )
                continue
            for i, ev in enumerate(evdata):
                evdict = dict(list(zip(ev_fields, [ev[field] for field in ev_fields])))
                evdict["task"] = task_id
    ...
    def get_model_bold_dataset(
        ...
        **kwargs
        ...
        for sub in subj_id:
            ...
            for task in tasks:
                run_ids_ = (
                    run_ids if run_ids is not None else self.get_bold_run_ids(sub, task)
                )
    ...
    def get_anatomy_image(self, subj, path=None, fname="highres001.nii.gz"):
        ...
        if path is None:
            path = []
        return self._load_subj_data(subj, ["anatomy"] + path + [fname], nb.load)
