...
def load_example_fmri_dataset(name="1slice", literal=False):
    ...
    mask = {
        "1slice": pathjoin(pymvpa_dataroot, "mask.nii.gz"),
        "25mm": pathjoin(basedir, "sub001", "masks", "25mm", "brain.nii.gz"),
    }[name]
    ...
    if literal:
        ...
        ds = openfmri.get_model_bold_dataset(
            model, subj, flavor=name, mask=mask, noinfolabel="rest"
        )
        ...
        if name == "25mm":
            raise ValueError(
                "The 25mm dataset is no longer available with " "numerical labels"
            )
...
def load_tutorial_data(path=None, roi="brain", add_fa=None, flavor=None):
    ...
    if path is None:
        if flavor in ("1slice", "25mm"):
            ...
            path = mvpa2.cfg.get("location", "tutorial data", default=os.path.curdir)
    ...
    ds = dhandle.get_model_bold_dataset(
        model, subj, flavor=flavor, mask=mask, add_fa=add_fa, noinfolabel="rest"
    )
