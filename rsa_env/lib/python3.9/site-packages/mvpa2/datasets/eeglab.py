...
def eeglab_dataset(samples):
    ...
    if _looks_like_filename(samples):
        if not os.path.exists(samples):
            raise ValueError(
                "Input looks like a filename, but file" " %s does not exist" % samples
            )
    ...
    for i, line in enumerate(lines):
        ...
        if i == 0:
            ...
            if len(eeg) != n_channels:
                raise ValueError(
                    "Line %d: expected %d values but found %d" % (n_channels, len(eeg))
                )
    ...
    if len(n_timepoints_unique) != 1:
        raise ValueError(
            "Different number of time points in different"
            "samples: found %d different lengths" % len(n_timepoints_unique)
        )
    ...
    timepoint_array = np.asarray([samples[0][i][0] for i in range(n_timepoints)])
    ...
    for i, sample in enumerate(samples):
        for j, (t, values) in enumerate(sample):
            ...
            if i > 0 and timepoint_array[j] != t:
                raise ValueError(
                    "Sample %d, time point %s is different "
                    "than the first sample (%s)" % (i, t, timepoint_array[j])
                )
    ...
    timepoint_array_3D = np.tile(
        np.reshape(timepoint_array, (-1, 1)), (1, 1, n_channels)
    )
...
def _eeglab_set_attributes(ds):
    setattr(
        ds.__class__,
        "nchannels",
        property(fget=lambda self: len(set(self.fa["time_channel_indices"][:, 1]))),
    )
    setattr(
        ds.__class__,
        "ntimepoints",
        property(fget=lambda self: len(set(self.fa["time_channel_indices"][:, 0]))),
    )

    setattr(
        ds.__class__,
        "channelids",
        property(fget=lambda self: np.unique(self.fa["channelids"].value)),
    )
    setattr(
        ds.__class__,
        "timepoints",
        property(fget=lambda self: np.unique(self.fa["timepoints"].value)),
    )

    setattr(
        ds.__class__,
        "t0",
        property(fget=lambda self: np.min(self.fa["timepoints"].value)),
    )
    ...
    setattr(
        ds.__class__,
        "get_features_by_timepoints",
        property(
            fget=lambda self: lambda f: selector(f, self.fa["timepoints"]),
    ...
    setattr(
        ds.__class__,
        "get_features_by_channelids",
        property(
            fget=lambda self: lambda f: selector(f, self.fa["channelids"]),
            doc="Given a filter function f returns the "
            "indices of features for which f(x) holds "
