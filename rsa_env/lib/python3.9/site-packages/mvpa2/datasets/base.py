...
class Dataset(AttrDataset):
    ...
    def select(self, sadict=None, fadict=None, strict=True):
        ...
        if fadict is None:
            ...
            return self[
                self.sa.match(sadict, strict=strict),
    ...
    def find_collection(self, attr):
        ...
        if attr in self.sa:
            ...
            if __debug__ and (attr in self.fa or attr in self.a):
                warning(
                    "An attribute with name '%s' is also present "
            ...
            raise LookupError(
                "Cannot find '%s' attribute in any dataset " "collection." % attr
            )
        return col
    ...
    def _collection_id2obj(self, col):
        if col == "sa":
            ...
            raise LookupError(
                "Unknown collection '%s'. Possible values "
                "are: 'sa', 'fa', 'a'." % col
            )
        return col
    ...
    def from_wizard(
        ...
        if targets is not None:
            sa_items["targets"] = _expand_attribute(
                targets, samples.shape[0], "targets"
            )

        if chunks is not None:
            # unlike previous implementation, we do not do magic to do chunks
            # if there are none, there are none
            sa_items["chunks"] = _expand_attribute(chunks, samples.shape[0], "chunks")
        ...
        if mask is None:
            ...
            if len(samples.shape) > 2 and (
                (flatten is None and mapper is None) or flatten  # auto case
            ):  # bool case
    ...
    @classmethod
    def from_channeltimeseries(
        cls, samples, targets=None, chunks=None, t0=None, dt=None, channelids=None
    ):
    ...
    targets = property(
        fget=lambda self: self.sa.targets,
    ...
    chunks = property(
        fget=lambda self: self.sa.chunks,
...
class HollowSamples(object):
    ...
    def __reduce__(self):
        return (
            self.__class__,
            ((len(self.sid), len(self.fid)), self.sid, self.fid, self.dtype),
        )
    ...
    def __getitem__(self, args):
        ...
        return HollowSamples((len(sid), len(fid)), sid=sid, fid=fid, dtype=self.dtype)
