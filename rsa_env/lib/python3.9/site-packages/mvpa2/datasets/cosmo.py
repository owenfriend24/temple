...
def _from_singleton(x, ndim=2):
    ...
    if x.shape != s:
        raise ValueError(
            "Expected singleton shape %s for %s, found %s"
            % (
                s,
                x,
                x.shape,
...
def _attributes_cosmo2dict(cosmo):
    ...
    # go over 'sa', 'fa' and 'a'
    for fieldname, do_transpose in list(_attr_fieldname2do_transpose.items()):
        attrs = dict()
...
def _mat_replace_functions_by_string(x):
    ...
    unsupported_classes = (matlab.mio5_params.MatlabFunction, FlattenMapper)
...
def _mat_make_saveable(x, fixer=_mat_replace_functions_by_string):
    ...
    if type(x) is dict:
        # use recursion
        return dict((k, _mat_make_saveable(v, fixer=fixer)) for k, v in list(x.items()))
...
def from_any(x):
    ...
    if isinstance(x, dict):
        ...
        x = dict((k, v) for k, v in list(x.items()) if not _is_private_key(k))
...
class CosmoQueryEngine(QueryEngineInterface):
    ...
    def _check_mapping(mapping):
        ...
        for k, v in list(mapping.items()):
            ...
            if not isinstance(v, np.ndarray):
                raise TypeError("Value %s for key %s must be numpy array" % (v, k))
    ...
    def from_mat(cls, neighbors, a=None, fa=None, origin=None):
        ...
        for id, nbr_fids in enumerate(neighbors_vec):
            ...
            if len(nbr_fids_vec):
                ...
                nbr_fids_vec_int = _numpy_array_astype_unsafe(nbr_fids_vec, np.int_)
    ...
    def set_output_dataset_attributes(self, ds):
        ...
        if not "center_ids" in ds.fa:
            raise KeyError(
                "Dataset seems not to be the result from "
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not isinstance(queryengine, expected_type):
            raise TypeError(
                "Queryengine should be a %s, found type %s"
                % (expected_type, type(queryengine))
            )

        super(CosmoSearchlight, self).__init__(
            datameasure,
            queryengine,
