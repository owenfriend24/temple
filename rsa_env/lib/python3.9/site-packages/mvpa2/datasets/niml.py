...
def from_niml(dset, fa_labels=None, sa_labels=None, a_labels=None):
    ...
    labels_collections = [(sa_labels_, sa), (fa_labels_, fa), (a_labels_, a)]

    infix2collection = {"sa": sa, "fa": fa, "a": a}
...
def hstack(
    dsets,
    pad_to_feature_index=None,
    hstack_method="drop_nonunique",
    set_empty_value=0.0,
):
    ...
    for i, (dset, pad_to) in enumerate(zip(dsets, pad_to_feature_index)):
        ...
        if pad_to is None or pad_to == max_node_index + 1:
            ...
            if nfeatures_empty < 0:
                raise ValueError(
                    "Dataset has %d features, cannot pad "
                    "to %d" % (dset.nfeatures, pad_to)
                )

            # make empty array
            empty_arr = (
                np.zeros((dset.nsamples, nfeatures_empty), dtype=dset.samples.dtype)
                + set_empty_value
            )
        ...
        if label in dset_keys:
            if use_label is None:
                ...
                if len(node_indices) != dset.nfeatures:
                    raise ValueError(
                        "Node indices mismatch: found %d values "
                ...
                if not np.array_equal(dset.fa[label].value, node_indices_int):
                    raise ValueError(
                        "Different indices for feature attributes"
...
def read(fn):
    ...
    keys = [
        exts
        for exts in list(readers_converters.keys())
    ...
    if n_keys == 1:
        ...
        # error message
        for reader, converter in list(readers_converters.values()):
            try:
                r = reader(fn)
                if converter:
                    r = converter(r)
