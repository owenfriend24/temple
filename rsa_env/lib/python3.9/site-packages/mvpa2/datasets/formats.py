...
def to_lightsvm_format(dataset, out, targets_attr="targets", domain=None, am=None):
    ...
    if domain in ["multiclass", "binary"]:
        ...
        if domain == "binary" and set(utargets) != {-1, 1}:
            # need mapping
            if len(utargets) != 2:
                raise ValueError(
            ...
            elif set(am.keys()) != {-1, 1}:
                raise ValueError(
                    "Provided %s doesn't map into binary " "labels -1,+1" % (am,)
                )
        elif domain == "multiclass" and set(utargets) != set(
            range(1, len(utargets) + 1)
        ):
            if am is None:
                am = AttributeMap(
                    dict(list(zip(utargets, list(range(1, len(utargets) + 1)))))
                )
            elif set(am.keys()) != {-1, 1}:
                raise ValueError(
                    "Provided %s doesn't map into multiclass " "range 1..N" % (am,)
                )
