...
from mvpa2.support.nibabel.surf_gifti import to_gifti_image as anat_surf_to_gifti_image
...
def _get_gifti_image(samples):
    ...
    if not isinstance(samples, required_class):
        raise TypeError("Input of type %s must be a %s" % (samples, required_class))
...
def map2gifti(ds, filename=None, encoding="GIFTI_ENCODING_B64GZ", surface=None):
    ...
    def _build_array(data, intent, encoding=encoding):
        ...
        arr = gifti.GiftiDataArray.from_array(
            data.astype(dtype), intent, encoding=encoding
        )
...
def _warn_if_fmri_dataset(ds):
    ...
    fmri_fields = {"imgaffine", "imgtype", "imghdr"}

    ds_fmri_fields = set.intersection(set(ds.a.keys()), fmri_fields)

    if len(ds_fmri_fields) > 0:
        warning(
            "dataset attribute .a has fields %s, which suggest it is an "
