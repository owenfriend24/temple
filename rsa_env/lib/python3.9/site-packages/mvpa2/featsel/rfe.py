...
class RFE(IterativeFeatureSelection):
    ...
    sensitivities = ConditionalAttribute(
        enabled=False, doc="History of sensitivities (might consume too much memory"
    )
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        # bases init first
        IterativeFeatureSelection.__init__(
            self, fmeasure, pmeasure, splitter, fselector, **kwargs
        )
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(RFE, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["update_sensitivity"], default=True)
        )
...
@Article{ GWB+02,
    ...
    def _train(self, ds):
        ...
        while wdataset.nfeatures > 0:
            ...
            if __debug__:
                debug("RFEC", "Step %d: nfeatures=%d" % (step, wdataset.nfeatures))
            ...
            selected_ids.sort()
            if self.ca.is_enabled("history") or self.ca.is_enabled("selected_ids"):
...
class SplitRFE(RFE):
    ...
    nested_errors = ConditionalAttribute(doc="History of errors per each nested split")
    ...
    def _train(self, dataset):
        ...
        if __debug__:
            debug(
                "RFEC",
                "Stage 2: running RFE on full training dataset to "
