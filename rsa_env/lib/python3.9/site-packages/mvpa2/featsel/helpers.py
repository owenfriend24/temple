...
class FixedErrorThresholdStopCrit(StoppingCriterion):
    ...
    def __init__(self, threshold):
        ...
        if threshold > 1.0 or threshold < 0.0:
            raise ValueError(
                "Threshold %f is out of a reasonable range [0,1]." % threshold
            )
...
class NStepsStopCrit(StoppingCriterion):
    ...
    def __init__(self, steps):
        ...
        StoppingCriterion.__init__(self)
        if steps < 0:
            raise ValueError("Number of steps %i is out of a reasonable range." % steps)
        self.__steps = steps
...
class ElementSelector(ClassWithCollections):
    ...
    def _set_mode(self, mode):
        ...
        if not mode in ["discard", "select"]:
            raise ValueError(
                "Unkown selection mode [%s]. Can only be one "
...
class RangeElementSelector(ElementSelector):
    ...
    def __init__(
        self, lower=None, upper=None, inclusive=False, mode="select", **kwargs
    ):
    ...
    def _call(self, seq):
        ...
        if __debug__:
            debug("ES", "Selected %d out of %d elements" % (len(result), len_seq))
        return result
...
class TailSelector(ElementSelector):
    ...
    def _set_tail(self, tail):
        ...
        if not tail in ["lower", "upper"]:
            raise ValueError(
                "Unkown tail argument [%s]. Can only be one "
...
class FixedNElementTailSelector(TailSelector):
    ...
    def __repr__(self):
        return "%s number=%f" % (TailSelector.__repr__(self), self.nelements)
    ...
    def _set_n_elements(self, nelements):
        if __debug__:
            if nelements <= 0:
                raise ValueError(
                    "Number of elements less or equal to zero " "does not make sense."
                )
    ...
    nelements = property(fget=lambda x: x.__nelements, fset=_set_n_elements)
...
class FractionTailSelector(TailSelector):
    ...
    def __repr__(self):
        return "%s fraction=%f" % (TailSelector.__repr__(self), self.__felements)
    ...
    def _set_f_elements(self, felements):
        ...
        if felements > 1.0 or felements < 0.0:
            raise ValueError("Fraction (%f) cannot be outside of [0.0,1.0]" % felements)
