    ...
    [
        ...
        (
            ...
            dict(
                type=str,
                metavar="FILENAME",
                help="""6-column motion parameter file in FSL's McFlirt format. Six
...
def setup_parser(parser):
    ...
    parser_add_common_opt(
        parser, "multidata", metavar="dataset", nargs="*", default=None
    )
...
def run(args):
    ...
    if args.add_vol_attr is not None:
        ...
        if not len(args.add_vol_attr) == len(vol_attr):
            warning(
                "--vol-attr option with duplicate attribute name: " "check arguments!"
            )
    ...
    if args.txt_data is not None:
        ...
        verbose(
            1,
            "Load data from OpenFMRI model specification %s" % args.openfmri_modelbold,
        )
    ...
    if args.openfmri_modelbold is None and args.add_fsl_mcpar is not None:
        ...
        for param in mc_par:
            verbose(
                2, "Add motion regressor as sample attribute '%s'" % ("mc_" + param)
            )
