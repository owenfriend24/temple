...
def setup_parser(parser):
    ...
    for src in (common_args, detrend_args, bandpassfilter_args, normalize_args):
    ...
    if args.filter_passband is not None:
        ...
        if len(args.filter_passband) > 1:
            ...
            if not len(args.filter_passband) == len(args.filter_stopband):
                raise ValueError(
                    "passband and stopband specifications have to " "match in size"
                )
        ...
        try:
            ord, wn = buttord(
                wp, ws, args.filter_passloss, args.filter_stopattenuation, analog=False
            )
            b, a = butter(ord, wn, btype=btype)
        except OverflowError:
            raise ValueError(
                "cannot contruct Butterworth filter for the given " "specification"
            )
