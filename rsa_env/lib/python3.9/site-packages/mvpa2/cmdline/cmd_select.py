...
from mvpa2.cmdline.helpers import parser_add_common_opt, ds2hdf5, hdf2ds
    ...
    while len(rev_expr):
        ...
        if not ex in ("and", "or"):
            ...
            try:
                ...
                raise ValueError(
                    "unknown attribute '%s' in expression [%s]. Valid attributes: %s"
            ...
            try:
                ...
                raise ValueError(
                    "unknown operator '%s' in expression [%s]. Valid operators: %s"
            ...
            try:
                ...
                raise ValueError(
                    "can't convert '%s' into attribute data type '%s' in expression [%s]"
...
def run(args):
    ...
    # indices
    for opt, col, which in (
        (args.samples_by_index, ds.sa, "samples"),
    ...
    for opt, col, which in (
        (args.samples_by_attr, ds.sa, "samples"),
    ...
    # strip attributes
    for attrarg, col, descr in (
        (args.strip_sa, ds.sa, "sample "),
        ...
        if attrarg is not None:
            for attr in attrarg:
                try:
                    ...
                    warning("dataset has no %sattribute '%s' to remove" % (descr, attr))
    # and store
