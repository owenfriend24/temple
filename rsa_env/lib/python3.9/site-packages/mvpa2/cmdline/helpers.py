        ...
        if option_string == "--help":
            ...
            try:
                ...
                subprocess.check_call(
                    "man %s 2> /dev/null" % parser.prog.replace(" ", "-"), shell=True
                )
                sys.exit(0)
...
def param2arg(parser, param, arg_names=None, **kwargs):
    ...
    **kwargs
    ...
    # conversion
    parser.add_argument(*arg_names, help=help, **kwargs)
...
def ca2arg(parser, klass, ca, arg_names=None, help=None):
    ...
    if help:
        help_ = help_ + help
    parser.add_argument(*arg_names, help=help_, default=False, action="store_true")
...
def arg2learner(arg, index=0):
    ...
    if arg in clfswh.descriptions:
        ...
        try:
            ...
            if not len(learner):
                raise argparse.ArgumentTypeError(
                    "not match for given learner capabilities %s in the warehouse"
                    % (arg,)
                )
...
def script2obj(filepath):
    locals = {}
    exec(compile(open(filepath, "rb").read(), filepath, "exec"), locals, locals)
    if not len(locals):
        raise argparse.ArgumentTypeError(
...
def parser_add_optgroup_from_def(parser, defn, exclusive=False, prefix=None):
    ...
    for opt in defn[1]:
        ...
        if (
            len(namespec) == 2
            and not isinstance(namespec[0], str)
...
def process_common_dsattr_opts(ds, args):
    ...
    if args.add_sa_attr is not None:
        ...
        for a in ("targets", "chunks"):
            verbose(2, "Add sample attribute '%s' from sample attributes file" % a)
        ...
        if srcvar is not None:
            for spec in srcvar:
                ...
                if dst_collection is ds.sa:
                    verbose(
                        2, "Add sample attribute '%s' from '%s'" % (attr_name, spec[1])
                    )
                else:
                    verbose(
                        2, "Add feature attribute '%s' from '%s'" % (attr_name, spec[1])
                    )
                attr = loader(spec[1:])
                try:
...
def _load_csv_table(f):
    ...
    table = dict(
        list(zip(reader.fieldnames, [list() for i in range(len(reader.fieldnames))]))
    )
    for row in reader:
...
def get_crossvalidation_instance(
    learner,
    partitioner,
    errorfx,
    ...
    if permutations > 0:
        ...
        permutator = AttributePermutator(
            learner_space, limit={partitioner.get_space(): 1}, count=1
        )
        ...
        null_cv = CrossValidation(
            learner, perm_gen_node, postproc=postproc, errorfx=errorfx
        )
        # Monte Carlo distribution estimator
        distr_est = MCNullDist(
            repeater, tail=prob_tail, measure=null_cv, enable_ca=["dist_samples"]
        )
...
multimask = ("masks", ("-m", "--masks"), {"nargs": "+"})

mask = ("mask", ("-m", "--mask"), {"help": "single mask item"})
...
output_prefix = (
    "outprefix",
    ("-o", "--output-prefix"),
    {"type": str, "metavar": "PREFIX", "help": "prefix for all output file"},
)
