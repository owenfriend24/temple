...
def arg2transform(args):
    ...
    if not args[0] in ("samples", "features"):
        raise ValueError(
            "transformation axis must be 'samples' or 'features' (was: %s)" % args[0]
        )
    axis = args[0]
...
def _describe_attr(attr, style):
    if style == "terse":
        return "%s %s" % (attr.name, _limit_lines(str(attr.value), 1))
    else:
        return "IMPLEMENT ME\n"
...
def txt_content_summary_terse(ds, args):
    ...
    for cdesc, col, describer in (
        ...
        info += " %s attributes:\n" % cdesc
        for attr in sorted(list(col.values()), cmp=lambda x, y: cmp(x.name, y.name)):
...
def sample_histogram(ds, args):
    ...
    if args.ylim is not None:
        pl.ylim(*args.ylim)
    for opt, fx in ((args.x_marker, pl.axvline), (args.y_marker, pl.axhline)):
        if opt is not None:
            for val in opt:
                fx(val, linestyle="--")
...
output_grp = (
    ...
    [
        (("--figure-title",), dict(type=str, help="""title for a plot""")),
        (
            ("--histogram-bins",),
            dict(
                type=int,
