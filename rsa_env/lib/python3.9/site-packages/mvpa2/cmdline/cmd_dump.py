...
from mvpa2.cmdline.helpers import arg2ds, parser_add_common_opt, hdf5compression
...
component_grp = (
    ...
    [
        (
            ("-s", "--samples"),
            dict(action="store_true", help="""dump the dataset samples."""),
        ),
        (("--sa",), dict(help="""name of the sample attribute to be dumped.""")),
        (("--fa",), dict(help="""name of the feature attribute to be dumped.""")),
        (("--da",), dict(help="""name of the dataset attribute to be dumped.""")),
    ],
)

hdf5_grp = ("option for HDF5 output", [hdf5compression[1:]])


def setup_parser(parser):
    from .helpers import parser_add_optgroup_from_def, parser_add_common_attr_opts
    ...
    if args.samples:
        ...
        for attr, col in ((args.sa, ds.sa), (args.fa, ds.fa), (args.da, ds.a)):
            ...
            try:
                ...
                raise ValueError(
                    "unknown attribute '%s', known are %s)" % (attr, list(col.keys()))
                )
    else:
    ...
    if args.format == "txt":
        ...
        try:
            ...
            if hasattr(dumpy, "shape") and len(dumpy.shape) == 1:
                ...
                warning(
                    "conversion to plain text is not supported for " "this data type"
                )
