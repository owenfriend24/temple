...
from mvpa2.cmdline.helpers import (
    strip_from_docstring,
...
parser_args = {
    "description": strip_from_docstring(
        Hyperalignment.__doc__, paragraphs=(4,), sections=("Examples", "Notes")
    ),
...
def setup_parser(parser):
    ...
    parser_add_common_opt(inputargs, "multidata", action="append", required=True)
    parser_add_common_opt(
        inputargs,
        "multidata",
        names=("-t", "--transform"),
...
as the correspond mapper will be used to transform each individual dataset.""",
    ...
    for oopt in sorted(_output_specs):
        outopts.add_argument(
            "--%s" % oopt, action="store_true", help=_output_specs[oopt]["help"]
        )
...
def run(args):
    ...
    hyper = Hyperalignment(
        enable_ca=enabled_ca,
        alignment=ProcrusteanMapper(svd="dgesvd", space="commonspace"),
        **params
    )
    ...
    if args.commonspace:
        ...
        h5save(
            "%s%s.hdf5"
            % (args.output_prefix, _output_specs["commonspace"]["output_suffix"]),
            hyper.commonspace,
    ...
    for ca in _supported_cas:
        ...
        if getattr(args, ca):
            ...
            np.savetxt(
                "%s%s" % (args.output_prefix, _supported_cas[ca]["output_suffix"]),
    ...
    if args.store_transformation:
        for i, pm in enumerate(promappers):
            ...
            h5save(
                "%s%s.hdf5" % (args.output_prefix, "_map%.3i" % i),
                pm,
                compression=args.hdf5_compression,
            )
    if args.transform:
        ...
        for i, td in enumerate(tdss):
            ...
            h5save(
                "%s%s.hdf5" % (args.output_prefix, "_transformed%.3i" % i),
                td,
                compression=args.hdf5_compression,
            )
