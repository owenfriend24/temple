...
__all__ = ["Kernel", "NumpyKernel", "CustomKernel", "PrecomputedKernel", "CachedKernel"]
...
class CachedKernel(NumpyKernel):
    ...
    def _cache(self, ds1, ds2=None):
        ...
        if __debug__ and "KRN" in debug.active:
            debug(
                "KRN",
                "Caching %(inst)s for ds1=%(ds1)s, ds2=%(ds1)s"
    ...
    def compute(self, ds1, ds2=None, force=False):
        ...
        if __debug__ and "KRN" in debug.active:
            debug(
                "KRN",
                "Computing kernel %(inst)s on ds1=%(ds1)s, ds2=%(ds1)s"
                % dict(inst=self, ds1=ds1, ds2=ds2),
            )
        ...
        if len(self.params.which_set()) or changedData or self._lhsids is None:
