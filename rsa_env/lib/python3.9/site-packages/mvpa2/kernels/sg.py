...
class SGKernel(Kernel):
    ...
    def _data2features(data):
        ...
        if __debug__:
            debug(
                "KRN_SG",
                "Converting data of shape %s into shogun RealFeatures" % (data.shape,),
            )
...
class _BasicSGKernel(SGKernel):
    ...
    def _compute(self, d1, d2):
        ...
        if self._normalizer_cls:
            self._k.set_normalizer(self._normalizer_cls(*self._normalizer_args))
...
class PolySGKernel(_BasicSGKernel):
    ...
    inhomogenous = Parameter(
        True, constraints="bool", doc="Whether +1 is added within the expression"
    )

    if not exists("sg ge 0.6.5"):

        use_normalization = Parameter(
            False, constraints="bool", doc="Optional normalization"
        )
