...
class PolyKernel(NumpyKernel):
    ...
    def _compute(self, d1, d2):
        self._k = np.power(
            self.params.gamma * np.dot(d1, d2.T) + self.params.coef0, self.params.degree
        )
...
class ConstantKernel(NumpyKernel):
    ...
    def _compute(self, data1, data2):
        ...
        data2 : numpy.ndarray
          rhs data
        """
        self._k = (self.params.sigma_0 ** 2) * np.ones((data1.shape[0], data2.shape[0]))
...
class GeneralizedLinearKernel(NumpyKernel):
    ...
    gradients = ConditionalAttribute(
        enabled=False, doc="Dictionary of gradients per a parameter"
    )

    gradientslog = ConditionalAttribute(
        enabled=False, doc="Dictionary of gradients per a parameter in logspace"
    )
    ...
    def _compute(self, data1, data2):
        ...
        # the given value
        elif len(Sigma_p.shape) == 1 and Sigma_p.shape[0] == data2.shape[1]:
        ...
        # product
            ...
            len(Sigma_p.shape) == 2
            and Sigma_p.shape[0] == Sigma_p.shape[1] == data2.shape[1]
        ):
            # which due to numpy broadcasting is the same as product
            # with scalar above
            ...
            raise ValueError(
                "Please provide Sigma_p as a scalar, vector, "
        ...
        if do_g or do_gl:
            if np.isscalar(Sigma_p):
                ...
                if do_g:
                    g_Sigma_p = np.empty(gsize)
                if do_gl:
                    gl_Sigma_p = np.empty(gsize)
                for i in range(nfeat):
                    outer = np.multiply.outer(data1[:, i], data2[:, i])
                    if do_g:
                        g_Sigma_p[:, :, i] = outer
                    if do_gl:
                        gl_Sigma_p = Sigma_p[i] * outer
            if do_g:
                self.ca.gradients = dict(sigma_0=2 * sigma_0, Sigma_p=g_Sigma_p)
            if do_gl:
                self.ca.gradientslog = dict(
                    sigma_0=2 * sigma_0 ** 2, Sigma_p=gl_Sigma_p
                )
...
class ExponentialKernel(NumpyKernel):
    ...
    length_scale = Parameter(
        1.0,
        constraints=EnsureFloat() | EnsureListOf(float),
        doc="""
        The characteristic length-scale (or length-scales) of the phenomenon
        under investigation.""",
    )

    sigma_f = Parameter(1.0, constraints="float", doc="""Signal standard deviation.""")
    ...
    def _compute(self, data1, data2):
        ...
        self.wdm = np.sqrt(
            squared_euclidean_distance(data1, data2, weight=(params.length_scale ** -2))
        )
        self._k = params.sigma_f ** 2 * np.exp(-self.wdm)
    ...
    def compute_lml_gradient(self, alphaalphaT_Kinv, data):
        ...
        if np.isscalar(self.length_scale) or self.length_scale.size == 1:
            ...
            for i in range(self.length_scale.size):
                K_grad_i = (
                    (self.length_scale[i] ** -3)
                    * (self.wdm ** -1)
                    * self.kernel_matrix
                    * np.subtract.outer(data[:, i], data[:, i]) ** 2
                )
...
class SquaredExponentialKernel(NumpyKernel):
    ...
    def _compute(self, data1, data2):
        ...
        self.wdm2 = squared_euclidean_distance(
    ...
    length_scale = property(fget=lambda x: x._length_scale, fset=_setlength_scale)
    pass
...
class Matern_3_2Kernel(NumpyKernel):
    ...
    def _compute(self, data1, data2):
        ...
        if self.numerator == 3.0:
            ...
            self._k = (
                self.sigma_f ** 2
                * (1.0 + np.sqrt(5.0) * tmp2 + 5.0 / 3.0 * tmp)
...
class RationalQuadraticKernel(NumpyKernel):
    ...
    def _compute(self, data1, data2):
        ...
        self._k = self.sigma_f ** 2 * (1.0 + tmp / (2.0 * self.alpha)) ** -self.alpha
