...
class PLR(Classifier):
    ...
    def _train(self, data):
        ...
        if set(d) != {0, 1}:
            raise ValueError(
                "Regressors for logistic regression should be [0,1]. Got %s" % (set(d),)
            )
        ...
        X = np.matrix(np.concatenate((X, np.ones((1, npatterns), "d")), 0))
        ...
        while np.sum(np.ravel(dw.A ** 2)) > self.__criterion:
            ...
            if k > self.__maxiter:
                raise ConvergenceError(
                    "More than %d Iterations without convergence" % self.__maxiter
                )
...
class PLRWeights(Sensitivity):
    ...
    def _call(self, dataset=None):
        ...
        if attrmap:
            ...
            labels = attrmap.to_literal(
                asobjarray([tuple(sorted(labels_num))]), recurse=True
            )
        else:
            labels = [(0, 1)]  # we just had our good old numeric ones

        ds = Dataset(clf.w.T, sa={clf.get_space(): labels, "biases": [clf.bias]})
        return ds
