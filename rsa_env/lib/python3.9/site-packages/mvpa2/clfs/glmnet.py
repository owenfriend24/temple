...
class _GLMNET(Classifier):
    ...
    __tags__ = ["glmnet", "linear", "has_sensitivity", "does_feature_selection", "rpy2"]
    ...
    alpha = Parameter(
        1.0,
        constraints=EnsureFloat() & EnsureRange(min=0.01, max=1.0),
    ...
    def _train(self, dataset):
        ...
        try:
            self.__trained_model = trained_model = r.glmnet(
                dataset.samples,
                targets,
        ...
        self.__last_lambda = last_lambda = np.asanyarray(Rrx2(trained_model, "lambda"))[
            -1
        ]
        ...
        if self.params.family == "multinomial":
            ...
            raise NotImplementedError(
                "Somehow managed to get here with family %s." % (self.params.family,)
            )
    ...
    def _predict(self, data):
        ...
        values = np.array(
            r.predict(
                self.__trained_model, newx=data, type="link", s=self.__last_lambda
            )
        )
        ...
        if self.params.family == "multinomial":
            ...
            class_ind = np.array(
                r.predict(
                    self.__trained_model, newx=data, type="class", s=self.__last_lambda
                )
            )
...
class GLMNET_R(_GLMNET):
    ...
    def __init__(self, **kwargs):
        ...
        if family != regr_family:
            warning(
                "You specified the parameter family=%s, but we "
                'force this to be "%s" for regression.' % (family, regr_family)
            )
            family = regr_family
...
class GLMNET_C(_GLMNET):
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.pop("family", None) is not None:
            warning(
                'You specified the "family" parameter, but we '
