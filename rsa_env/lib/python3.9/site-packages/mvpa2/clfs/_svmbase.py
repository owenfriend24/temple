...
from mvpa2.base.dochelpers import handle_docstring, _rst, _rst_section, _rst_indentstr
    ...
    * _KNOWN_IMPLEMENTATIONS: map(dict) should define assignment to a
    ...
    _SVM_PARAMS = {
        "C": Parameter(
            -1.0,
            doc=(
                "Trade-off parameter between width of the "
        ...
        "nu": Parameter(
            0.5, min=0.0, max=1.0, doc="Fraction of datapoints within the margin"
        ),
        "cache_size": Parameter(
            100, doc="Size of the kernel cache, specified in megabytes"
        ),
        "tube_epsilon": Parameter(
            0.01,
            doc=(
                "Epsilon in epsilon-insensitive loss function of "
        ...
        "probability": Parameter(
            0,
            doc=(
                "Flag to signal either probability estimate is obtained "
                "within LIBSVM"
            ),
        ),
        "shrinking": Parameter(1, doc="Either shrinking is to be conducted"),
        "weight_label": Parameter(
            [],
            constraints=EnsureListOf(int),
            doc="To be used in conjunction with weight for custom " "per-label weight",
        ),
        # TODO : merge them into a single dictionary
        "weight": Parameter(
            [], constraints=EnsureListOf(float), doc="Custom weights per label"
        ),
    ...
    def __init__(self, **kwargs):
        ...
        if not svm_impl in self._KNOWN_IMPLEMENTATIONS:
            raise ValueError(
                ...
                "Known are: %s"
                % (svm_impl, self.__class__, list(self._KNOWN_IMPLEMENTATIONS.keys()))
            )
        self._svm_impl = svm_impl

        impl, add_params, add_internals, descr = self._KNOWN_IMPLEMENTATIONS[svm_impl]

        # Add corresponding parameters to 'known' depending on the
        # implementation chosen
        if add_params is not None:
            self._KNOWN_PARAMS = self._KNOWN_PARAMS[:] + list(add_params)
        ...
        if "weight" in self.params and "weight_label" in self.params:
            if not len(self.params.weight_label) == len(self.params.weight):
                raise ValueError(
                    "Lenghts of 'weight' and 'weight_label' lists " "must be equal."
                )

        if __debug__:
            debug("SVM", "Initialized %s with kernel %s" % (self, self.params.kernel))
    ...
    def __repr__(self):
        ...
        res = "%s(svm_impl=%r" % (self.__class__.__name__, self._svm_impl)
        sep = ", "
    ...
    def _get_cvec(self, data):
        ...
        for i in range(len(Cs)):
            if Cs[i] < 0:
                ...
                if __debug__:
                    debug(
                        "SVM", "Default C for %s was computed to be %s" % (C[i], Cs[i])
                    )
    ...
    def _get_default_c(self, data):
        ...
        if self.params.kernel.__kernel_name__ == "linear":
            ...
            datasetnorm = np.mean([np.sqrt(np.sum(sq_func(s))) for s in data])
            if datasetnorm == 0:
                warning(
                    "Obtained degenerate data with zero norm for training "
            ...
            warning(
                "TODO: Computation of default C is not yet implemented"
...
@Book{Vapnik95:SVM,
 ...
 pymvpa-keywords = "support vector machine, SVM"
    ...
    def get_sensitivity_analyzer(self, **kwargs):
        ...
        sana = self._KNOWN_SENSITIVITIES.get(self.params.kernel.__kernel_name__, None)
        if sana:
            return sana(self, **kwargs)
        else:
            raise NotImplementedError(
                "Sensitivity analyzers for kernel %s is unknown" % self.params.kernel
            )
...
"""
        ...
        idoc += "".join(
            [
                ...
                for k, v in list(cls._KNOWN_IMPLEMENTATIONS.items())
            ]
        )
...
"""
        ...
        idoc += (
            ...
            + "\n"
            + "\n".join(
                [v._paramdoc() for k, v in list(cls._SVM_PARAMS.items()) if k in NOS.seen]
            )
        )
...
for k, v in list(_SVM._SVM_PARAMS.items()):
    v._set_name(k)
