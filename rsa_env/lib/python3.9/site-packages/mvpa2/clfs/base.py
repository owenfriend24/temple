...
__all__ = ["Classifier", "accepts_dataset_as_samples", "accepts_samples_as_dataset"]
...
class Classifier(Learner):
    ...
    predictions = ConditionalAttribute(
        enabled=True, doc="Most recent set of predictions"
    )

    estimates = ConditionalAttribute(
        enabled=True,
        doc=(
            "Internal classifier estimates the most recent "
                ...
                for key, data_ in (
                    ...
                    if __invalidatedChangedData.get(key, False):
                        if __debug__ and not _changedData[key]:
                            debug(
                                "CLF_",
                                "Found that idhash for %s was "
                                "invalidated by retraining",
                                (key,),
                            )
    ...
    def _posttrain(self, dataset):
        ...
        if ca.is_enabled("training_stats") and not ca.is_set("training_stats"):
    ...
    def summary(self):
        ...
        if len(ca_enabled):
            s += "\n enabled ca:%s" % ", ".join([str(ca[x]) for x in ca_enabled])
        return s
    ...
    def _prepredict(self, dataset):
        ...
        if self.params.retrainable:
            if not self.__changedData_isset:
                ...
                _changedData["testdata"] = self.__was_data_changed("testdata", data)
                if __debug__:
                    debug(
                        "CLF_",
                        "prepredict: Obtained _changedData is %s",
                        (_changedData,),
                    )
    ...
    def predict(self, dataset):
        ...
        if self.__trainednfeatures > 0 or "notrain2predict" in self.__tags__:
        ...
        if (force or value != pretrainable.value) and "retrainable" in self.__tags__:
            ...
            if value:
                ...
                ca["retrained"] = ConditionalAttribute(
                    enabled=True, doc="Either retrainable classifier was retrained"
                )
    ...
    def __reset_changed_data(self):
        ...
        """
        if __debug__:
            debug("CLF_", "Retrainable: resetting flags on either data was changed")
    ...
    def __was_data_changed(self, key, entry, update=True):
        ...
        if __debug__ and changed:
            debug(
                "CLF_",
                "Changed %s from %s to %s.%s",
                (key, __idhashes[key], idhash_, ("", "updated")[int(update)]),
            )
        if update:
    ...
    def retrain(self, dataset, **kwargs):
        ...
        if __debug__:
            if not self.params.retrainable:
                raise RuntimeError(
                    "Do not use re(train,predict) on non-retrainable %s" % self
                )
    ...
    def repredict(self, dataset, **kwargs):
        ...
        if __debug__ and "CHECK_RETRAIN" in debug.active:
            for key, data_ in (("testdata", dataset.samples),):
                ...
                if self.__was_data_changed(key, data_, update=False):
                    raise RuntimeError(
                        "Data %s found changed although wasn't " "labeled as such" % key
                    )
        ...
        if (
            ...
            raise ValueError(
                "In repredict got dataset with %s size, "
