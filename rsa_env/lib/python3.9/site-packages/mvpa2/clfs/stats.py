...
class Nonparametric(object):
    ...
    def __repr__(self):
        return "%s(%r%s)" % (
            ...
            self._dist_samples,
            ("", ", correction=%r" % self._correction)[int(self._correction != "clip")],
        )
    ...
    def cdf(self, x):
        ...
        return self._cdf(x, np.vectorize(lambda v: (self._dist_samples <= v).mean()))
...
def _pvalue(x, cdf_func, rcdf_func, tail, return_tails=False, name=None):
    ...
    def stability_assurance(cdf):
        if __debug__ and "CHECK_STABILITY" in debug.active:
            ...
            if cdf_min < 0 or cdf_max > 1.0:
                ...
                warning(
                    "Stability check of cdf %s failed%s. Min=%s, max=%s"
    ...
    x_nans = np.isnan(x)
    if np.any(x_nans):
        if (isinstance(x, np.ndarray) and x.shape == pvalues.shape) or (
            pvalues.ndim == 1 and len(x) == len(pvalues)
        ):
            pvalues[x_nans] = 1.0
        else:
            raise ValueError(
...
class NullDist(ClassWithCollections):
    ...
    def _set_tail(self, tail):
        ...
        if tail not in ("left", "right", "any", "both"):
            raise ValueError('Unknown value "%s" to `tail` argument.' % tail)
        self.__tail = tail
    ...
    def p(self, x, return_tails=False, **kwargs):
        ...
        """
        peas = _pvalue(
            x, self.cdf, self.rcdf, self.__tail, return_tails=return_tails, **kwargs
        )
        if is_datasetlike(x):
...
class MCNullDist(NullDist):
    ...
    def fit(self, measure, ds):
        ...
        for p, permuted_ds in enumerate(self.__permutator.generate(ds)):
            ...
            if __debug__:
                debug(
                    "STATMC",
                    "Doing %i permutations: %i" % (self.__permutator.count, p + 1),
                    cr=True,
                )
        ...
        if not len(dist_samples) and skipped > 0:
            raise RuntimeError(
                ...
                "skipped. Check above warnings, and your code/data" % (measure, skipped)
            )
        ...
        self.ca.dist_samples = Dataset(
            np.rollaxis(dist_samples, 0, len(dist_samples.shape))
        )
        ...
        for samples in dist_samples_rs.T:
            ...
            if __debug__ and "STAT__" in debug.active:
                debug(
                    "STAT",
                    "Estimated parameters for the %s are %s"
    ...
    def _cdf(self, x, cdf_func):
        ...
        if len(self._dist) != len(x):
            raise ValueError(
                "Distribution was fit for structure with %d"
...
class FixedNullDist(NullDist):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(FixedNullDist, self).__repr__(prefixes=prefixes_ + prefixes)
...
class AdaptiveNullDist(FixedNullDist):
    ...
    def fit(self, measure, wdata, vdata=None):
        ...
        if __debug__:
            debug(
                "STAT",
                "Adapted parameters for %s to be %s, %s" % (dist_gen, args, kwargs),
            )
...
if externals.exists("scipy"):
    ...
    class rv_semifrozen(object):
        ...
        def fit(self, data, *args, **kwds):
            ...
            opt_x = scipy.optimize.fmin(self.nnlf, x0, args=(np.ravel(data),), disp=0)
    ...
    def match_distribution(
        ...
        **kwargs
        ...
        if not test in _KNOWN_TESTS:
            raise ValueError(
                "Unknown kind of test %s. Known are %s" % (test, _KNOWN_TESTS)
            )
        ...
        if nsamples is not None:
            if __debug__:
                debug(
                    "STAT",
                    "Sampling %d samples from data for the "
        ...
        if test == "p-roc":
            ...
            if true_positives == 0:
                raise ValueError(
                    "Provided data has no elements in non-"
            ...
            if __debug__:
                debug(
                    "STAT_",
                    "Number of positives in non-parametric "
        ...
        try:
            ...
            if sp_version >= "0.9.0":
                for d_ in ["ncf"]:
                    if d_ in sp_dists:
                        warning(
                            "Not considering %s distribution because of "
        ...
        for d in distributions:
            ...
            try:
                ...
                if args_ is not None or loc_ is not None or scale_ is not None:
                    dist_opt = rv_semifrozen(
                        dist_gen_, loc=loc_, scale=scale_, args=args_
                    )
                else:
                    dist_opt = dist_gen_
                ...
                if test == "p-roc":
                    ...
                    cdf_p = np.abs(
                        _pvalue(data, cdf_func, rcdf_func, tail, name=dist_gen)
                    )
                    cdf_p_thr = cdf_p <= p_thr
                    D, p = (
                        np.sum(np.abs(data_p_thr - cdf_p_thr)) * 1.0 / true_positives,
                        1,
                    )
                    if __debug__:
                        res_sum = "D=%.2f" % D
                ...
                if __debug__:
                    debug(
                        "STAT__", "Testing for %s distribution failed due to %s", (d, e)
                    )
                continue
            ...
            if p > p_thr and not np.isnan(D):
                ...
                if __debug__:
                    debug("STAT_", "Tested %s distribution: %s", (dist_name, res_sum))
            else:
                if __debug__:
                    debug("STAT__", "Cannot consider %s dist. with %s", (d, res_sum))
                continue
        ...
        if __debug__ and "STAT" in debug.active:
            ...
            if nresults > 0:
                ...
                debug(
                    "STAT",
                    "Best distribution %s. Next best: %s"
    ...
    if externals.exists("pylab"):
        ...
        def plot_distribution_matches(
            ...
            halign = externals.versions["matplotlib"] >= "1.0.0" and "mid" or "center"
            ...
            for i in range(min(nbest, len(matches))):
                ...
                if p is not None and legend > 2:
                    ...
                    label += " tp/fp/fn=%d/%d/%d)" % tuple(map(np.sum, [tp, fp, fn]))
