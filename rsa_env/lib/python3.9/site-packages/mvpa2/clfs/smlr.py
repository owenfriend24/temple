...
if externals.exists("ctypes"):
    ...
    try:
        ...
        warning(
            "Failed to load fast implementation of SMLR.  May be you "
    ...
    _cStepwiseRegression = None
    warning(
        "SMLR implementation without ctypes is overwhelmingly slow."
...
class SMLR(Classifier):
    ...
    def _python_stepwise_regression(
        self,
        w,
        X,
        XY,
            ...
            if m == 0:
                ...
                if basis == 0:
                    ...
                    incr = np.sqrt(sum2_w_diff) / (
                        np.sqrt(sum2_w_old) + np.finfo(float).eps
                    )
    ...
    def _train(self, dataset):
        ...
        auto_corr = ((M - 1.0) / (2.0 * M)) * (np.sum(X * X, 0))
        XY = np.dot(X.T, Y[:, :c_to_fit])
        lambda_over_2_auto_corr = (self.params.lm / 2.0) / auto_corr
        ...
        if cycles >= self.params.maxiter:
            # did not converge
            raise ConvergenceError(
                "More than %d iterations without convergence" % self.params.maxiter
            )
        ...
        if self.params.ties:
            if self.params.ties == "random":
                ...
                if len(ties):
                    warning(
                        "SMLR: detected ties in categories %s.  Small "
                ...
                raise ValueError(
                    "Do not know how to treat ties=%r" % (self.params.ties,)
                )
        ...
        if self.ca.is_enabled("feature_ids"):
            self.ca.feature_ids = np.where(
                np.max(np.abs(w[: dataset.nfeatures, :]), axis=1) > 0
            )[0]
        ...
        if __debug__:
            debug(
                "SMLR",
                "train finished in %d cycles on data.shape=%s " % (cycles, X.shape)
    ...
    def _unsparsify_weights(self, samples, weights):
        ...
        if __debug__:
            debug(
                "SMLR_",
                "Start nonzero: %d; Finish nonzero: %d"
    ...
    def _predict(self, data):
        ...
        if self.params.fit_all_weights:
            ...
            w = np.hstack(
                (self.__weights_all, np.zeros((self.__weights_all.shape[0], 1)))
            )
        ...
        if self.__ties is not None:
            ...
            E[:, self.__ties] += 1e-5 * np.random.normal(
                size=(len(E), len(self.__ties))
            )
        S = np.sum(E, 1)
        ...
        if __debug__:
            debug(
                "SMLR",
                "predict on data.shape=%s min:max(data)=%f:%f "
        ...
        predictions = np.asarray([self._ulabels[np.argmax(vals)] for vals in values])
...
class SMLRWeights(Sensitivity):
    ...
    def _call(self, dataset=None):
        ...
        if __debug__:
            debug(
                "SMLR",
                "Extracting weights for %d-class SMLR" % (len(weights) + 1)
        ...
        ds = Dataset(weights, sa={clf.get_space(): clf._ulabels[: len(weights)]})
