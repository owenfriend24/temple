...
class ModelSelector(object):
    ...
    def max_log_marginal_likelihood(
        self,
        hyp_initial_guess,
        maxiter=1,
        ...
        self.logscale = (
            logscale  # use log-scale on hyperparameters to enhance numerical stability
        )
        ...
        def f(x):
            ...
            try:
                ...
                if __debug__:
                    debug(
                        "MOD_SEL", "WARNING: Cholesky failed! Invalid hyperparameters!"
                    )
                return -np.inf
            log_marginal_likelihood = (
                self.parametric_model.compute_log_marginal_likelihood()
            )
        ...
        def df(x):
            ...
            if self.logscale:
                gradient_log_marginal_likelihood = (
                    self.parametric_model.compute_gradient_log_marginal_likelihood_logscale()
                )
            else:
                gradient_log_marginal_likelihood = (
                    self.parametric_model.compute_gradient_log_marginal_likelihood()
                )
                pass
    ...
    def solve(self, problem=None):
        ...
        if result.stopcase == -1:
            ...
            self.hyperparameters_best = self.hyp_initial_guess.copy()
            if self.logscale:
                self.hyperparameters_best[self.freeHypers] = np.exp(
                    result.xf
                )  # best hyperparameters found # NOTE is it better to return a copy?
            else:
                self.hyperparameters_best[self.freeHypers] = result.xf
                pass
            self.log_marginal_likelihood_best = (
                result.ff
            )  # actual best vuale of log_marginal_likelihood
            pass
