...
class GDA(Classifier):
    ...
    def _get_priors(self, nlabels, nsamples, nsamples_per_class):
        ...
        if prior == "uniform":
            ...
            priors = (1 + np.squeeze(nsamples_per_class)) / (float(nsamples) + nlabels)
        elif prior == "ratio":
    ...
    def _train(self, dataset):
        ...
        self.means = means = np.zeros((nlabels, nfeatures))
        # degenerate dimension are added for easy broadcasting later on
        # XXX might want to remove -- for now taken from GNB as is
        self.nsamples_per_class = nsamples_per_class = np.zeros((nlabels, 1))
        self.cov = cov = np.zeros((nlabels, nfeatures, nfeatures))

        # Estimate cov
        # better loop than repmat! ;)
        for l, il in list(label2index.items()):
            Xl = X[labels == l]
    ...
    def _inv(self, cov):
        try:
            ...
            raise DegenerateInputError(
                "Data is probably singular, since inverse fails. Got %s" % (e,)
            )
...
class LDA(GDA):
    ...
    def _train(self, dataset):
        ...
        self.cov = cov = np.sum(self.cov, axis=0) / (
...
class QDA(GDA):
    ...
    def _train(self, dataset):
        ...
        self._b = np.array(
            [
                np.log(p) - 0.5 * np.log(np.linalg.det(c))
