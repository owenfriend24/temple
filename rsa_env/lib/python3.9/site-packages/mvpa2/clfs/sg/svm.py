...
if externals.exists("shogun", raise_=True):
    ...
    from mvpa2.kernels.sg import LinearSGKernel
    ...
    if hasattr(shogun.Classifier, "M_DEBUG"):
        ...
        warning(
            "Could not figure out debug IDs within shogun. "
...
from mvpa2.clfs.base import accepts_samples_as_dataset
...
def _setdebug(obj, partname):
    ...
    if __debug__ and "SG_" in debug.active:
        debug(
            "SG_",
            "Setting verbosity for shogun.%s instance: %s to %s"
    ...
    if __debug__ and "SG_LINENO" in debug.active:
        try:
            ...
            warning(
                "Cannot enable SG_LINENO debug target for shogun %s"
    ...
    try:
        ...
        warning(
            "Shogun version %s has no way to enable progress"
...
class SVM(_SVM):
    ...
    if externals.exists("shogun", raise_=True):
        _KNOWN_IMPLEMENTATIONS = {
            "libsvm": (
                shogun.Classifier.LibSVM,
                ("C",),
                ("multiclass", "binary"),
                "LIBSVM's C-SVM (L2 soft-margin SVM)",
            ),
            "gmnp": (
                shogun.Classifier.GMNPSVM,
                ("C",),
                ("multiclass", "binary"),
    ...
    def __init__(self, **kwargs):
        ...
        if svm_impl in ["svmocas"]:
            if not (self.__kernel is None or self.__kernel.__kernel_name__ == "linear"):
                raise ValueError(
...
@article{Sonnenburg+2010:Shogun,
 ...
 publisher = {JMLR.org},
    ...
    def _train(self, dataset):
        ...
        if "kernel-based" in self.__tags__ and (
            not retrainable
            or _changedData["traindata"]
            or _changedData["kernel_params"]
        ):
            ...
            if retrainable and __debug__:
                if _changedData["traindata"]:
                    debug("SG", "Re-Creating kernel since training data has changed")
        ...
        if not retrainable or self.__svm is None or _changedData["params"]:
            ...
            if self._svm_impl in ["libsvr", "svrlight"]:
                ...
                self.__svm = svm_impl_class(
                    Cs[0], self.params.tube_epsilon, self.__kernel, labels
                )
            ...
            self.__svm_apply = (
                externals.versions["shogun"] >= "1"
                and self.__svm.apply
                or self.__svm.classify
            )  # the last one for old API
            ...
            if "tube_epsilon" in self.params and hasattr(
                self.__svm, "set_tube_epsilon"
            ):
            ...
            if _changedData["targets"]:  # labels were changed
                if __debug__:
                    debug("SG__", "Assigning new labels")
                self.__svm.set_labels(labels)
            if newkernel:  # kernel was replaced
                if __debug__:
                    debug("SG__", "Assigning new kernel")
        ...
        # Report on training
        if (__debug__ and "SG__" in debug.active) or self.ca.is_enabled(
            "training_stats"
        ):
            if __debug__:
        ...
        if __debug__ and "SG__" in debug.active:
            debug(
                "SG__",
                "Original labels: %s, Trained labels: %s"
    ...
    def _predict(self, dataset):
        ...
        if retrainable:
            changed_testdata = (
                self._changedData["testdata"] or self.__kernel_test is None
            )
        ...
        if retrainable:
            ...
            if __debug__:
                debug(
                    "SG__",
                    "Re-assigning learing kernel. Repredicted is %s" % repredicted,
                )
        ...
        if self.__is_regression__ or len(ul) == 2:
            ...
            if self._svm_impl == "libsvm":
                ...
                raise RuntimeError(
                    ...
                    % (self._svm_impl, self.__traindataset.sa[self.get_space()].unique)
                )
            if __debug__:
                debug(
                    "SG_",
                    "Using %s for multiclass data of %s"
...
if externals.exists("shogun"):
    for name, item, params, descr in [
        (
            "mpd",
            "shogun.Classifier.MPDSVM",
            "('C',), ('binary',)",
            "MPD classifier from shogun",
        ),
        (
            "lightsvm",
            "shogun.Classifier.SVMLight",
            "('C',), ('binary',)",
            "SVMLight classification http://svmlight.joachims.org/",
        ),
        (
            "svrlight",
            "shogun.Regression.SVRLight",
            "('C','tube_epsilon',), ('regression',)",
            "SVMLight regression http://svmlight.joachims.org/",
        ),
        (
            "krr",
            "shogun.Regression.KRR",
            "('tau',), ('regression',)",
            "Kernel Ridge Regression",
        ),
        (
            "svmocas",
            "shogun.Classifier.SVMOcas",
            "('C',), ('binary', 'linear')",
        ...
        if externals.exists("shogun.%s" % name):
            exec(
                'SVM._KNOWN_IMPLEMENTATIONS["%s"] = (%s, %s, "%s")'
