...
def squared_euclidean_distance(data1, data2=None, weight=None):
    ...
    if __debug__:
        ...
        if not np.issubdtype(data1.dtype, "f") or (
            data2 is not None and not np.issubdtype(data2.dtype, "f")
        ):
            warning("Computing euclidean distance on integer data " "is not supported.")
    ...
    if __debug__ and "CHECK_STABILITY" in debug.active:
        ...
        if less0num > 0:
            ...
            if totalnorm != 0 and norm0 / totalnorm > 1e-8:
                warning(
                    "Found %d elements out of %d unstable (<0) in "
...
def one_minus_correlation(X, Y):
    ...
    if __debug__:
        if not X.shape[1] == Y.shape[1]:
            raise ValueError(
                "correlation() requires to matrices with the "
                "same #columns (Got: %s and %s)" % (X.shape, Y.shape)
            )
...
def pnorm_w_python(
    data1, data2=None, weight=None, p=2, heuristic="auto", use_sq_euclidean=True
):
    r"""Weighted p-norm between two datasets (pure Python implementation)
    ...
    if p == 2 and use_sq_euclidean:
        return np.sqrt(
            squared_euclidean_distance(data1=data1, data2=data2, weight=weight ** 2)
        )
    ...
    if heuristic == "auto":
        heuristic = {False: "samples", True: "features"}[(F1 / S1) < 500]

    if heuristic == "features":
        #  Efficient implementation if the feature size is little.
        for NF in range(F1):
            d += pf(np.abs(np.subtract.outer(data1[:, NF], data2[:, NF])) * weight[NF])
            pass
        ...
        raise ValueError(
            "Unknown heuristic '%s'. Need one of "
