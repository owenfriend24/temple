...
def _fix_rawniml_table_output(r):
    ...
    colms_tp["data"] = "R_col;G_col;B_col;A_col;" "Node_Index_Label;Node_String_Label"
...
def merge(annots):
    ...
    def annot2idx_table_data(annot):
        return annot["node_indices"], annot["AFNI_labeltable"]["data"], annot["data"]

    idxs, tables, datas = list(
        map(list, list(zip(*list(map(annot2idx_table_data, annots)))))
    )
    ...
    for i in range(ncols):
        ...
        if all(
            isinstance(d[i], np.ndarray) and np.issubdtype(m.dtype, int)
            for m in columns
        ):
