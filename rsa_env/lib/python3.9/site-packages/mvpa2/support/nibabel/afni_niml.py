...
def _datastring2rawniml(s, niml):
    ...
    if onetype is None or ([onetype] == types.str2codes("string") and len(tps) > 1):
    ...
    if not niform or niform == "text":
        ...
        debug(
            "NIML",
            "data vector has %d elements, reshape to %d x %d = %d",
...
def find_attribute_node(niml_dict, key, value, just_one=True):
    ...
    tp = type(niml_dict)
    if tp is list:
        r = sum([find_attribute_node(d, key, value, False) for d in niml_dict], [])

    elif tp is dict:
        r = [niml_dict] if niml_dict.get(key, None) == value else []
        r.extend(
            find_attribute_node(niml_dict[k], key, value, False)
            for k, v in list(niml_dict.items())
...
def rawniml2string(p, form="text"):
    ...
    if "nodes" in q:
        ...
        data = types.nimldataassupporteddtype(
            data
        )  # ensure the data format is supported by NIML
    ...
    if isinstance(data, str):
        ...
        if form == "text" or types.numpy_data_isstring(data):
            ...
            return _TEXT_ROWSEP.join(
                [
                    _TEXT_COLSEP.join([f(data[row, col]) for col in range(ncols)])
        ...
        if ncols == 0:
            ...
            fs = [
                types.numpy_data2printer(d[0] if type(d) is list else d) for d in data
            ]

            return _TEXT_ROWSEP.join(
                [
                    _TEXT_COLSEP.join([fs[col](data[col][row]) for col in range(ncols)])
                    for row in range(nrows)
                ]
            ).encode()
    ...
    while True:
        ...
        if m is None:
            ...
            if m is None:
                ...
                debug(
                    "NIML",
                    "Completed parsing, length %d (%d elements)",
                    (len(s), len(nimls)),
                )
                if return_pos:
                    return i, nimls
            ...
            debug(
                "NIML", "Parsing header %s, header end position %d", (name, i + m.end())
            )
            ...
            if niml.get("ni_form", None) == "ni_group":
                ...
                debug(
                    "NIML",
                    "Parsing element %s from position %d, total " "length %d",
                    (niml["name"], i, len(s)),
                )
                ...
                is_string = niml["ni_type"] == "String" or not "ni_form" in niml
                if is_string:
                    ...
                    is_multiple_string_data = len(vec_typ) > 1 and types._one_str2code(
                        "String"
                    ) == types.findonetype(vec_typ)

                    if is_mixed_data or is_multiple_string_data:
                        debug(
                            "NIML",
                            "Data is mixed type (string=%s)" % is_multiple_string_data,
                        )
                        # strpat = ('\s*(?P<data>.*)\s*</%s>' % \
                        #                        (name.decode())).encode()
                        strpat = (
                            r"\s*(?P<data>.*?)\s*</%s>" % (name.decode())
                        ).encode()
                    ...
                    if m is None:
                        ...
                        raise ValueError(
                            "Could not parse string data from "
                            "pos %d: %s" % (i, _partial_string(s, i))
                        )
                    ...
                    if "base64" in niml["ni_form"]:
                        ...
                        debug(
                            "NIML",
                            "Raw data with %d bytes - total length "
                            "%d, starting at %d",
                            (nbytes, len(s), i),
                        )
                    ...
                    if s[i : (i + len(endstr))].decode() != endstr:
                        raise ValueError(
                            "Not found expected end string %s"
                            "  (found %s...)" % (endstr, _partial_string(s, i))
                        )
                    i += len(endstr)

            debug(
                "NIML",
                "Adding element '%s' with keys %r" % (niml["name"], list(niml.keys())),
            )
            nimls.append(niml)
...
def _binary_data_bytecount(niml):
    ...
    debug(
        "NIML",
        "Number of bytes for %s: %d x %d with %d bytes / element",
