...
def load_afni(fn, master_fn=None):
    ...
    if master_fn is None:
        cmd = "3dbucket -overwrite -prefix %s %s[0]" % (tmp_fn, fn)
    else:
...
def load_nii(fn):
    ...
    if len(data.shape) != 3:
        raise ValueError("Expected 3D array, but shape is %s" % (data.shape,))
    return data
...
def blend(datas, weights=None):
    ...
    for (data, weight) in zip(datas, weights):
        ...
        if data.shape != b.shape:
            raise ValueError("Shape mismatch: %s %s %s" % slice_.shape, b.sh)
        b += weight * data
...
def clip_inplace(x, range_=(0.0, 1.0)):
    """
    Clips data in-place
...
def make_plot(
    ulay, olay, dims, pos, title=None, ulay_range=None, olay_range=None, output_fn=None
):
    """
    ...
    for j in range(ncol):
        for i in range(nrow):
            ...
            if externals.versions["matplotlib"] < "2.0.0":
                ...
                ax.imshow(sl[i][j].astype(np.float32), extent=(0, imglength) * 2)
...
def make_scatter(ulay, olay, output_fn=None):
    ...
    cutoff_rel = 0.1  # ignore lowest 10% of values
    figsize = (15, 12)  # size of figure
    histbincount = 25  # number of bins in histogram
    internal_rel = 0.25  # only show voxels within 50% of center of mass
    ...
    xyzm = np.asarray(
        xyz[msk.ravel(), :], dtype=np.float_
    )  # mask & get to native float dtype
...
def get_parser():
    ...
    p.add_argument(
        ...
        default=[0, 1, 2],
        type=int,
        help="Dimensions to plot; 0=x, 1=y, 2=z",
    )
    p.add_argument(
        "-p",
        "--pos",
        ...
        type=float,
        help="Relative slice positions in range 0..1",
    )
