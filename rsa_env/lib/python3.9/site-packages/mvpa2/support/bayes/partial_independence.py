...
import numpy as np
from mvpa2.support.bayes.multivariate_polya import (
    log_multivariate_polya_vectorized as log_multivariate_polya,
)
...
def compute_logp_H(X, psi, alpha=None):
    ...
    for group in psi:
        if len(group) == 1:
            logp_H += log_multivariate_polya(X[group[0], :], alpha[group[0], :])
        else:
            nogroup = [a for a in range(X.shape[1]) if a not in group]
            logp_H += np.sum(
                [
                    log_multivariate_polya(
                        [X[i, group].sum()] + X[i, nogroup].tolist(),
                        [alpha[i, group].sum()] + alpha[i, nogroup].tolist(),
                    )
                    for i in group
                ]
            )
            logp_H += compute_logp_independent_block(
                X[np.ix_(group, group)], alpha[np.ix_(group, group)].sum(0)
            )  # should we use sum(0) or mean(0)? or else?
    return logp_H
...
if __name__ == "__main__":
    ...
    X = np.array([[10, 10, 0], [10, 10, 0], [0, 0, 20]])
    # X = np.array([[13, 3, 4],
    #               [ 3,14, 3],
    #               [ 4, 3,13]])
