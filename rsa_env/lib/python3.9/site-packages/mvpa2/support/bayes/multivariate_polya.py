...
def log_multivariate_polya_vectorized(X, alpha):
    ...
    )  # log(\frac{N!}{\prod_i (X_i)!})
    log_likelihood += (
        gammaln(A) - gammaln(alpha).sum()
    )  # log(\frac{\Gamma(\sum_i alpha_i)}{\prod_i(\Gamma(\alpha_i))})
    log_likelihood += gammaln(X + alpha).sum() - gammaln(
        N + A
    )  # log(\frac{\prod_i(\Gamma(X_i +\alpha_i))}{\Gamma(\sum_i X_i+\alpha_i)})
...
if __name__ == "__main__":
    ...
    print((
        "log of the basic vectorized formula:",
        np.log(multivariate_polya_vectorized(x, alpha)),
    ))
