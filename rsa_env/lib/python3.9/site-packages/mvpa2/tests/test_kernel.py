...
from mvpa2.clfs.distance import squared_euclidean_distance, pnorm_w, pnorm_w_python
...
class KernelTests(unittest.TestCase):
    ...
    def kernel_equiv(self, k1, k2, accuracy=None, relative_precision=0.6):
        ...
        if accuracy is None:
            ...
            k12mean_nz = k12mean >= np.finfo(k1m.dtype).resolution * 1e1
        ...
        self.assertTrue(
            np.all(k1m.astype("float32") == k2m.astype("float32")),
    ...
    def test_linear_kernel(self):
        ...
        lk.compute(d1)
        self.assertTrue(
            lk._k.shape == (10, 10), "Failure computing LinearKernel (Size mismatch)"
        )
    ...
    def test_precomputed_kernel(self):
        ...
        nk.compute()
        self.assertTrue(
            (d == nk._k).all(), "Failure setting and retrieving PrecomputedKernel data"
        )
    ...
    def test_cached_kernel(self):
        ...
        self.assertTrue(ck._recomputed, "CachedKernel was not initially computed")
        ...
        for chunk in [d[d.sa.chunks == i] for i in range(nchunks)]:
            ...
            self.assertFalse(
                ck._recomputed, "CachedKernel incorrectly recomputed it's kernel"
            )
        ...
        ck.compute(d)
        self.assertTrue(
            ck._recomputed, "CachedKernel doesn't recompute on kernel change"
        )
        rk.params.sigma = 3.5
        rk.compute(d)
        self.assertTrue(
        ...
        ck.compute(d2)
        self.assertTrue(
            ck._recomputed, "CachedKernel did not automatically recompute new data"
        )
        ck.compute(d)
        self.assertTrue(
            ck._recomputed,
    ...
    if _has_sg:
        ...
        def test_custom_sg(self):
            ...
            custom = sgK.CustomSGKernel(sgK.sgk.PolyKernel, kernel_params=poly_params)
            ...
            self.assertTrue(
                np.all(lk.as_np()._k == cl.as_np()._k),
                "CustomSGKernel does not agree with Linear",
            )
            self.assertTrue(
                np.all(poly.as_np()._k == custom.as_np()._k),
    ...
    def test_pnorm_w(self):
        ...
        self.assertRaises(
            ValueError, pnorm_w_python, data0[:10, :2], p=1.2, heuristic="buga"
        )
        self.assertRaises(ValueError, pnorm_w_python, data0[:10, :2], weight=weight)

        self.assertRaises(
            ValueError, pnorm_w_python, data0[:10, :2], data0[:10, :3], weight=weight
        )
        self.assertRaises(
