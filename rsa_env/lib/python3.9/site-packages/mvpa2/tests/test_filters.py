...
def test_resample():
    ...
    ds = Dataset(
        np.vstack((np.sin(time), np.cos(time))).T,
        sa={"time": time, "section": np.repeat(list(range(10)), 10)},
    )
    assert_equal(ds.shape, (100, 2))

    # downsample
    num = 10
    rm = FFTResampleMapper(
        num, window=("gauss", 50), position_attr="time", attr_strategy="sample"
    )
    mds = rm.forward(ds)
    ...
    rm = FFTResampleMapper(
        num, attr_strategy="sample", chunks_attr="chunks", window=("gauss", 50)
    )
    mcds = rm.forward(cds)
