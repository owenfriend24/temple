...
def _get_test_sample_node_data(format_=None):
    ...
    if format_ is None:
        ...
        samples = [
            (("%.3f " * nfeatures) % tuple(sample.tolist())).strip()
        ...
        samples = [
            "eJzz4mFykI1K2b+nJHi//t0p9gAsDAZU",
...
<GIFTI Version="1.0"  NumberOfDataArrays="%d">""" % (
        ndata_arrays
    )
    ...
    body_elements.extend(_build_data_array(sample, "NONE") for sample in samples)
...
def test_gifti_dataset(fn, format_, include_nodes):
    ...
    ds3_sa = gifti_dataset(
        fn, targets=expected_ds_sa.targets, chunks=expected_ds_sa.chunks
    )
    ...
    if include_nodes:
        ...
        assert_equal(node_arr.intent, intent_codes.code["NIFTI_INTENT_NODE_INDEX"])
    ...
    for i in range(nsamples):
        ...
        assert arr.intent not in [intent_codes.code[s] for s in illegal_intents]
    ...
    if include_nodes:
        assert_raises(AssertionError, assert_datasets_almost_equal, ds6, expected_ds)
    else:
