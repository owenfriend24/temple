...
class VerboseOutputTest(unittest.TestCase):
    ...
    def test_verbose_negative(self):
        ...
        self.assertRaises(ValueError, verbose._set_level, -10)
    ...
    if __debug__:
        ...
        def test_debug(self):
            ...
            self.assertTrue(
                rematch, msg="Cannot match %s with regexp %s" % (svalue, regexp)
            )
            # find metrics
            self.assertTrue(
                "RSS/VMS:" in svalue, msg="Cannot find vmem metric in " + svalue
            )
            self.assertTrue(
                ">test_verbosity:" in svalue, msg="Cannot find tbc metric in " + svalue
            )
            self.assertTrue(" sec" in svalue, msg="Cannot find tbc metric in " + svalue)

        def test_debug_rgexp(self):
            verbose.handlers = []  # so debug doesn't spoil it
            debug.active = [".*"]
            # we should have enabled all of them
            self.assertEqual(set(debug.active), set(debug.registered.keys()))
            debug.active = ["S.*", "CLF"]
            self.assertEqual(
                set(debug.active),
                set(
                    [x for x in list(debug.registered.keys()) if x.startswith("S")]
                    + ["CLF"]
                ),
            )
            debug.active = ["SG", "CLF"]
