...
class DochelpersTests(unittest.TestCase):
    ...
    def test_borrow_kwargs(self):
        ...
        for m in (B.met1, B.met_nodoc, B.met_nodockwargs, B.met_excludes):
            docstring = m.__doc__
            ...
            self.assertTrue((("kp1 " in docstring) ^ (m == B.met_excludes)))
    ...
    def test_recursive_reprs(self):
        ...
        self.assertEqual(
            repr(c2),
            "%(mod)sC2(p=%(mod)sC1(f=<bound %(mod)sC2%(c2id)s.trouble>)%(c1id)s)%(c2id)s"
            % locals(),
        )
    ...
    def test_strip_strid(self):
        self.assertEqual(
            strip_strid("#1"), "#1"
        )  # very short ones left alone -- unlikely to be id
        self.assertEqual(strip_strid("#140136298269872"), "")  # long ones replaced
        self.assertEqual(
            strip_strid("a#140136298269872 b#140136298269872"), "a b"
        )  # multiple
