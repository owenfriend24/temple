...
from mvpa2.support.nibabel import (
    afni_niml,
    afni_niml_dset,
    afni_niml_roi,
    surf,
...
class SurfTests(unittest.TestCase):
    ...
    def _get_rng(self):
        keys = [(17 * i ** 5 + 78234745 * i + 8934) % (2 ** 32 - 1) for i in range(624)]
    ...
    def test_afni_niml(self):
        ...
        data = np.asarray(
            [[1347506771, 1347506772], [1347506773, 1347506774]], dtype=np.int32
        )
        ...
        for fmt, data_repr in list(fmt_data_reprs.items()):
                ...
                if k == "nodes":
                    ...
                    for node in v:
                        for kk, vv in list(node.items()):
                            ...
                            dvals = [
                                d[0]["nodes"][i].get(kk, None) for i in range(len(v))
                            ]
            ...
            assert_array_equal(
                d[0]["nodes"][0]["data"], d_empty_nodes[0]["nodes"][0]["data"]
            )
            assert_equal(
                list(d[0]["nodes"][0].keys()), list(d_empty_nodes[0]["nodes"][0].keys())
            )
            assert_array_equal(
                d[0]["nodes"][1]["data"], d_empty_nodes[0]["nodes"][1]["data"]
            )
            assert_equal(
                list(d[0]["nodes"][1].keys()), list(d_empty_nodes[0]["nodes"][1].keys())
            )
    ...
    def test_surface_dset_niml_io_with_unicode(self, fn):
        ...
        for name, expected_dtype in list(expected_dtypes.items()):
    ...
    def test_afni_niml_dset(self, fn):
        ...
        labels = ["lab_%d" % round(rng.uniform() * 1000) for _ in range(sz[1])]
                    ...
                    for k in dset_keys:
                        ...
                        if k == "data":
                            if mode == "sparse2full":
                                ...
                                for pos, val in list(expected_vals.items()):
                                    if val is None:
    ...
    def test_niml(self, fn):
        ...
        readers = (niml.from_any, lambda x: niml.from_niml(afni_niml_dset.read(x)))
...
# </Node_ROI>"""
        ...
        expected_edges = [
            arr([42946]),
            arr([42946, 42947, 43062, 43176, 43289, 43401, 43512, 43513, 43623, 43732]),
            arr([43732, 43623, 43514, 43404, 43293, 43181, 43068, 42954]),
            arr([42954, 42953, 42952, 42951, 42950, 42949, 42948, 42947, 42946]),
        ]
    ...
    def test_afni_suma_spec(self, temp_dir):
        ...
        # generate surfaces
        inflated_surf = surf.generate_plane((0, 0, 0), (0, 1, 0), (0, 0, 1), 10, 10)
        white_surf = inflated_surf + 1.0
        ...
        assert_true("SurfaceSpec instance with 2 surfaces" ", 2 states " in "%s" % spec)
        ...
        rh_inflated_fn = _tmp(os.path.split(inflated_fn)[1].replace("_lh", "_rh"))
        ...
        # test adding views
        added_specs = afni_suma_spec.hemi_pairs_add_views(
            (lh_spec, rh_spec), "inflated", ".asc"
        )

        for hemi, added_spec in zip(("l", "r"), added_specs):
            states = ["smoothwm", "inflated"] + ["CoM%sinflated" % i for i in "msiap"]
            assert_equal(states, [s["SurfaceState"] for s in added_specs[0].surfaces])
            all_temp_fns.extend([s["SurfaceName"] for s in added_spec.surfaces])
        ...
        assert_equal(
            [s["SurfaceState"] for s in mh_spec.surfaces],
