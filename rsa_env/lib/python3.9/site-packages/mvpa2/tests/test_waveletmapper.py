...
class WaveletMappersTests(unittest.TestCase):
    def test_simple_wdm(self):
        ...
        self.assertRaises(ValueError, WaveletTransformationMapper, wavelet="bogus")
        self.assertRaises(ValueError, WaveletTransformationMapper, mode="bogus")

        # use wavelet mapper
        for wdm, wdm_swap in (
            (WaveletTransformationMapper(), WaveletTransformationMapper(dim=2)),
            (WaveletPacketMapper(), WaveletPacketMapper(dim=2)),
        ):
    ...
    def test_simple_wp1_level(self):
        ...
        if externals.exists("pywt wp reconstruct"):
            ...
            # input... but should be very close ;-)
            self.assertEqual(
                d3d_rev.shape, d3d.shape, msg="Shape should be the same after iDWT"
            )
    ...
    def _test_compare_to_old(self):  # pragma: no cover
        ...
        # use wavelet mapper
        for wdm, wdm_ in (
            (WaveletTransformationMapper(), wavelet_.WaveletTransformationMapper()),
