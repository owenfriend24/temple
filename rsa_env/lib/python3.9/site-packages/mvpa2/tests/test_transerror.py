...
class ErrorsTests(unittest.TestCase):
    def test_confusion_matrix(self):
        ...
        self.assertEqual(len(cm.sets), 1, msg="Should have a single set so far")
        self.assertEqual(
            cm.matrix.shape,
            (3, 3),
        ...
        self.assertEqual(cm.labels, [1, 2, 3, 4], msg="We should have gotten 4th label")
        ...
        self.assertTrue(
            len(cm.as_string(header=True, summary=True, description=True)) > 100
        )
    ...
    def test_confusion_matrix_acc(self):
        ...
        self.assertTrue(cm.stats["CHI^2"] == (0.0, 1.0))
    ...
    def test_null_dist_prob(self, l_clf):
        ...
        num_perm = 10
        permutator = AttributePermutator("targets", count=num_perm, limit="chunks")
        ...
        cvte = CrossValidation(
            l_clf,
            OddEvenPartitioner(),
            null_dist=MCNullDist(permutator, tail="left", enable_ca=["dist_samples"]),
        ...
        self.assertEqual(
            cvte.null_dist.ca.dist_samples.shape[2],
    ...
    def test_auc(self, clf):
        ...
        if isinstance(clf, MulticlassClassifier) or "<kNN" in str(clf):
            raise SkipTest("TODO: handle values correctly in %s" % clf)
        ...
        ds3.sa.targets = nl
        for ds in [datasets["uni2" + ds_size], ds2, datasets["uni3" + ds_size], ds3]:
            cv = CrossValidation(
    ...
    def test_confusion_plot(self):
        ...
        if externals.exists("pylab plottable"):
            ...
            fig, im, cb = cm.plot(
                labels=labels_order[1:2] + labels_order[:1] + labels_order[2:],
                numbers=True,
            )
    ...
    def test_confusionmatrix_nulldist(self):
        ...
        for snr in [
            ...
            ds = normal_feature_dataset(
                snr=snr, perlabel=42, nchunks=3, nonbogus_features=[0, 1], nfeatures=2
            )

            clf = GNB()
            num_perm = 50
            permutator = AttributePermutator("targets", limit="chunks", count=num_perm)
            cv = CrossValidation(
                clf,
                NFoldPartitioner(),
            ...
            if cfg.getboolean("tests", "labile", default="yes"):
                if snr == 0.0:
                    ...
                    assert_array_less(0.9, cvnp[(np.array([0, 1]), np.array([1, 0]))])
...
def test_confusion_as_node():
    ...
    ds = normal_feature_dataset(
        snr=2.0, perlabel=42, nchunks=3, nonbogus_features=[0, 1], nfeatures=2
    )
    clf = GNB()
    cv = CrossValidation(
        clf,
    ...
    cv = CrossValidation(
        ...
        NFoldPartitioner(),
        errorfx=None,
        postproc=ChainNode([Confusion(labels=ds.UT), BayesConfusionHypothesis()]),
    )
    res = cv(ds)
...
def test_bayes_confusion_hyp():
    ...
    conf = np.array([[10, 0, 5, 5], [0, 10, 5, 5], [5, 5, 10, 0], [5, 5, 0, 10]])
