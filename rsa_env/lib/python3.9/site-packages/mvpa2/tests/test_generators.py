...
def test_splitter():
    ...
    spl2 = Splitter(
        "targets", attr_values=[0, 1, 1, 2, 3, 3, 3], count=4, noslicing=True
    )
...
def test_attrpermute():
    ...
    permutation = AttributePermutator(
        "targets", limit="chunks", strategy="uattrs", assure=True
    )
    pds = permutation(ds)
    ...
    # test combinations
    permutation = AttributePermutator(
        ["targets", "odds"], limit="chunks", strategy="uattrs", assure=True
    )
    pds = permutation(ds)
    # Due to assure above -- we should have changed things
    assert_not_equal(
        list(zip(ds.targets, ds.sa.odds)), list(zip(pds.targets, pds.sa.odds))
    )
    # In each chunk we should have unique remappings
    for c in ds.UC:
        chunk_idx = ds.C == c
...
def test_balancer():
    ...
    assert_equal(list(get_nelements_per_value(res.sa.targets).values()), [2] * 4)
    ...
    assert_equal(
        list(get_nelements_per_value(res[res.sa.chunks == 3].sa.targets).values()),
        [2] * 4,
    )
    assert_equal(
        list(get_nelements_per_value(res.sa.chunks).values()),
        [10, 10, 10, 8, 10, 10, 10, 10, 10, 10],
    )
    # fixed amount
    bal = Balancer(amount=1, limit={"chunks": 3}, apply_selection=True)
    res = bal(ds)
    assert_equal(list(get_nelements_per_value(res.sa.targets).values()), [1] * 4)
    # fraction
    ...
    res = bal(ds)
    assert_equal(list(get_nelements_per_value(res.fa.one).values()), [4] * 2)
...
def test_sifter():
    ...
    ds = Dataset(
        samples=np.arange(8).reshape((4, 2)),
        sa={"chunks": [0, 1, 2, 3], "targets": ["c", "c", "p", "p"]},
    )
...
def test_sifter_with_balancing():
    ...
    # "... somewhat duplicating the doctest"
    ds = Dataset(
        samples=np.arange(12).reshape((-1, 2)),
        sa={"chunks": [0, 1, 2, 3, 4, 5], "targets": ["c", "c", "c", "p", "p", "p"]},
    )
    ...
    # entries
    assert_raises(
        ValueError, lambda x: list(Sifter([("targets", dict(wrong=1))]).generate(x)), ds
    )
...
def test_exclude_targets_combinations_subjectchunks():
    ...
    ds = Dataset(
        ...
        sa={"chunks": np.arange(9) // 3, "subjects": np.arange(9) % 3},
    )
...
def test_permute_chunks():
    ...
    permutation = AttributePermutator(
        attr="targets", chunk_attr="chunks", strategy="chunks", assure=True
    )
    ...
    permutation = AttributePermutator(attr="targets", strategy="chunks")
...
def test_factorialpartitioner():
    ...
    ds_unbalanced = Dataset(
        list(range(4)), sa={"subord": [0, 0, 1, 2], "superord": [1, 1, 2, 2]}
    )
    ...
    for out_part, true_part, super_out, sub_out in zip(
        p_fpart, p_unbalanced, superord_unbalanced, subord_unbalanced
    ):
        assert_array_equal(out_part, true_part)
        assert_array_equal(
            (
                ds_unbalanced[out_part == 1].sa.superord.tolist(),
        ...
        assert_array_equal(
            (
                ds_unbalanced[out_part == 1].sa.subord.tolist(),
    ...
    ds_dummy = Dataset(
        list(range(4)), sa={"subord": list(range(4)), "superord": [1, 2] * 2}
    )
...
def test_factorialpartitioner_big():
    ...
    ds = normal_feature_dataset(nlabels=6, perlabel=66, nfeatures=2, nchunks=11)

    # and now let's do factorial partitioner

    def partition(ds_=ds, **kwargs):
        partitioner = FactorialPartitioner(
            partitioner=NFoldPartitioner(attr="targets"), attr="chunks", **kwargs
        )
        return [p.sa.partitions for p in partitioner.generate(ds_)]
