...
def test_dual_gaussian(f):
    ...
    histfit = fit2histogram(
        np.repeat(data[None, :], 2, axis=0),
        f,
...
def test_random_affine_transformation():
    ...
    assert_array_almost_equal(
        np.dot(
            (ds_d.samples - ds_d.a.random_shift) / ds_d.a.random_scale,
...
def test_ttest_1samp_masked():
    ...
    for null in 0, 0.5:
        # 1D case
        _assert_array_equal(ttest_1samp(d[0], null), ttest_1samp_masked(d[0], null))

        for axis in 0, 1, None:
            _assert_array_equal(
                ttest_1samp(d, null, axis=axis), ttest_1samp_masked(d, null, axis=axis)
            )
    ...
    d = np.arange(10)
    _assert_array_equal(
        ttest_1samp(d[3:], 0), ttest_1samp_masked(d, 0, mask=[False] * 3 + [True] * 7)
    )

    # random mask
    m = random_mask(d.shape)
    _assert_array_equal(ttest_1samp(d[m], 0), ttest_1samp_masked(d, 0, mask=m))
    ...
    for axis in range(d.ndim):
        for t0 in (0, 1.0):  # test for different targets
            ...
            for d_, m_, t_, p_ in zip(
                iterflat_view(d), iterflat_view(m), ts.flatten(), ps.flatten()
            ):
