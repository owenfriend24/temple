...
def test_ex_from_masked():
    ds = Dataset.from_wizard(
        samples=np.atleast_2d(np.arange(5)).view(myarray), targets=1, chunks=1
    )
    ...
    ds = vstack(
        (
            ds,
            Dataset.from_wizard(np.random.standard_normal((2, 5)), targets=2, chunks=2),
        )
    )
    ...
    ds = vstack(
        (
            ds,
            Dataset.from_wizard(np.random.standard_normal((2, 5)), targets=3, chunks=5),
        )
    )
...
def test_shape_conversion():
    ds = Dataset.from_wizard(
        np.arange(24).reshape((2, 3, 4)).view(myarray), targets=1, chunks=1
    )
    # array subclass survives
    ok_(isinstance(ds.samples, myarray))
...
def test_ds_array(dsp):
    ...
    dsname, ds = dsp
    if dsname != "hollow":
        ok_(np.asarray(ds) is ds.samples, msg="Must have been the same on %s=%s" % dsp)
    else:
        ok_(
            np.asarray(ds) is not ds.samples,
            msg="Should have not been the same on %s=%s" % dsp,
        )
    ok_(
        np.array(ds) is not ds.samples,
...
def test_mergeds():
    ...
    assert_equal(merged.shape, (np.sum([len(ds) for ds in datasets]), data1.nfeatures))
    ...
    assert_equal(merged.shape, (len(data1), np.sum([ds.nfeatures for ds in datasets])))
...
def test_hstack():
    ...
    for fav in list(ds3dstacked.fa.values()):
        v = fav.value
        ok_(len(v) == nf3)
    ...
    for stacker in (vstacker, hstacker):
        for add_param in add_params:
            ...
            if add_param == 1:
                ...
                assert_equal(
                    set(r.a.keys()),
                    {"one", "two", "three", "four", "common", "array"},
                )
                ...
                assert_equal(set(r.a.keys()), {"common", "three", "four", "array"})
...
def test_stack_add_attributes():
    ...
    for xstack, colname, ocolname in ((vstack, "fa", "sa"), (hstack, "sa", "fa")):
...
def test_mergeds2():
    ...
    # now try adding pattern with wrong shape
    assert_raises(
        ValueError, vstack, (data, dataset_wizard(np.ones((2, 3)), targets=1, chunks=1))
    )
    ...
    assert_raises(ValueError, dataset_wizard, dss[:4, :5], targets=[1, 2, 3], chunks=2)

    # test wrong origin length
    assert_raises(
        ValueError, dataset_wizard, dss[:4, :5], targets=[1, 2, 3, 4], chunks=[2, 2, 2]
    )


def test_combined_samplesfeature_selection():
    data = dataset_wizard(
        np.arange(20).reshape((4, 5)).view(myarray),
        targets=[1, 2, 3, 4],
        chunks=[5, 6, 7, 8],
    )
...
def test_labelpermutation_randomsampling():
    ds = vstack(
        [
            Dataset.from_wizard(np.ones((5, 10)), targets=list(range(5)), chunks=i)
            for i in range(1, 6)
        ]
    )
...
def test_feature_masking():
    ...
    mask[2, 1] = True
    mask[4, 0] = True
    data = Dataset.from_wizard(
        np.arange(60).reshape((4, 5, 3)), targets=1, chunks=1, mask=mask
    )
...
def test_idhash():
    ...
    ok_(origid != ds.idhash, msg="Changing all targets should alter dataset's idhash")
    ...
    assert_equal(origid, ds.idhash, msg="Accessing shouldn't change idhash")
    z[2] = 333
    ok_(origid != ds.idhash, msg="Changing value in attribute should change idhash")
...
def test_arrayattributes():
    ...
    assert_raises(ValueError, ds.sa.__setitem__, "complex_list2", [[], [1], [1, 2]])


def test_repr():
    attr_repr = (
        "SampleAttribute(name='TestAttr', doc='my own test', "
        "value=array([0, 1, 2, 3, 4]), length=None)"
    )
    sattr = SampleAttribute(name="TestAttr", doc="my own test", value=np.arange(5))
    ...
    ds = Dataset(
        [[0, 1]], a={"dsa1": "v1"}, sa={"targets": [0]}, fa={"targets": ["b", "n"]}
    )
    ds_repr = repr(ds)
    ...
    if cfg_repr == "full":
        ...
        raise AssertionError(
            "Unknown kind of datasets.repr configuration %r" % cfg_repr
        )


def test_str():
    args = (np.arange(12, dtype=np.int8).reshape((4, 3)), list(range(4)), [1, 1, 2, 2])
...
def test_other_samples_dtypes():
    ...
    stypes = [
        np.arange(np.prod(dshape)).reshape(dshape),
    ...
    for s in stypes:
        ...
        if sparse.isspmatrix(ds.samples):
            ...
            assert_array_equal(
                np.mean(ds, axis=0), np.array(np.mean(ds.samples, axis=0)).squeeze()
            )
        ...
        if is_bsr(s):
            ...
            if sparse.isspmatrix(sel.samples):
                assert_array_equal(sel.samples[1].todense(), ds.samples[2].todense())
            else:
                assert_array_equal(sel.samples[1], ds.samples[2])
        ...
        if is_bsr(s):
            ...
            if sparse.isspmatrix(sel.samples):
                assert_array_equal(
                    sel.samples[:, 1].todense(), ds.samples[:, 2].todense()
                )
            else:
                assert_array_equal(sel.samples[:, 1], ds.samples[:, 2])
...
@sweepargs(
    ...
    + [
        ...
        dataset_wizard(
            np.arange(4), targets=["a", "bc", "a", "bc"], chunks=[1, 1, 2, 2]
        ),
        dataset_wizard(
            np.arange(4), targets=["a", "bc", "a", None], chunks=[1, 1, 2, 2]
        ),
    ]
)
...
def test_all_equal():
    ...
    a = np.random.normal(size=(10, 10)) + 1000.0
    b = np.zeros((10, 10))
    c = np.zeros(10)
    d = np.zeros(11)
    e = 0
    ...
    for ii, v in enumerate(values):
        for jj, w in enumerate(values):
            ...
            assert_equal(
                all_equal(copy.deepcopy(v), copy.deepcopy(w)),
                ii == jj,
...
def test_hollow_samples():
    sshape = (10, 5)
    ds = Dataset(
        HollowSamples(sshape, dtype=int),
...
def test_dataset_select_getitem():
    ds = Dataset(
        ...
        sa=dict(targets=list(range(5)), chunks=["a", "b", "a", "b", "a"]),
        fa=dict(voxel_indices=[[1, 2], [2, 1], [0, 0]], roi=["x", "x", "z"]),
    )
    ...
    for ds_ in (
        ...
        ds.select(None, fd),
        ds.select(fadict=fd),
        ds.select(fadict=fd, strict=False),
    ):
    ...
    ds_ = ds[{"chunks": ["b"]}, {"voxel_indices": [[0, 0], [1, 2]]}]
    assert ds_.shape == (2, 2)
    ...
    for strict in [True, False]:
        ...
        assert_raises(ValueError, cmp, voxel_indices, [[1, 2, 3]], strict=strict)
    assert_array_equal(
        cmp(voxel_indices, [[2, 1, 1], [2, 1, 1]]), [False, True, False, False, True]
    )
    ...
    assert_array_equal(
        cmp(voxel_indices, [[2, 1, 1], [2, 1, 999]], strict=False),
...
def test_attributes_match():
    col = FeatureAttributesCollection(
        {"roi": ["a", "b", "c", "a"], "coord": [[0, 1], [0, 2], [0, 1], [0, 2]]}
    )
    assert_array_equal(col.match(dict(roi=["a", "c"])), [True, False, True, True])
    assert_array_equal(
        col.match(dict(roi=["a", "c"], coord=[[0, 1]])), [True, False, True, False]
    )
    ...
    assert_array_equal(
        col.match(dict(roi=["a", "c", "xxx"]), strict=False), [True, False, True, True]
    )
