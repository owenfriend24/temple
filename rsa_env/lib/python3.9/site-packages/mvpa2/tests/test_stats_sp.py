...
class StatsTestsScipy(unittest.TestCase):
    ...
    def test_null_dist_prob_any(self):
        ...
        p_100 = null.p(100)
        self.assertAlmostEqual(null.p(-100), p_100)
        self.assertAlmostEqual(p100[0], p_100)
    ...
    def test_dataset_measure_prob(self, nd):
        ...
        self.assertTrue(
            (null_prob_nonbogus.samples < 0.05).all(),
        ...
        self.assertTrue(
            np.mean(np.abs(null_prob_bogus)) > np.mean(np.abs(null_prob_nonbogus))
        )
        ...
        if cfg.getboolean("tests", "labile", default="yes"):
            ...
            self.assertTrue(
                ...
                % (
                    ...
                    list(
                        zip(
                            m.ca.null_t[0, ds.a.bogus_features],
        ...
        for loc in [None, data_mean]:
            for test in ["p-roc", "kstest"]:
                ...
                matched = match_distribution(
                    ...
                    distributions=[
                        "scipy",
                        ("norm", {"name": "norm_fixed", "loc": 0.2, "scale": 0.3}),
                    ],
                    test=test,
                    loc=loc,
                    p=0.05,
    ...
    def test_match_distribution_semifrozen(self):
        ...
        matches = match_distribution(
            np.arange(10),
            distributions=["uniform", ("uniform", {"loc": 0})],
            p=-1,  # so we get all matches
        )
        self.assertEqual(len(matches), 2)  # we must get some match

        self.assertTrue(
            abs(matches[0][-1][0]) < 4e-1
        )  # full fit should get close to true loc
        ...
        if externals.versions["scipy"] >= "0.10":
            ...
            self.assertTrue(
                abs(matches[0][-1][1] - 9) < 1e-1
            )  # full fit should get close to true scale
        else:
    ...
    def test_r_dist_stability(self):
        ...
        try:
            ...
            self.fail(
                "Failed to compute rdist.pdf due to numeric"
        ...
        try:
            ...
            self.fail(
                "Failed due to bug which leads to InvalidIndex if only"
                " scalar is provided to cdf"
            )
        except Exception as e:
            self.fail(
                "Failed to compute rdist.cdf due to numeric"
                " loss of precision. Exception was %s" % e
            )
    ...
    def test_anova_compliance(self):
        ...
        fwm = OneWayAnova()
        f = fwm(ds)
        f_sp = f_oneway(ds[ds.targets == "L1"].samples, ds[ds.targets == "L0"].samples)
    ...
    def test_statsmodels(self):
        ...
        self.assertTrue(
            (tstats.samples[1] > 1000).all(), msg="constant tvalues should be huge"
        )

        if cfg.getboolean("tests", "labile", default="yes"):
            self.assertTrue(
                np.absolute(betas[0, 0]) > betas[0, 1],
...
def test_binomial_proportion_ci():
    ...
    matlab_truth = {
        "wald": (0.4020, 0.5980),
        "wilson": (0.4038, 0.5962),
        "agresti-coull": (0.4038, 0.5962),
        "jeffreys": (0.4032, 0.5968),
        "clopper-pearson": (0.3983, 0.6017),
        "arc-sine": (0.4026, 0.5974),
        "logit": (0.4032, 0.5968),
        "anscombe": (0.4037, 0.5963),
    }
    for m in list(matlab_truth.keys()):
        npt.assert_array_almost_equal(
            matlab_truth[m], binomial_proportion_ci(n, X, p, m), decimal=4, err_msg=m
        )
        # can deal with the extremes
        # those two have numerical limits
        if not m in ("clopper-pearson", "logit"):
            npt.assert_array_almost_equal(
                binomial_proportion_ci(1000, 1000, 0.05, m), [1, 1], decimal=2
            )
            npt.assert_array_almost_equal(
                binomial_proportion_ci(1000, 0, 0.05, m), [0, 0], decimal=2
            )
