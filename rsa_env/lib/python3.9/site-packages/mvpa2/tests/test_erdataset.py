...
def test_erdataset():
    ...
    samples = np.repeat(np.arange(nchunks * ntargets * blocklength), nfeatures).reshape(
        -1, nfeatures
    )
    ...
    assert_equal(
        sorted([a for a in ds.sa if not a.startswith("event")]), ["chunks", "targets"]
    )
    # samples as expected?
    assert_array_equal(erds.samples[0], np.repeat(np.arange(blocklength), nfeatures))
    ...
    assert_equal(len(erds), 1)
    assert_array_equal(
        erds.samples[0], np.repeat(np.arange(2, 2 + expected_nsamples), nfeatures)
    )
    assert_array_equal(erds.sa.orig_onset, [evs[0]["onset"]])
    assert_array_equal(erds.sa.orig_duration, [evs[0]["duration"]])
    ...
    assert_array_equal(
        erds.a.mapper.reverse1(results), results.reshape(expected_nsamples, nfeatures)
    )
    ...
    assert_array_equal(rds.sa.myattr, np.repeat(results.sa.myattr, expected_nsamples))
    ...
    # overide both
    evds = extract_boxcar_event_samples(ds, evs, event_offset=-2, event_duration=1)
    assert_equal(evds.shape, (len(evs), 1 * ds.nfeatures))
...
def test_hrf_modeling():
    ...
    # with centered data
    zscore(ds)
    evds_demean = eventrelated_dataset(
        ds,
        events,
        time_attr="time_coords",
        condition_attr="targets",
    ...
    assert evds.a.add_regs[0].samples.mean() > evds_demean.a.add_regs[0].samples.mean()
    # from eyeballing the sensitivity example -- would be better to test this on
    # the tutorial data
    assert (
        evds_demean[evds.sa.targets == "shoe"].samples.max()
    ...
    ds.sa["time_indices_powers"] = np.vstack(
        [ds.sa.time_indices ** 2, ds.sa.time_indices ** 3]
    ).T
    # custom regressors
    evds_regrs = eventrelated_dataset(
        ds,
    ...
    # comes last before constant
    assert_array_equal(
        ["negative_trend", "time_coords", "constant"],
