...
from mvpa2.support.nibabel.surf import (
...
class SurfingSurfaceTests(unittest.TestCase):
    ...
    def test_vector_alignment_find_rotation_illegal_inputs(self):
        ...
        for illegal_arg in illegal_args:
            assert_raises(
                (ValueError, IndexError), vector_alignment_find_rotation, *illegal_arg
            )
    ...
    def test_surface_face_normal(self, do_deterioriate_surface):
        ...
        for f_n in plane_face_normals:
            ...
            assert_almost_equal(
                f_n, surf.normalized(plane.nanmean_face_normal), decimal=0
            )
    ...
    @staticmethod
    def add_noise_to_surface(s, noise_level=0.05):
        vertices = s.vertices
        noise = np.random.uniform(
            size=vertices.shape,
            low=-0.5 * noise_level,
            high=-0.5 * noise_level,
        )
        vertices_noisy = vertices + np.random.uniform(size=vertices.shape) * noise_level

        return Surface(vertices_noisy, s.faces, check=False)

    @staticmethod
    def set_nan_to_surface_vertices(s, nan_ratio=0.05):
        # make some vertices NaN (as might be the case for flat surfaces)
        nan_count = int(np.ceil(s.nvertices * nan_ratio))
        nan_vertices_ids = np.random.random_integers(s.nvertices, size=(nan_count,)) - 1
        vertices_noisy = s.vertices + 0.0
        vertices_noisy[nan_vertices_ids, :] = np.nan

        return Surface(vertices_noisy, s.faces, check=False)

    @staticmethod
    def deterioriate_surface(s, noise_level=0.05, nan_ratio=0.05):
        s = SurfingSurfaceTests.add_noise_to_surface(s, noise_level=noise_level)
        s = SurfingSurfaceTests.set_nan_to_surface_vertices(s, nan_ratio=nan_ratio)
        return s

    @staticmethod
    def assert_coordinates_almost_equal_modulo_rotation(p_xyz, q_xyz, max_difference):
    ...
    def test_surface_flatten(self, dim):
        ...
        noise_level = 0.05
        nan_vertices_ratio = 0.05

        # add some noise to spatial coordinates
        vertices = plane.vertices
        noise = (
            np.random.uniform(size=vertices.shape, low=-0.5, high=0.5)
            * noise_level
            * scale
        )
        ...
        nan_vertices = np.random.random_integers(plane.nvertices, size=(nan_count,)) - 1
        ...
        # test average normal
        assert_array_almost_equal(plane.nanmean_face_normal, f_normal_avg, decimal=2)
    ...
    def test_flat_surface_plotting(self):
        side = 10
        step = 1 / float(side)
        plane = surf.generate_plane((0, 0, 0), (step, 0, 0), (0, step, 0), side, side)
        ...
        img_side = 50
        fsp = FlatSurfacePlotter(plane, min_nsteps=img_side, color_map=color_map)
        img_arr = fsp(data)
