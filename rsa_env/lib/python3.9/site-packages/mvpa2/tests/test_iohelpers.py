...
class IOHelperTests(unittest.TestCase):
    ...
    def test_samples_attributes(self):
        sa = SampleAttributes(
            pathjoin(pymvpa_dataroot, "attributes_literal.txt"), literallabels=True
        )
        ...
        ev = find_events(**sa)
        ok_(len(ev) == 17 * (max(sa.chunks) + 1), msg="Not all events got detected.")

        ok_(
            ev[0]["targets"] == ev[-1]["targets"] == "rest",
            msg="First and last event are rest condition.",
        )

        ok_(
            ev[-1]["onset"] + ev[-1]["duration"] == sa.nrows,
...
4 4.4 d"""
        ...
        assert_equal(set(attr.keys()), {"a", "b", "c"})
    ...
    def test_fsl_ev(self):
        ...
        # check header (sort because order in dict is unpredictable)
        self.assertTrue(sorted(d.keys()) == ["durations", "intensities", "onsets"])
    ...
    def test_fsl_ev2(self):
        ...
        # check header (sort because order in dict is unpredictable)
        self.assertTrue(sorted(attr.keys()) == ["chunks", "targets"])
    ...
    def test_bv_rtc(self):
        ...
        self.assertTrue(
            len(attr.r_60_B) == attr.nrows,
            "We must have got access to column by property",
        )
        self.assertTrue(
            attr.toarray() is not None, "We must have got access to column by property"
        )
    ...
    def testdesign2labels(self):
        ...
        labels = design2labels(attr, baseline_label="silence", func=lambda x: x > 0.5)
        Nsilence = lambda x: len(np.where(np.array(x) == "silence")[0])

        nsilence0 = Nsilence(labels0)
        nsilence = Nsilence(labels)
        self.assertTrue(
            nsilence0 < nsilence, "We must have more silence if thr is higher"
        )
        self.assertEqual(
            len(labels), attr.nrows, "We must have the same number of labels as rows"
        )
        self.assertRaises(
            ValueError,
            design2labels,
            attr,
            baseline_label="silence",
            func=lambda x: x > -1.0,
        )
    ...
    def test_read_fsl_design(self):
        fname = pathjoin(pymvpa_dataroot, "sample_design.fsf")
        # use our function
        ...
        set_lines = [x for x in read_lines if x.startswith("set ")]
