...
class StatsTests(unittest.TestCase):
    ...
    def test_null_dist_prob(self, null):
        ...
        if cfg.getboolean("tests", "labile", default="yes"):
            self.assertTrue(
                np.abs(prob[0]) < 0.05, msg="Expected small p, got %g" % prob[0]
            )
        if cfg.getboolean("tests", "labile", default="yes"):
            self.assertTrue(
                (np.abs(prob[1:]) > 0.05).all(),
    ...
    def test_anova(self):
        ...
        if cfg.getboolean("tests", "labile", default="yes"):
            ...
            self.assertTrue(
                (ac.samples[np.arange(4), np.array(ds.a.nonbogus_features)] >= 1).all()
            )
...
def test_tsboxplot():
    ...
    stats, outlier = compute_ts_boxplot_stats(
        ([ts, ts, ts, [0, 1, 2, 3, 30]]), outlier_thresh=1.0, greedy_outlier=True
    )
    ...
    assert_array_equal(np.sum(outlier.mask, axis=1), [len(ts), len(ts), len(ts), 0])
    # now for non-greedy outlier marking
    stats, outlier = compute_ts_boxplot_stats(
        ([ts, ts, ts, [0, 1, 2, 3, 30]]), outlier_thresh=1.0, greedy_outlier=False
    )
    ...
    stats, outlier = compute_ts_boxplot_stats(
        ([ts, ts, [0, np.nan, 2, 3, 30], ts]), outlier_thresh=1.0, greedy_outlier=False
    )
