...
class RFETests(unittest.TestCase):
    ...
    def test_n_back_history_stop_crit(self):
        ...
        self.assertTrue(stopcrit([1, 0.9, 0.8] + [0.9] * (stopcrit.steps - 1)) == False)
        # should stop if we got 10 more after minimal
        self.assertTrue(stopcrit([1, 0.9, 0.8] + [0.9] * stopcrit.steps) == True)
    ...
    def test_multi_stop_crit(self):
        ...
        stopcrit = MultiStopCrit(
            [FixedErrorThresholdStopCrit(0.5), NBackHistoryStopCrit(steps=4)]
        )
    ...
    def test_feature_selector(self):
        ...
        self.assertRaises(UnknownStateError, selector.ca.__getattribute__, "ndiscarded")
        ...
        self.assertTrue(
            (
                RangeElementSelector(lower=0)(data) == np.array([0, 1, 2, 3, 7, 8, 9])
            ).all()
        )

        self.assertTrue(
            (
                RangeElementSelector(lower=0, inclusive=True)(data)
                == np.array([0, 1, 2, 3, 5, 6, 7, 8, 9])
            ).all()
        )

        self.assertTrue(
            (
                RangeElementSelector(lower=0, mode="discard", inclusive=True)(data)
                == np.array([4])
            ).all()
        )
        ...
        # simple range 'below'
        self.assertTrue(
            (RangeElementSelector(upper=2)(data) == np.array([4, 5, 6])).all()
        )

        self.assertTrue(
            (
                RangeElementSelector(upper=2, inclusive=True)(data)
                == np.array([4, 5, 6, 7])
            ).all()
        )
        ...
        self.assertTrue(
            (
                RangeElementSelector(upper=2, mode="discard", inclusive=True)(data)
        ...
        # ranges
        self.assertTrue(
            (RangeElementSelector(lower=2, upper=9)(data) == np.array([0, 2, 3])).all()
        )

        self.assertTrue(
            (
                RangeElementSelector(lower=2, upper=9, inclusive=True)(data)
                == np.array([0, 2, 3, 7, 9])
            ).all()
        )
        ...
        self.assertTrue(
            (
                RangeElementSelector(upper=2, lower=9, mode="discard", inclusive=True)(
                    data
                )
                == RangeElementSelector(lower=2, upper=9, inclusive=False)(data)
            ).all()
        )

        # non-0 elements -- should be equivalent to np.nonzero()[0]
        self.assertTrue((RangeElementSelector()(data) == np.nonzero(data)[0]).all())
    ...
    def test_feature_selection_pipeline(self):
        ...
        feature_selections = [
            SensitivityBasedFeatureSelection(sens_ana, FractionTailSelector(0.25)),
            SensitivityBasedFeatureSelection(sens_ana, FixedNElementTailSelector(4)),
        ]
    ...
    def test_rfe(self, clf):
        ...
        pmeasure = ProxyMeasure(
            clf, postproc=BinaryFxNode(mean_mismatch_error, "targets")
        )
        cvmeasure = CrossValidation(
            clf, NFoldPartitioner(), errorfx=mean_mismatch_error, postproc=mean_sample()
        )
        ...
        for rfe, data in [
            ...
            self.assertTrue(
                set(rfe.ca.history) == set(range(len(np.array(rfe.ca.errors))))
            )
            ...
            self.assertTrue(
                rfe.ca.nfeatures[-1]
                == len(np.where(rfe.ca.history == max(rfe.ca.history))[0])
            )
    ...
    def test_james_problem(self):
        ...
        fs = RFE(
            rfesvm_split.get_sensitivity_analyzer(),
            ProxyMeasure(
                rfesvm_split, postproc=BinaryFxNode(mean_mismatch_error, "targets")
            ),
            Splitter("train"),
            fselector=FractionTailSelector(
                percent / 100.0, mode="select", tail="upper"
            ),
            update_sensitivity=True,
        )
        ...
        class StoreResults(object):
            ...
            def __call__(self, data, node, result):
                self.storage.append(
                    (node.measure.mapper.ca.history, node.measure.mapper.ca.errors)
                ),
        ...
        try:
            ...
            self.fail(
                "CrossValidation cannot handle classifier with RFE "
    ...
    def test_SplitRFE(self, fmeasure):
        ...
        dataset = normal_feature_dataset(
            perlabel=20, nlabels=2, nfeatures=11, snr=1.0, nonbogus_features=[1, 5]
        )
        ...
        ok_(
            len(set(dataset.a.nonbogus_features).intersection(rfeclf.mapper.slicearg))
            > 0
        )
        ...
        s = str(rfeclf)
        ok_(
            ("partitioner=NFoldP" in r)
            or ("partitioner=mvpa2.generators.partition.NFoldPartitioner" in r)
        )
        ok_("lrn=" in r)
        ...
        cv = CrossValidation(
            rfeclf, NFoldPartitioner(count=1), callback=store_me, enable_ca=["stats"]
        )
        _ = cv(dataset)
