...
class RegressionsTests(unittest.TestCase):
    ...
    def test_non_regressions(self, ml):
        ...
        self.assertTrue(
            ("binary" in ml.__tags__) != ml.__is_regression__,
    ...
    def test_regressions_classifiers(self, clf):
        ...
        cv = CrossValidation(
            clf, NFoldPartitioner(), enable_ca=["stats", "training_stats"]
        )
    ...
    def test_sensitivities(self, regr):
        ...
        """
        myds = dataset_wizard(
            samples=np.random.normal(size=(10, 5)), targets=np.random.normal(size=10)
        )
