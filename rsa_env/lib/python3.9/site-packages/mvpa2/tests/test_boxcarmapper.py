...
def test_simpleboxcar():
    ...
    assert_equal((np.prod(ds.shape[:2]),) + singlesample.shape[1:], m.reverse(ds).shape)
...
def test_datasetmapping():
    ...
    ds = Dataset(
        data,
        sa={"timepoints": np.arange(6), "multidim": data.copy()},
        fa={"fid": np.arange(4)},
    )
    ...
    assert_equal(mds.sa.multidim.shape, (len(startpoints), boxlength) + ds.shape[1:])
    assert_equal(mds.sa.timepoints.shape, (len(startpoints), boxlength))
    assert_array_equal(
        mds.sa.timepoints.flatten(),
