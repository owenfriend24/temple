...
data = np.array(
    [
        [0.22366105, 0.51562476, 0.62623543, 0.28081652, 0.56513533],
...
def test_PDistConsistency():
    ...
    center = squareform(
        np.corrcoef(pdist(dc1, "correlation"), pdist(dc2, "correlation")), checks=False
    ).reshape((1, -1))
    dsm1 = stats.rankdata(pdist(data[0:3, :], "correlation").reshape((1, -1)))
    dsm2 = stats.rankdata(pdist(data[3:6, :], "correlation").reshape((1, -1)))

    spearman = squareform(np.corrcoef(np.vstack((dsm1, dsm2))), checks=False).reshape(
        (1, -1)
    )
    ...
    for i, p in enumerate(res4.sa.pairs):
        sqval = np.asscalar(
            res4_sq[res4_sq.sa.chunks == p[0], res4_sq.sa.chunks == p[1]]
        )
...
def test_CDist():
    ...
    for sattr in [["targets"], None]:
        for metric in metrics:
            metric_kwargs = {"VI": VI_mahalanobis} if metric == "mahalanobis" else {}
            scipy_cdist = cdist(
                train_data.samples, test_data.samples, metric, **metric_kwargs
            )
            scipy_pdist = pdist(train_data.samples, metric, **metric_kwargs)
            pymvpa_cdist = CDist(
            ...
            assert_array_almost_equal(res_cv.samples.ravel(), scipy_cdist.ravel())
            # if called with train_data again, results should match with pdist
            assert_array_almost_equal(
                res_nocv.samples.ravel(), squareform(scipy_pdist).ravel()
            )
...
def test_PDistTargetSimilarity():
    ...
    tdcm2 = PDistTargetSimilarity(tdsm, pairwise_metric="euclidean")
    tdcm3 = PDistTargetSimilarity(tdsm, comparison_metric="spearman")
    tdcm4 = PDistTargetSimilarity(tdsm, corrcoef_only=True)
    a1 = tdcm1(ds)
    a2 = tdcm2(ds)
    a3 = tdcm3(ds)
    a4 = tdcm4(ds)
...
def test_Regression():
    ...
    coefs = regr(ds)
    assert_almost_equal(coefs.samples, 1.0)
    ...
    outputs = [
        ...
        np.array([[0.76665188], [0.0], [0.0]]),
        np.array([[0.5], [0.0], [1.75]]),
        np.array([[0.92307692], [0.0], [0.26923077]]),
        np.array([[0.0], [0.0], [3.70074342e-17]]),
        np.array([[8.57142857e-01], [0.0], [-2.64338815e-17]]),
        np.array([[0.0], [0.0], [1.33333333]]),
        np.array([[0.84210526], [0.0], [0.21052632]]),
        np.array([[0.0], [0.0]]),
        np.array([[0.76665188], [0.0]]),
        np.array([[0.92982456], [0.0]]),
        np.array([[0.92850288], [0.07053743]]),
        np.array([[0.0], [0.0]]),
        np.array([[0.85714286], [0.0]]),
        np.array([[0.625], [0.0]]),
        np.array([[0.87272727], [0.14545455]]),
    ]

    for i, (fit_intercept, rank_data, normalize, reg_type) in enumerate(
        product([True, False], [True, False], [True, False], reg_types)
    ):
        regr = Regression(
            predictors,
            alpha=1,
