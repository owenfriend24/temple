...
class PrototypeMapperTests(unittest.TestCase):
    ...
    def build_vector_based_pm(self):
        ...
        self.pm = PrototypeMapper(
            similarities=self.similarities, prototypes=self.prototypes
        )
        # train Prototype
    ...
    def test_size(self):
        ...
        assert_array_equal(
            self.pm.proj.shape,
            (self.samples.shape[0], self.prototypes.shape[0] * len(self.similarities)),
        )

    def test_symmetry(self):
        self.build_vector_based_pm()
        assert_array_almost_equal(
            self.pm.proj[:, self.samples.shape[0]],
    ...
    def test_size_random_prototypes(self):
        ...
        self.pm2 = PrototypeMapper(
            similarities=self.similarities, prototypes=self.prototypes2
        )
        ...
        assert_array_equal(
            ...
            (
                ...
                self.pm2.prototypes.shape[0] * len(self.similarities),
            ),
        )
    ...
    def build_streamline_things(self):
        ...
        self.streamline_samples = np.array(
            [np.random.rand(3, 3), np.random.rand(5, 3), np.random.rand(7, 3)],
            dtype="object",
        )
    ...
    def test_streamline_equal_mapper(self):
        ...
        assert_array_equal(
            ...
            (
                ...
                len(self.prototypes_equal) * len(self.similarities),
            ),
        )
        # test symmetry
    ...
    def test_streamline_random_mapper(self):
        ...
        ## debug("MAP","Generating "+str(prototype_number)+" random prototypes.")
        self.prototypes_random = self.dataset.samples[
            np.random.permutation(self.dataset.samples.size)
        ][:prototype_number]
        ## debug("MAP","prototypes: "+str(self.prototypes_random))

        self.pm = PrototypeMapper(
            similarities=self.similarities,
            prototypes=self.prototypes_random,
            demean=False,
        )
        ...
        assert_array_equal(
            ...
            (
                len(self.dataset.samples),
                len(self.prototypes_random) * len(self.similarities),
            ),
        )
