...
from mvpa2.measures.gnbsearchlight import sphere_gnbsearchlight, GNBSearchlight
from mvpa2.clfs.gnb import GNB
from mvpa2.clfs.distance import one_minus_correlation

from mvpa2.measures.nnsearchlight import sphere_m1nnsearchlight, M1NNSearchlight
...
class SearchlightTests(unittest.TestCase):
    ...
    def test_searchlights_doc_qe(self):
        ...
        for sl in (sphere_searchlight, sphere_gnbsearchlight, sphere_m1nnsearchlight):
            for kw in ("queryengine", "qe"):
                ok_(
                    not kw in sl.__doc__,
        ...
        for sl in (Searchlight, GNBSearchlight, M1NNSearchlight):
            for kw in ("queryengine",):
                ok_(
                    kw in sl.__init__.__doc__,
                    msg="There should be %r in %s.__init__.__doc__" % (kw, sl),
                )
            for kw in ("qe",):
                ok_(
                    not kw in sl.__init__.__doc__,
    ...
    @sweepargs(
        lrn_sllrn_SL_partitioner=[
            (
                ...
                0.0,  # correction for the error range
            )
            for v in (True, False)
        ]
        +
        ...
        [
            (
                ChainMapper(
                    [mean_group_sample(["targets", "partitions"]), kNN(1)],
                    space="targets",
                    descr="M1NN",
                ),
                kNN(1),
    ...
    @reseed_rng()
    def test_spatial_searchlight(
        self, lrn_sllrn_SL_partitioner, do_roi=False, results_backend="native"
    ):
        ...
        if (
            __debug__
            and "ENFORCE_CA_ENABLED" in debug.active
        ...
        skwargs = dict(
            radius=1, enable_ca=["roi_sizes", "raw_results", "roi_feature_ids"]
        )
        ...
        for indexsum in indexsums:
            sl = SL(
                sllrn, partitioner, indexsum="fancy", reuse_neighbors=True, **skwargs
            )
            mvpa2.seed()
            result1 = sl(ds)
            mvpa2.seed()
    ...
    def test_adhocsearchlight_perm_testing(self):
        ...
        slargs = (kNN(1), NFoldPartitioner(0.5, selection_strategy="random", count=9))
        ...
        assert_array_equal(distr_est.ca.dist_samples.shape, (1, ds.nfeatures, 8))
    ...
    def test_gnbsearchlight_matchaccuracy(self):
        ...
        ds = datasets["3dmedium"].copy()
        ds.fa["voxel_indices"] = ds.fa.myspace
        sl_err = sphere_gnbsearchlight(GNB(), NFoldPartitioner(cvtype=1), radius=0)
        sl_acc = sphere_gnbsearchlight(
            GNB(), NFoldPartitioner(cvtype=1), radius=0, errorfx=mean_match_accuracy
        )
    ...
    def test_partial_searchlight_with_full_report(self):
        ...
        sl = sphere_searchlight(
            lambda x: np.vstack((x.fa.roi_seed, x.samples)),
            radius=1,
            add_center_fa=True,
            center_ids=[12],
        )
        res = sl(ds)
        assert_array_equal(
            res.samples[1:, res.samples[0].astype("bool")].squeeze(), ds.samples[:, 12]
        )
    ...
    def test_add_center_fa(self):
        ...
        for n, check in [
            (HollowSphere(1, 0), check_no_center),
            (Sphere(0), check_center),
            (Sphere(1), check_center),
        ]:
            Searchlight(check, IndexQueryEngine(myspace=n), add_center_fa="center")(ds)
    ...
    def test_partial_searchlight_with_confusion_matrix(self):
        ...
        sl = sphere_searchlight(
            ...
            radius=1,
            center_ids=[3, 5, 50],
            null_dist=MCNullDist(permutator, tail="right", enable_ca=["dist_samples"]),
        )
        res_perm = sl(ds)
        ...
        assert_equal(
            sl.null_dist.ca.dist_samples.shape, res_perm.shape + (no_permutations,)
        )
    ...
    def test_chi_square_searchlight(self):
        ...
        cv = CrossValidation(sample_clf_lin, NFoldPartitioner(), enable_ca=["stats"])
    ...
    def test_1d_multispace_searchlight(self):
        ...
        res = Searchlight(
            measure, IndexQueryEngine(coord1=Sphere(0), coord2=Sphere(0)), nproc=1
        )(ds)
        assert_array_equal(res.samples, [["0", "1", "2", "3", "4", "5"]])
        res = Searchlight(
            measure, IndexQueryEngine(coord1=Sphere(0), coord2=Sphere(1)), nproc=1
        )(ds)
        assert_array_equal(res.samples, [["0+1", "0+1", "2+3", "2+3", "4+5", "4+5"]])
        res = Searchlight(
            measure, IndexQueryEngine(coord1=Sphere(1), coord2=Sphere(0)), nproc=1
        )(ds)
        assert_array_equal(
            res.samples, [["0+2", "1+3", "0+2+4", "1+3+5", "2+4", "3+5"]]
        )
    ...
    def test_regression_with_additional_sa(self):
        ...
        cvbeh = CrossValidationWithBeh(regr, OddEvenPartitioner(), errorfx=corr_error)
        # regular cv
        ...
        assert_array_lequal(slmapbeh.samples[:, features], slmap.samples[:, features])
    ...
    def test_swaroop_case(self, preallocate_output):
        ...
        class sw_measure(Measure):
            ...
            def _call(self, dataset):
                ...
                return Dataset(
                    np.array(
                        [{"d": np.ones(shape=(5, 5)) * np.sum(dataset)}], dtype=object
                    )
                )
        ...
        for backend in ["native"] + (externals.exists("h5py") and ["hdf5"] or []):
            sl = sphere_searchlight(
                sw_measure(),
                radius=1,
        ...
        if not externals.exists("h5py"):
            self.assertRaises(
                RuntimeError, sphere_searchlight, sw_measure(), results_backend="hdf5"
            )
    ...
    def test_nblocks(self, preallocate_output):
        ...
        res1 = sphere_searchlight(
            cv, radius=1, nproc=2, preallocate_output=preallocate_output
        )(ds)
        res2 = sphere_searchlight(
            cv, radius=1, nproc=2, nblocks=5, preallocate_output=preallocate_output
        )(ds)
    ...
    def test_custom_results_fx_logic(self):
        ...
        def measure(ds):
            ...
            # processed
            t0 = time.time()
            while os.path.exists(f) and time.time() - t0 < 4.0:
                time.sleep(0.5)  # so it does take time to compute the measure
                pass
            if os.path.exists(f):
                print_("ERROR: ", f)
                raise AssertionError("File %s must have been processed by now" % f)
