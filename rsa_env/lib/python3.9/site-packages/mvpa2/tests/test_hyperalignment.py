...
class HyperAlignmentTests(unittest.TestCase):
    ...
    @reseed_rng()
    def test_basic_functioning(
        self, ref_ds, zscore_common, zscore_all, level1_equal_weight
    ):
        ha = Hyperalignment(
            ref_ds=ref_ds,
        ...
        for noisy, dss in ((False, dss_rotated_clean), (True, dss_rotated)):
            ...
            ndcss = []
            ds_orig_Rref = (
                np.dot(ds_orig.samples, Rs[ref_ds]) * random_scales[ref_ds]
            ...
            for ds_back in dss_clean_back:
                ...
                # corrcoef
                ndcs = np.diag(
                    np.corrcoef(ds_back.samples.T, ds_orig_Rref.T)[nf:, :nf], k=0
                )
                ndcss += [ndcs]
    ...
    def test_hpal_svd_combo(self):
        ...
        dss_rotated = [random_affine_transformation(dss_rotated[i]) for i in range(n)]
        ...
        dss_back = [m.forward(ds_) for m, ds_ in zip(mappers_orig, dss_rotated)]
        ...
        for sd in dss_back:
            ndcs = np.diag(
                np.corrcoef(sd.samples.T, ds_orig.samples.T)[nf_true:, :nf_true], k=0
            )
    ...
    def test_hypal_michael_caused_problem(self):
        ...
        assert (
            np.corrcoef(ds_test[2], ds_test[1])[0, 1] < 0.99
    ...
    def _test_on_swaroop_data(self):  # pragma: no cover
        ...
        for sub in subj:
            ds.append(
                fmri_dataset(
                    samples=sub + "_movie.nii.gz", mask=sub + "_mask_vt.nii.gz"
                )
            )
        ...
        for i, sd in enumerate(ds):
            ...
            for j, sd2 in enumerate(ds[i + 1 :]):
                ...
                feature_scores[i] = feature_scores[i] + np.max(corr_temp, axis=1)
                feature_scores[j + i + 1] = feature_scores[j + i + 1] + np.max(
                    corr_temp, axis=0
                )
        ...
        for sub in subj:
            mkdg_ds.append(
                fmri_dataset(
                    samples=sub + "_mkdg.nii.gz",
                    targets=md_labels,
                    chunks=np.repeat(list(range(8)), 192),
        ...
        cvterr = CrossValidation(clf, NFoldPartitioner(), enable_ca=["confusion"])
