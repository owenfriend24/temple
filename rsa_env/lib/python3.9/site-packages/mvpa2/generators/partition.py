...
class Partitioner(Node):
    ...
    def _set_selection_strategy(self, strategy):
        ...
        if not strategy in self._STRATEGIES:
            raise ValueError(
                "selection_strategy is not known. Known are %s" % str(self._STRATEGIES)
            )
    ...
    def get_partitions_attr(self, ds, specs):
        ...
        for spec in specs:
            if spec is None:
                ...
                filter_ = np.array([i in spec for i in splitattr_data], dtype="bool")
        ...
        if none_specs > 1:
            raise ValueError(
                "'%s' cannot handle more than one `None` "
    ...
    def get_selected_indexes(self, n_cfgs):
        ...
        if __debug__:
            debug(
                "SPL",
                "For %s selection strategy selected %s "
    ...
    selection_strategy = property(
        fget=lambda self: self.__selection_strategy, fset=_set_selection_strategy
    )
...
class OddEvenPartitioner(Partitioner):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(OddEvenPartitioner, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["usevalues"], default=False)
        )
    ...
    def _get_partition_specs(self, uniqueattrs):
        ...
        if self.__usevalues:
            return [
                (None, uniqueattrs[(uniqueattrs % 2) == True]),
...
class NGroupPartitioner(Partitioner):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(NGroupPartitioner, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["ngroups"], default=4)
        )
    ...
    def _get_partition_specs(self, uniqueattrs):
        ...
        if len(uniqueattrs) < self.__ngroups:
            raise ValueError(
                "Number of groups (%d) " % self.__ngroups
                + "must be less than "
        ...
        split_list = [
            (None, uniqueattrs[split_ind == i]) for i in range(self.__ngroups)
        ]
        return split_list
...
class CustomPartitioner(Partitioner):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(CustomPartitioner, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["splitrule"])
        )
...
class NFoldPartitioner(Partitioner):
    ...
    def __init__(self, cvtype=1, **kwargs):
        ...
        if isinstance(cvtype, float):
            ...
            if not (0 < cvtype < 1):
                raise ValueError(
                    "Float value for cvtype must be within range "
                    "(0, 1), excluding boundaries. Got %r." % cvtype
                )
        self.cvtype = cvtype
    ...
    def __repr__(self, prefixes=None):  # pylint: disable-msg=W0102
        ...
        return super(NFoldPartitioner, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["cvtype"], default=1)
        )
    ...
    def _get_partition_specs(self, uniqueattrs):
        ...
        if (
            ...
            combs = support.xrandom_unique_combinations(uniqueattrs, n, self.count)
        ...
        if self.count is None or self.selection_strategy != "random":
            ...
            # right away
            return [(None, i) for ind, i in enumerate(combs) if ind < self.count]
...
class FactorialPartitioner(Partitioner):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(FactorialPartitioner, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["partitioner"], default=1)
        )
    ...
    def generate(self, ds):
        ...
        all_partitionings = [
            self.partitioner.generate(fakeds[uattr_mask]) for uattr_mask in uattr_masks
        ]
...
class ExcludeTargetsCombinationsPartitioner(Node):
    ...
    def generate(self, ds):
        ...
        for combination in support.xunique_combinations(utargets, self.k):
            ...
            partitioning[
                np.logical_and(testing_part, combination_nonmatches)
            ] = self.partition_assign
            partitioning[
                np.logical_and(nontesting_part, combination_matches)
            ] = self.partition_assign
            pds = ds.copy(deep=False)
