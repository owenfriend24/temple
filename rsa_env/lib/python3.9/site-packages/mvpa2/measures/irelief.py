...
class IterativeRelief_Devel(FeaturewiseMeasure):
    ...
    def __init__(
        self, threshold=1.0e-2, kernel=None, kernel_width=1.0, w_guess=None, **kwargs
    ):
    ...
    def _call(self, dataset):
        ...
        while True:
            ...
            for n in range(NS):
                ...
                gamma_n = 1.0 - np.nan_to_num(
                    d_w_k[n, M[n]].sum() / (d_w_k[n, :].sum() - d_w_k[n, n])
                )
                alpha_n = np.nan_to_num(d_w_k[n, M[n]] / (d_w_k[n, M[n]].sum()))
                beta_n = np.nan_to_num(d_w_k[n, H[n]] / (d_w_k[n, H[n]].sum()))

                m_n = (np.abs(samples[n, :] - samples[M[n], :]) * alpha_n[:, None]).sum(
                    0
                )
                h_n = (np.abs(samples[n, :] - samples[H[n], :]) * beta_n[:, None]).sum(
                    0
                )
                ni += gamma_n * (m_n - h_n)
            ni = ni / NS
    ...
    def _call(self, dataset):
        ...
        while change > self.threshold and iteration < self.max_iter:
            ...
            for t in range(NS):
                ...
                d_w_k_xn_Mn = (
                    self.k.computed(
                        dataset.samples[None, n, :], dataset.samples[M[n], :]
                    )
                    .as_raw_np()
                    .squeeze()
                )
                ...
                d_w_k_xn_x = (
                    self.k.computed(dataset.samples[None, n, :], dataset.samples)
                    .as_raw_np()
                    .squeeze()
                )
                ...
                d_w_k_xn_Hn = (
                    self.k.computed(
                        dataset.samples[None, n, :], dataset.samples[H[n], :]
                    )
                    .as_raw_np()
                    .squeeze()
                )
                ...
                m_n = (
                    np.abs(dataset.samples[n, :] - dataset.samples[M[n], :])
                    * alpha_n[:, np.newaxis]
                ).sum(0)
                h_n = (
                    np.abs(dataset.samples[n, :] - dataset.samples[H[n], :])
                    * beta_n[:, np.newaxis]
                ).sum(0)
                pi = gamma_n * (m_n - h_n)
