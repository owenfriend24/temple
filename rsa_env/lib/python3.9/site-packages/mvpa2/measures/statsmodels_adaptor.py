...
class UnivariateStatsModels(FeaturewiseMeasure):
    ...
    def __init__(self, exog, model_gen, res="params", add_constant=True, **kwargs):
        """
        Parameters
        ----------
        exog : array-like
    ...
    def __fitmodel1d(self, Y):
        ...
        if isinstance(res, np.ndarray):
            if len(res.shape) == 1:
                ...
                return [np.asscalar(i) for i in [fstats.fvalue, fstats.pvalue]] + [
                    fstats.df_num,
                    fstats.df_denom,
                ]
            else:
                raise ValueError(
                    "Test specification (via `res`) has to be 1d or 2d array"
                )
