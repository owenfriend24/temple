...
class WinnerMeasure(Measure):
    ...
    def __init__(self, axis, fx, other_axis_prefix=None, **kwargs):
        ...
        if type(axis) is str:
            ...
            if not axis in str2num:
                raise ValueError("Illegal axis: should be %s" % " or ".join(str2num))
            axis = str2num[axis]
    ...
    def __repr__(self, prefixes=None):
        if prefixes is None:
            prefixes = []
        prefixes_ = [
            "axis=%r,fx=%r,other_axis_prefix=%r"
    ...
    def _call(self, ds):
        ...
        # copy each attribute
        for k, v in list(to_copy[0].items()):
        ...
        for k, v in list(src.items()):
            ...
            if prek in trg:
                raise KeyError("Key clash: %s already in %s" % (prek, to_copy[1]))
...
if __name__ == "__main__":
    ...
    n = ns * nf
    ds = Dataset(
        np.reshape(np.mod(np.arange(0, n * 5, 5) + 0.5 * n, n), (ns, nf)),
        sa=dict(targets=[0, 0, 1, 1], x=[3, 2, 1, 0]),
        fa=dict(v=[3, 2, 1], w=["a", "b", "c"]),
    )

    measures2out = {
        feature_winner_measure: [1, 0, 2, 1],
    ...
    for m, out in list(measures2out.items()):
