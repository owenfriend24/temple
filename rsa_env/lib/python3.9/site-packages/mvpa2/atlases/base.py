...
class XMLBasedAtlas(BaseAtlas):
    ...
    def __init__(
        ...
        if not {"header", "data"} == set([i.tag for i in self.getchildren()]):
            raise IOError("No header or data were defined in %s" % filename)

        header = self.header
        headerChildrenTags = XMLBasedAtlas._children_tags(header)
        if not ("images" in headerChildrenTags) or not (
            "imagefile" in XMLBasedAtlas._children_tags(header.images)
        ):
    ...
    def _check_range(self, c):
        ...
        if not check_range(c, self.extent):
            warning(
                "Coordinates %r are not within the extent %r."
    ...
    @property
    def version(self):
        if self.__atlas is not None and ("version" in list(self.__atlas.attrib.keys())):
    ...
    def set_coordT(self, coordT):
        ...
        if self._image is not None:
            ...
            coordT = Linear(
                np.linalg.inv(self._image.header.get_qform()), previous=coordT
            )
        self._spaceT = SpaceTransformation(previous=coordT, to_real_space=False)
    ...
    def levels_listing(self):
        ...
        return "\n".join(["%d: " % k + str(self._levels[k]) for k in lkeys])
    ...
    def _get_selected_levels(self, levels=None):
        ...
        """
        if levels is None:
            levels = [i for i in range(self.nlevels)]
            ...
            levels = [i for i in range(start, stop, step)]
            ...
            raise TypeError('Given levels "%s" are of unsupported type' % repr(levels))
    ...
    coordT = property(fget=lambda self: self._spaceT, fset=set_coordT)
...
class Level(object):
    ...
    def __repr__(self):
        return "%s Level: %s" % (self.level_type, self.description)
        ...
        if level_type is None:
            if not "type" in Elevel.attrib:
                raise XMLAtlasException(
                    "Level must have type specified. Level: " + repr(Elevel)
                )
            level_type = Elevel.get("type")

        levelTypes = {"label": LabelsLevel, "reference": ReferencesLevel}
...
class LabelsLevel(Level):
    ...
    def __repr__(self):
        return Level.__repr__(self) + " [%d] " % self.__index
    ...
    def from_xml(Elevel, levelIndex=None):
        ...
        maxindex = max([int(i.get("index")) for i in Elevel.label[:]])
        labels = [None for i in range(maxindex + 1)]
        ...
        return LabelsLevel(Elevel.get("description"), index, labels)
    ...
    def find(self, target, unique=True):
        ...
        if unique:
            if len(res) != 1:
                raise ValueError(
                    "Got %d matches whenever just 1 was "
...
class ReferencesLevel(Level):
    ...
    def from_xml(Elevel):
        ...
        return ReferencesLevel(Elevel.get("description"), indexes)
...
class PyMVPAAtlas(XMLBasedAtlas):
    ...
    def __init__(self, *args, **kwargs):
        ...
        header = self.header
        headerChildrenTags = XMLBasedAtlas._children_tags(header)
        if not ("space" in headerChildrenTags) or not (
            "space-flavor" in headerChildrenTags
        ):
            raise XMLAtlasException(
                "PyMVPA Atlas requires specification of"
    ...
    def _load_images(self):
        ...
        if len(self._data.shape[0:-4]) > 0:
            ...
            if max(bogus_dims) > 1:
                raise RuntimeError(
                    "Atlas %s has more than 4 of non-singular"
...
class LabelsAtlas(PyMVPAAtlas):
    ...
    def label_voxel(self, c, levels=None):
        ...
        for level in levels:
            ...
            resultIndex = int(self._data[level_.index, c[0], c[1], c[2]])
...
class ReferencesAtlas(PyMVPAAtlas):
    ...
    def __init__(self, distance=0, reference_level=None, *args, **kwargs):
        ...
        if (
            self.__referenceAtlas.space != self.space
            or self.__referenceAtlas.space_flavor != self.space_flavor
        ):
            raise XMLAtlasException(
    ...
    def label_voxel(self, c, levels=None):
        ...
        if self.__referenceLevel is None:
            warning(
                "You did not provide what level to use "
                "for reference. Assigning 0th level -- '%s'" % (self._levels[0],)
            )
        ...
        if __debug__:
            debug(
                "ATL__",
                "Closest referenced point for %r is "
    ...
    reference_level = property(
        fget=lambda self: self.__referenceLevel, fset=set_reference_level
    )
