...
class FSLAtlas(XMLBasedAtlas):
    ...
    def _load_images(self):
        ...
        for imagefilename in imagefile_candidates:
            ...
            if resolution is None:
                ...
                if ni_image is None or resolution_ < ni_image.header.get_zooms()[0]:
                    ni_image = ni_image_
        ...
        if ni_image is None:
            msg = "Could not find an appropriate atlas among %d atlases." % len(
                imagefile_candidates
            )
            if resolution is not None:
                msg += " Atlases had resolutions %s" % (resolutions,)
    ...
    def _check_version(version):
        return re.search(r"^[0-9]+\.[0-9]", version) is not None
...
class FSLProbabilisticAtlas(FSLAtlas):
    ...
    def label_voxel(self, c, levels=None):
        ...
        if self.sort or self.strategy == "max":
            resultLabels.sort(cmp=lambda x, y: cmp(x["prob"], y["prob"]), reverse=True)
    ...
    def get_map(self, target, strategy="unique", axes_order="xyz"):
        ...
        if isinstance(target, int):
            ...
            if strategy == "unique":
                return self.get_map(
                    lev.find(target, unique=True).index, axes_order=axes_order
                )
            else:
    ...
    def get_maps(self, target, axes_order="xyz", key_attr=None, overlaps=None):
        ...
        res = [
            [key_gen(l), self.get_map(l.index, axes_order=axes_order)]
