...
class MDPNodeMapper(Mapper):
    ...
    def __init__(self, node, nodeargs=None, **kwargs):
        ...
        if (
            externals.versions["mdp"] >= (2, 5) and node.has_multiple_training_phases()
        ) or not len(node._train_seq) == 1:
            raise ValueError(
                "MDPNodeMapper does not support MDP nodes with "
    ...
    def __repr__(self):
        ...
        return s.replace(
            "(", "(node=%s, nodeargs=%s, " % (repr(self.node), repr(self.nodeargs)), 1
        )
...
class PCAMapper(MDPNodeMapper):
    ...
    def __init__(self, alg="PCA", nodeargs=None, **kwargs):
        ...
        **kwargs
        ...
        if alg == "PCA":
            ...
            raise ValueError("Unkown algorithm '%s' for PCAMapper." % alg)
        MDPNodeMapper.__init__(self, node, nodeargs=nodeargs)

    proj = property(
        fget=lambda self: self.node.get_projmatrix(),
    ...
    centroid = property(
        fget=lambda self: self.node.avg, doc="Mean of the training data"
    )
    ...
    def __init__(self, alg="FastICA", nodeargs=None, **kwargs):
        ...
        **kwargs
        ...
        if alg == "FastICA":
            ...
            raise ValueError("Unkown algorithm '%s' for ICAMapper." % alg)
        MDPNodeMapper.__init__(self, node, nodeargs=nodeargs)
...
class MDPFlowMapper(Mapper):
    ...
    def __init__(self, flow, node_arguments=None, **kwargs):
        ...
        if node_arguments is not None and len(node_arguments) != len(flow):
            raise ValueError(
                ...
                "number of nodes in the flow (%i)." % (len(node_arguments), len(flow))
            )
        Mapper.__init__(self, **kwargs)
    ...
    def __repr__(self):
        ...
        return s.replace(
            "(",
            "(flow=%s, node_arguments=%s, "
            % (repr(self.flow), repr(self.node_arguments)),
            1,
        )
