...
def _get_indexes(shape, dim):
    ...
    """
    if len(shape) < dim:
        raise ValueError("Dimension %d is incorrect for a shape %s" % (dim, shape))
    n = len(shape)
    curindexes = [0] * n
...
class WaveletPacketMapper(_WaveletMapper):
    ...
    def __forward_single_level(self, data):
        ...
        for indexes in _get_indexes(data.shape, self._dim):
            ...
            WP = pywt.WaveletPacket(
                data[indexes], wavelet=wavelet, mode=mode, maxlevel=level
            )
            ...
            if wp is None:
                ...
                if __debug__:
                    debug(
                        "MAP_",
                        "Initializing storage of size %s for single "
                        "level (%d) mapping of data of size %s"
                        % (newdim, level, data.shape),
                    )
    ...
    def __reverse_single_level(self, wp):
        ...
        WP = pywt.WaveletPacket(
            data=None, wavelet=self._wavelet, mode=self._mode, maxlevel=self.__level
        )
...
class WaveletTransformationMapper(_WaveletMapper):
    ...
    def _wm_reverse(self, wd):
        ...
        for indexes in _get_indexes(wd.shape, self._dim):
            ...
            wd_coeffs = [
                wd_sample[wd_offsets[i] : wd_offsets[i + 1]] for i in range(nlevels)
            ]
