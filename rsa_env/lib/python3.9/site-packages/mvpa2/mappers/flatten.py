...
class FlattenMapper(Mapper):
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(FlattenMapper, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["shape", "maxdims"])
        )
    ...
    def _forward_data(self, data):
        ...
        if oshape is None:
            raise RuntimeError(
                "FlattenMapper needs to be trained before it " "can be used."
            )
        # at least the first feature axis has to match match
        if oshape[0] != sshape[0]:
            raise ValueError(
                "FlattenMapper has not been trained for data "
                "shape '%s' (known only '%s')." % (str(sshape), str(oshape))
            )
    ...
    def _forward_dataset(self, dataset):
        ...
        for k in dataset.fa:
            ...
            if __debug__:
                debug(
                    "MAP_",
                    "Broadcasting fa '%s' %s %d times" % (k, attr.shape, multiplier),
                )
...
class ProductFlattenMapper(FlattenMapper):
    ...
    def __init__(self, factor_names, factor_values=None, **kwargs):
        ...
        if factor_values is not None:
            if len(factor_values) != len(factor_names):
                raise ValueError(
                    "factor_values must have %d elements, "
                    "found %d" % (len(factor_names), len(factor_names))
                )
    ...
    def __repr__(self, prefixes=None):
        ...
        return super(ProductFlattenMapper, self).__repr__(
            prefixes=prefixes + _repr_attrs(self, ["factor_names", "factor_values"])
        )
    ...
    def _check_factor_name_values(self, ds):
        ...
        for nm, value in zip(*(self._factor_names, self._factor_values)):
            if any(ds.a[nm].value != value):
                raise ValueError(
                    "Mismatch for attribute %s: %s != %s" % (nm, value, ds.a[nm].value)
                )
    ...
    def _forward_dataset(self, dataset):
        ...
        for i, (name, value) in enumerate(factor_names_values):
            ...
            for repr, postfix in ((value, None), (np.arange(len(value)), "_indices")):
                ...
                if repr_arr.shape[1:] != oshape:
                    raise ValueError(
                        "Shape mismatch: %s != %s - this should"
    ...
    def _check_is_trained(self):
        if self._factor_values is None or self.shape is None:
            raise ValueError(
                "Dataset is not trained, and factor_values not "
                "given in constructor of %s" % self.__class__.__name__
            )
    ...
    def _reverse_dataset(self, dataset):
        ...
        for (
            nm,
            values,
        ) in factor_names_values:  #:self._get_reversed_factor_name_values(mds):
        ...
        if shape is not None:
            ...
            if not shape == mask.shape:
                raise ValueError(
                    ...
                    "compatible with the provided shape %s." % (mask.shape, shape)
                )
    ...
    mapper = ChainMapper(
        [
            ...
            StaticFeatureSelection(
                flatmask, dshape=flatmask.shape, oshape=(len(flatmask.nonzero()[0]),)
            ),
        ]
    )
    return mapper
