...
class BoxcarMapper(Mapper):
    ...
    def __init__(self, startpoints, boxlength, offset=0, **kwargs):
        ...
        if np.issubdtype(startpoints.dtype, "i"):
            ...
            if __debug__:
                debug(
                    "MAP",
                    "Boxcar: obtained startpoints are not of int type."
        ...
        self.__selectors = [
            slice(i + offset, i + offset + boxlength) for i in startpoints
        ]
    ...
    def __reduce__(self):
        ...
        if badguy in state:
            del state[badguy]
        return self.__class__, (self.startpoints, self.boxlength, self.offset), state
    ...
    def __repr__(self):
        ...
        return s.replace(
            "(",
            "(boxlength=%d, offset=%d, startpoints=%s, "
    ...
    def forward1(self, data):
        ...
        if not self._outshape:
            raise RuntimeError(
                "BoxcarMapper needs to be trained before " ".forward1() can be used."
            )
        # first axes need to match
        if not data.shape[0] == self._outshape[2]:
            raise ValueError(
                "Data shape %s does not match sample shape %s."
    ...
    def _forward_dataset(self, dataset):
        ...
        if self.get_space():
            mds.fa[self.get_space() + "_offsetidx"] = np.arange(
                self.boxlength, dtype="int"
            )
    ...
    def reverse1(self, data):
        if __debug__:
            if not data.shape == self._outshape[1:]:
                raise ValueError(
                    "BoxcarMapper has not been train to "
    ...
    def _reverse_data(self, data):
        ...
        if not data.shape[1] == self.boxlength:
            ...
            raise ValueError(
                "%s cannot reverse-map, since the number of "
                "elements along the second axis (%i) does not "
                "match the boxcar-length (%i)."
                % (self.__class__.__name__, data.shape[1], self.boxlength)
            )
