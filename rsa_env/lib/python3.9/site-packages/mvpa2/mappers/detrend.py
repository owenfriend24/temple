...
class PolyDetrendMapper(Mapper):
    ...
    polyord = Parameter(
        1,
        doc="""Order of the Legendre polynomial to remove from the data.  This
    ...
    chunks_attr = Parameter(
        None,
        doc="""If None, the whole dataset is detrended at once. Otherwise, the given
    ...
    opt_regs = Parameter(
        None,
        doc="""List of sample attribute names that should be used as
    ...
    def _train(self, ds):
        ...
        # chunk-wise detrending is desired
            ...
            if not is_sequence_type(polyord):
                ...
                raise ValueError(
                    "If you specify a sequence of polyord values "
    ...
    def _forward_dataset(self, ds):
        ...
        if inspace is None and len(regs) != len(ds):
            raise ValueError(
                "Cannot detrend the dataset, since it neither "
                ...
                " and was trained on %i)." % (len(ds), len(regs))
            )
        ...
        if inspace is not None:
            if inspace in ds.sa:
                ...
                if not np.all(space_coords == polycoords):
                    ...
                    reg_idx = [
                        np.argwhere(polycoords == c).flatten()[0] for c in space_coords
                    ]
    ...
    def _forward_data(self, data):
        raise RuntimeError("%s cannot map plain data." % self.__class__.__name__)
