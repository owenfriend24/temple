...
class ZScoreMapper(Mapper):
    ...
    def _train(self, ds):
        ...
        if params is not None:
            ...
            if param_est is not None:
                ...
                est_ids = set(get_samples_by_attr(ds, est_attr, est_attr_values))
            else:
                est_ids = slice(None)
    ...
    def _forward_dataset(self, ds):
        ...
        if __debug__ and chunks_attr is not None:
            ...
            min_nsamples_per_chunk = np.min(list(nsamples_per_chunk.values()))
    ...
    def _forward_data(self, data):
        ...
        if self.__param_est is not None:
            raise RuntimeError(
                "%s cannot do Z-scoring with estimating "
    ...
    def _zscore(self, samples, mean, std):
        ...
        if np.isscalar(mean) or samples.shape[1] == len(mean):
            ...
            raise RuntimeError("mean should be a per-feature vector. Got: %r" % (mean,))
