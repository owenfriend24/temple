...
class TransposeMapper(Mapper):
    ...
    def _swap_samples_and_feature_axes(self, ds):
        out = Dataset(np.swapaxes(ds.samples, 0, 1), sa=ds.fa, fa=ds.sa, a=ds.a)
        return out
...
class AddAxisMapper(Mapper):
    ...
    def _forward_dataset(self, ds):
        ...
        if pos == 0:
            ...
            out_fa = dict(
                [
                    (attr, np.repeat(ds.fa[attr].value[None], len(ds), axis=0))
                    for attr in ds.fa
                ]
            )
        elif pos == 1:
            ...
            out_fa = ds.fa
        out = Dataset(
            ds.samples.__getitem__(tuple(slicer)), sa=out_sa, fa=out_fa, a=ds.a
        )
        return out
