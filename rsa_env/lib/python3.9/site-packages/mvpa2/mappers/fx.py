...
class FxMapper(Mapper):
    ...
    def __init__(
        self, axis, fx, fxargs=None, uattrs=None, attrfx="merge", order="uattrs"
    ):
        """
        Parameters
        ----------
        ...
        if not axis in ["samples", "features"]:
            raise ValueError(
                "%s `axis` arguments can only be 'samples' or "
    ...
    def __smart_apply_along_axis(self, data):
        ...
        try:
            ...
            if inspect.getargs(self.__fx.__code__).args[1] == "axis":
                ...
                debug("FX", "Will apply %s via apply_along_axis", self.__fx)
        ...
        if fx is not None:
            if __debug__:
                debug(
                    "FX",
                    "Applying %s directly to data giving axis=%d",
                    (self.__fx, naxis),
                )
    ...
    def _forward_data(self, data):
        if self.__uattrs is not None:
            raise RuntimeError(
                "%s does not support forward-mapping of plain "
                "data when data grouping based on attributes "
                "is requested" % self.__class__.__name__
            )
    ...
    def _forward_dataset_grouped(self, ds):
        ...
        self.__attrcombs = dict(
            list(zip(self.__uattrs, [col[attr].unique for attr in self.__uattrs]))
        )
        ...
        for comb in _orthogonal_permutations(self.__attrcombs):
            selector = reduce(
                ...
                [
                    ...
                    for attr, value in list(comb.items())
                ],
            )
        ...
        if order:
            ...
            # and attributes
            attrs = dict((k, [v[i] for i in order_idxs]) for k, v in list(attrs.items()))
        ...
        if self.__axis == "samples":
            attrs = dict(
                list(
                    zip(
                        list(ds.sa.keys()),
                        [self.__attrfx(ds.sa[attr].value) for attr in ds.sa],
                    )
                )
            )
        if self.__axis == "features":
            attrs = dict(
                list(
                    zip(
                        list(ds.fa.keys()),
                        [self.__attrfx(ds.fa[attr].value) for attr in ds.fa],
                    )
                )
            )
        return mdata, attrs
...
class BinomialProportionCI(Mapper):
    ...
    def _forward_data(self, data):
        ...
        return binomial_proportion_ci_from_bool(
            data, axis=0, alpha=1 - self.params.width, meth=self.params.meth
        )
...
class BinaryFxNode(Node):
    ...
    def _call(self, ds):
        ...
        if not values.shape == targets.shape:
            ...
            # boolean
            raise ValueError(
                "Trying to compute an error between data of "
                "different shape (%s vs. %s)." % (values.shape, targets.shape)
            )
    ...
    def _forward_data(self, data):
        ...
        if self.in_place:
            if not np.issubdtype(mdata.dtype, float):
                warning(
                    "Integer dtype. Mean removal won't work correctly for "
