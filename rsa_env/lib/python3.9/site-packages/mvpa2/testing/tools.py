...
if externals.exists("nose"):
    ...
    ok_ = (
        ...
        with_setup
    ) = raises = istest = nottest = make_decorator = assert_is_instance = _need_nose
        ...
        if isinstance(v1, np.ndarray):
            ...
            try:
                assert_equal(
                    v1,
                    v2,
                    msg="Values for key %s have different values: %s and %s"
                    % (k, v1, v2),
                )
...
def assert_datasets_almost_equal(
    x, y, ignore_a=None, ignore_sa=None, ignore_fa=None, decimal=6
):
    """
    Parameters
    ----------
...
def assert_datasets_equal(x, y, ignore_a=None, ignore_sa=None, ignore_fa=None):
    ...
    assert_datasets_almost_equal(
        x, y, ignore_a=ignore_a, ignore_sa=ignore_sa, ignore_fa=ignore_fa, decimal=None
    )
...
if externals.exists("mock"):
    ...
    def assert_warnings(messages):
        with mock.patch("warnings.warn") as mock_warnings:
            ...
            warning_list = [(call[2]["category"], call[1][0]) for call in mock_calls]
            assert_equal(messages, warning_list)
...
def skip_if_no_external(dep, ver_dep=None, min_version=None, max_version=None):
    ...
    if min_version is not None and externals.versions[ver_dep] < min_version:
        raise SkipTest(
            ...
            ". Test was skipped." % (min_version, ver_dep, externals.versions[ver_dep])
        )
    ...
    def decorate(func):
        def newfunc(*arg, **kw):
            ...
            if __debug__:
                debug(
                    "TEST",
                    "Running %s with temporary filename %s" % (func.__name__, filename),
                )
            try:
            ...
            for i in range(niter):
                try:
                    ...
                    if __debug__:
                        debug(
                            "TEST",
                            "Upon %i-th run, test %s failed with %s",
                            (i, func.__name__, e),
                        )
            ...
            for x_, y_ in zip(x, y):
                ...
                if strict and isinstance(x_, np.ndarray) and not (x_.dtype == y_.dtype):
                    raise AssertionError(
                        "dtypes %r and %r do not match" % (x_.dtype, y_.dtype)
                    )
                assert_objectarray_equal(x_, y_, xorig, yorig)
        except Exception as e:
            if not isinstance(e, AssertionError):
                raise AssertionError("%r != %r, thus %s != %s" % (x, y, xorig, yorig))
            raise
